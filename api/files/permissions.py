...
class CheckedOutOrAdmin(permissions.BasePermission):
    def has_object_permission(self, request, view, obj):
        assert isinstance(
            obj, BaseFileNode,
        ), 'obj must be a BaseFileNode, got {}'.format(obj)
    ...
    def has_object_permission(self, request, view, obj):
        ...
        if hasattr(obj.target, 'primary_file') and obj.target.primary_file == obj:
            if (
                ...
                and obj.target.machine_state != DefaultStates.INITIAL.value
            ):
                return False
            ...
            return super(IsPreprintFile, self).has_object_permission(
                request, view, obj.target,
            )
