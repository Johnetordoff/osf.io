...
def get_bannable_urls(instance):
    ...
    for host in get_varnish_servers():
        # add instance url
        varnish_parsed_url = urllib.parse.urlparse(host)
        parsed_absolute_url = urllib.parse.urlparse(instance.absolute_api_v2_url)
        ...
        if isinstance(instance, Comment):
            try:
                parsed_target_url = urllib.parse.urlparse(instance.target.referent.absolute_api_v2_url)
            ...
            try:
                parsed_root_target_url = urllib.parse.urlparse(instance.root_target.referent.absolute_api_v2_url)
            except AttributeError:
