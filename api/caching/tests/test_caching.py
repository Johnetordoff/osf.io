...
class TestVarnish(DbTestCase):
    ...
    def test_compare_python_responses_to_varnish_responses(self):
        ...
        data_dict = dict(
            nodes=dict(), users=dict(), comments=dict(), registrations=dict(),
        )
        ...
        for key, embed_values in querystrings.items():
            ...
            while len(embed_values) > 0:
                ...
                python_url = '{}{}/?embed={}&{}&esi=false'.format(
                    self.local_python_base_url, key, generated_qs, querystring_suffix,
                )
                varnish_url = '{}{}/?embed={}&{}&esi=true'.format(
                    self.local_varnish_base_url, key, generated_qs, querystring_suffix,
                )
                python_resp = requests.get(python_url, timeout=120)
                python_authed_resp = requests.get(
                    python_url, auth=self.authorization, timeout=120,
                )

                varnish_resp = requests.get(varnish_url, timeout=120)
                varnish_authed_resp = requests.get(
                    varnish_url, auth=self.authorization, timeout=120,
                )

                python_data[key]['_'.join(embed_values,)] = python_resp.json()
                self.validate_keys(
                    python_resp.json(), original_embed_values,
                )
                ...
                self.validate_keys(
                    python_authed_resp.json(), original_embed_values,
                )

                varnish_data[key]['_'.join(embed_values,)] = varnish_resp.json()
                self.validate_keys(
                    varnish_resp.json(), original_embed_values,
                )
                ...
                self.validate_keys(
                    varnish_authed_resp.json(), original_embed_values,
                )
    ...
    def validate_keys(self, data, embed_keys):
        ...
        for item in data['data']:  # all these should be lists.
            if 'embeds' in item.keys():
                ...
                ), 'Embed key mismatch: \n{}\n{}'.format(item__embed_keys, embed_keys,)
    ...
    def test_cache_invalidation(self):
        payload = dict(
            data=dict(
                type='nodes',
                attributes=dict(title='Awesome Test Node Title', category='project',),
            ),
        )
