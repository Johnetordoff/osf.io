...
def parse_datetimes(query_params):
    ...
    using_time = False
    if (start_datetime and 'T' in start_datetime) or (
        end_datetime and 'T' in end_datetime
    ):
        using_time = True
    ...
    if on_date and (start_datetime or end_datetime):
        raise ValidationError(
            'You cannot provide both an on date and an end or start datetime.',
        )
    ...
    if end_datetime and not start_datetime:
        raise ValidationError(
            'You cannot provide a specific end_datetime with no start_datetime',
        )
    ...
    if on_date:
        ...
        default_start = (now - timedelta(default_days_back + 1)).replace(
            hour=0, minute=0, second=0, microsecond=0,
        )
        default_end = (now - timedelta(1)).replace(
            hour=23, minute=59, second=59, microsecond=999,
        )
        ...
        try:
            start_datetime = (
                datetime.strptime(start_datetime, format_to_use).replace(
                    tzinfo=pytz.utc,
                )
                if start_datetime
                else default_start
            ...
            end_datetime = (
                datetime.strptime(end_datetime, format_to_use).replace(tzinfo=pytz.utc)
                if end_datetime
                else default_end
            )
        except ValueError:
            raise ValidationError(
                'You cannot use a mixture of date format and datetime format.',
            )
