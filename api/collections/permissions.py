...
class CanSubmitToCollectionOrPublic(permissions.BasePermission):
    def has_object_permission(self, request, view, obj):
        ...
        ), 'obj must be a Collection or CollectionSubmission, got {}'.format(obj)
        if isinstance(obj, CollectionSubmission):
            obj = obj.collection
        ...
        if request.method in permissions.SAFE_METHODS:
            return (
                obj.is_public
                or auth.user
                and auth.user.has_perm('read_collection', obj)
            )
        accepting_submissions = (
            obj.is_public and obj.provider and obj.provider.allow_submissions
        )
        return auth.user and (
            accepting_submissions or auth.user.has_perm('write_collection', obj)
        )
    ...
    def has_object_permission(self, request, view, obj):
        ...
        if request.method in permissions.SAFE_METHODS:
            return (
                ...
                or auth.user
                and auth.user.has_perm('read_collection', collection)
            )
...
class CollectionWriteOrPublicForPointers(permissions.BasePermission):
    ...
    def has_object_permission(self, request, view, obj):
        ...
        ), 'obj must be an Collection or CollectionSubmission, got {}'.format(obj)
        auth = get_user_auth(request)
