...
class JSONAPIBaseView(generics.GenericAPIView):
    ...
    def _get_embed_partial(self, field_name, field):
        ...
        def partial(item):
            ...
            view_kwargs.update({'request': request, 'is_embedded': True,},)
            ...
            if not view.get_serializer_class() in cache:
                cache[view.get_serializer_class()] = view.get_serializer_class()(
                    many=isinstance(view, ListModelMixin),
    ...
    def get_serializer_context(self):
        ...
        if 'fields[{}]'.format(serializer_class_type) in self.request.query_params:
            ...
            sparse_fields = self.request.query_params[
                'fields[{}]'.format(serializer_class_type)
            ]
        ...
        for field in fields_check:
            if (
                getattr(fields_check[field], 'always_embed', False)
                and field not in embeds
            ):
...
class LinkedRegistrationsRelationship(
    JSONAPIBaseView, generics.RetrieveUpdateDestroyAPIView, generics.CreateAPIView,
):
...
def root(request, format=None, **kwargs):
    ...
    flags = [
        flag.name for flag in Flag.objects.all() if flag.is_active(request._request)
    ]
    samples = [
        name
        for name in Sample.objects.values_list('name', flat=True)
...
class BaseChildrenList(JSONAPIBaseView, NodesFilterMixin):
    ...
    def get_queryset(self):
        ...
        node_pks = (
            node.node_relations.filter(is_node_link=False)
        ...
        return (
            ...
            .can_view(auth.user, auth.private_link)
            .order_by('-modified')
        )
    ...
    def build_query_from_field(self, field_name, operation):
        if field_name == 'permission':
            ...
            if query_val == READ:
                ...
                return Q(
                    user_id__in=(
                        resource.get_group(WRITE).user_set.values_list('id', flat=True)
        ...
        return super(BaseContributorList, self).build_query_from_field(
...
class BaseLinkedList(JSONAPIBaseView, generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        return (
            self.get_node()
            .linked_nodes.filter(is_deleted=False)
    ...
    def bulk_get_file_nodes_from_wb_resp(self, files_list):
        ...
        for item in files_list:
            ...
            base_class = BaseFileNode.resolve_class(
                attrs['provider'],
                BaseFileNode.FOLDER if attrs['kind'] == 'folder' else BaseFileNode.FILE,
            )
...
class DeprecatedView(JSONAPIBaseView):
    ...
    def determine_version(self, request, *args, **kwargs):
        version, scheme = super(DeprecatedView, self).determine_version(
            request, *args, **kwargs
        )
        if StrictVersion(version) > StrictVersion(self.max_version):
            self.is_deprecated = True
            raise NotFound(
                detail='This route has been deprecated. It was last available in version {}'.format(
                    self.max_version,
                ),
            )
    ...
    def finalize_response(self, request, response, *args, **kwargs):
        response = super(DeprecatedView, self).finalize_response(
        ...
        if response.status_code == 204:
            ...
            response.data = {}
        deprecation_warning = 'This route is deprecated and will be unavailable after version {}'.format(
            self.max_version,
        )
