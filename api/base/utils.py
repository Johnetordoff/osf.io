        ...
        try:
            ...
            if isinstance(query, dict):
                obj = (
                    ...
                    else model_cls.objects.filter(**query).select_for_update().get()
                )
            else:
                obj = (
                    model_cls.objects.get(query)
...
def default_node_list_queryset(model_cls):
    ...
    return model_cls.objects.filter(is_deleted=False).annotate(
...
def default_node_list_permission_queryset(user, model_cls):
    ...
    qs = default_node_permission_queryset(user, model_cls) & default_node_list_queryset(
        model_cls,
    )
...
def is_deprecated(request_version, min_version=None, max_version=None):
    ...
    min_version_deprecated = min_version and StrictVersion(
        request_version,
    ) < StrictVersion(str(min_version))
    max_version_deprecated = max_version and StrictVersion(
        request_version,
    ) > StrictVersion(str(max_version))
...
def assert_resource_type(obj, resource_tuple):
    ...
    assert isinstance(obj, resource_tuple), 'obj must be {} {}; got {}'.format(
        a_or_an, error_message, obj,
    )
