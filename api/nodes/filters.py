    ...
    def build_query_from_field(self, field_name, operation):
        if field_name == 'parent':
            if operation['op'] == 'eq':
                if operation['value']:
                    # filter[parent]=<nid>
                    parent = utils.get_object_or_error(
                        AbstractNode,
                        operation['value'],
                        self.request,
                        display_name='parent',
                ...
                if not operation['value']:
                    ...
                    child_ids = (
                        NodeRelation.objects.filter(is_node_link=False,)
        ...
        if field_name == 'root':
            ...
            return (
                ~with_as_root_query if operation['op'] == 'ne' else with_as_root_query
            )

        if field_name == 'preprint':
            preprint_query = Q(preprints_exist=True)
            return (
                preprint_query
                if utils.is_truthy(operation['value'])
                else ~preprint_query
            )

        return super(NodesFilterMixin, self).build_query_from_field(
    ...
    def build_query_from_field(self, field_name, operation):
        if field_name == 'current_user_permissions':
            ...
            if user.is_anonymous:
                ...
                return (
                    Q()
                    if StrictVersion(self.request.version) < StrictVersion('2.11')
                    and perm == permissions.READ
                    else Q(id__in=[])
                )
        ...
        return super(UserNodesFilterMixin, self).build_query_from_field(
            field_name, operation,
        )

    def build_node_list(self, user, perm, with_superuser=False):
        return Node.objects.get_nodes_for_user(user, permission=perm).values_list(
            'id', flat=True,
        )
