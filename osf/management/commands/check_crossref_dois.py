...
def check_crossref_dois(dry_run=True):
    ...
    while preprints:
        ...
        for preprint in preprint_batch:
            ...
            pending_dois.append(
                'doi:{}'.format(
                    settings.DOI_FORMAT.format(prefix=prefix, guid=preprint._id)
                )
            )
        ...
        try:
            ...
            logger.error(
                'Could not contact crossref to check for DOIs, response returned with exception {}'.format(
                    exc
                )
            )
            raise exc
...
def report_stuck_dois(dry_run=True):
    ...
    if preprints_with_pending_dois:
        ...
        logger.info(
            'There were {} stuck registrations for CrossRef, email sent to help desk'.format(
                preprints_with_pending_dois.count()
            )
        )
