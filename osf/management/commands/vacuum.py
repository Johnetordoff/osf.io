...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        models = [apps.get_model(each) for each in model_names]
        table_names = [each._meta.db_table for each in models]
        statement_format = 'VACUUM ANALYZE {table};' if analyze else 'VACUUM {table};'
        statements = [statement_format.format(table=table) for table in table_names]
        if dry:
