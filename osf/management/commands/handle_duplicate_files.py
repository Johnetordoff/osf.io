...
def compare_location_and_region(first_file, second_file):
    ...
    if first_loc and second_loc:
        return (
            ...
            and first_file['versions__region_id'] == first_file['versions__region_id']
        )
    return False
...
def inspect_duplicates(file_summary):
    ...
    for duplicate_record in file_summary:
        ...
        if not safe_to_remove:
            error_counter += 1
            logger.info(
                'Contact admins to resolve: target: {}, name: {}'.format(
                    target._id, name
                )
            )

    logger.info(
        '{}/{} errors to manually resolve.'.format(error_counter, len(file_summary))
    )
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for file_type in file_types:
            ...
            logger.info(
                '{} run time {}'.format(file_type, type_finish_time - type_start_time)
            )
