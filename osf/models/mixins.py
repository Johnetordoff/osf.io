...
class SpamOverrideMixin(SpamMixin):
    ...
    def confirm_ham(self, save=False, train_akismet=True):
        ...
        if self.logs.filter(action__in=[self.log_class.FLAG_SPAM, self.log_class.CONFIRM_SPAM]):
            ...
            # set objects to prior public state if known, unless it's a registration pending approval or embargoed.
            if spam_log.params.get('was_public', False) and not getattr(self, 'is_pending_registration', False) and not getattr(self, 'is_pending_embargo', False):
