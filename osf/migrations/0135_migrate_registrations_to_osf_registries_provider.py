...
def migrate_existing_registrations_into_osf_registries(state, schema):
    ...
    try:
        osf_registries = AbstractProvider.objects.get(
            _id='osf', type='osf.registrationprovider'
        )
    except AbstractProvider.DoesNotExist:
        # Allow test / local dev DBs to pass
        logger.warn(
            'Unable to find OSF Registries provider - assuming test environment.'
        )
        pass
    else:
        ...
        assert (updated_drafts, updated_registrations) == (
            draft_registrations.count(),
            registrations.count(),
        )
        logger.info(
            'Successfully migrated {} draft registrations and {} public registrations into OSFRegistries'.format(
                updated_drafts, updated_registrations
            )
        )
...
def remove_existing_registrations_from_osf_registries(state, schema):
    ...
    try:
        ...
        total_registrations = (
            osf_registries.registrations.count()
            + osf_registries.draft_registrations.count()
        )
        ...
        logger.info(
            'Successfully removed {} public and draft registrations from OSFRegistries'.format(
                total_registrations
            )
        )
