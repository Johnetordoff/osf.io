...
def unquote_folder_paths(state, schema):
    ...
    for obj in targets:
        try:
            obj.folder_path = urllib.parse.unquote(obj.folder_path).decode('utf-8')
        except UnicodeEncodeError:
            obj.folder_path = urllib.parse.unquote(obj.folder_path)
...
def quote_folder_paths(state, schema):
    try:
        ...
        return
    for obj in targets:
        obj.folder_path = urllib.parse.quote(obj.folder_path.encode('utf-8'))
    bulk_update(targets, update_fields=['folder_path'])
