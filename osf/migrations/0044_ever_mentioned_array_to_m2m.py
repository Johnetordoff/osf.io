...
def migrate_user_guid_array_to_m2m(state, schema):
    ...
    for comment in Comment.objects.exclude(_ever_mentioned=[]).all():
        for user in AddableOSFUser.objects.filter(
            id__in=[
                FindableOSFUser.objects.get(guids___id=_id).id
                for _id in comment._ever_mentioned
            ]
        ):
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RenameField(
            model_name='comment', old_name='ever_mentioned', new_name='_ever_mentioned'
        ),
        migrations.AddField(
            model_name='comment',
            name='ever_mentioned',
        ...
        migrations.RemoveField(model_name='comment', name='_ever_mentioned'),
    ]
