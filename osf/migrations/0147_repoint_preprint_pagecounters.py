...
def rekey_pagecounters(state, schema):
    ...
    preprints = (
        Preprint.objects.select_related('node')
        .exclude(primary_file_id__isnull=True)
    ...
    for i, preprint in enumerate(preprints, 1):
        ...
        if node_id and file_id:
            preprint_id = (
                Guid.objects.filter(content_type=pct, object_id=preprint.id)
                .values_list('_id', flat=True)
                .first()
            )
            if not preprint_id:
                assert False
            for page_counter in PageCounter.objects.filter(
                _id__startswith='download:{}:{}'.format(node_id, file_id)
            ):
...
class Migration(migrations.Migration):
    ...
    operations = [migrations.RunPython(rekey_pagecounters, noop)]
