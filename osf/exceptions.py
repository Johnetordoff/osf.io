...
class InvalidSanctionRejectionToken(TokenError):
    ...
    message_long = (
        'This disapproval link is invalid. Are you logged into the correct account?'
    )
...
class InvalidSanctionApprovalToken(TokenError):
    ...
    message_short = 'Invalid Token'
    message_long = (
        'This approval link is invalid. Are you logged into the correct account?'
    )
...
class InvalidTriggerError(Exception):
    def __init__(self, trigger, state, valid_triggers):
        ...
        self.message = 'Cannot trigger "{}" from state "{}". Valid triggers: {}'.format(
            trigger, state, valid_triggers
        )
...
class InvalidTransitionError(Exception):
    def __init__(self, machine, transition):
        self.message = 'Machine "{}" received invalid transitions: "{}" expected but not defined'.format(
            machine, transition
        )
