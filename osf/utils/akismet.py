...
class AkismetClient(object):
    ...
    def _is_apikey_valid(self):
        if self._apikey_is_valid is not None:
            ...
            res = requests.post(
                ...
                data={'key': self.apikey, 'blog': self.website},
    ...
    def check_comment(self, user_ip, user_agent, **kwargs):
        ...
        data = {k: kwargs.get(k) for k in ALLOWED_ARGS if k in kwargs}
        data['blog'] = self.website
        data['user_ip'] = user_ip
        data['user_agent'] = user_agent

        res = requests.post(
            '{}{}.{}/1.1/comment-check'.format(
                self.API_PROTOCOL, self.apikey, self.API_HOST
            ),
            data=data,
            headers=self._default_headers,
            timeout=5,
