...
def list_displayable_fields(cls):
    return [
        ...
        if x.editable and not x.is_relation and not x.primary_key
    ]
...
class OSFUserAdmin(admin.ModelAdmin):
    ...
    def formfield_for_manytomany(self, db_field, request, **kwargs):
        ...
        return super(OSFUserAdmin, self).formfield_for_manytomany(
    ...
    def save_related(self, request, form, formsets, change):
        ...
        """
        groups_to_preserve = list(
            form.instance.groups.filter(
                Q(name__startswith='preprint_')
                | Q(name__startswith='node_')
                | Q(name__startswith='osfgroup_')
                | Q(name__startswith='collections_')
