...
class TestNodeSerializers(OsfTestCase):
    ...
    def test_serialize_node_summary_private_node_should_include_id_and_primary_boolean_reg_and_fork(
        self,
    ):
        user = UserFactory()
        # user cannot see this node
        node = ProjectFactory(is_public=False)
        result = serialize_node_summary(node, auth=Auth(user), primary=True,)
    ...
    def test_serialize_node_summary_for_registration_uses_correct_date_format(self):
        ...
        assert_equal(
            res['registered_date'], reg.registered_date.strftime('%Y-%m-%d %H:%M UTC')
        )

    # https://github.com/CenterForOpenScience/openscienceframework.org/issues/858
    def test_serialize_node_summary_private_registration_should_include_is_registration(
        self,
    ):
        user = UserFactory()
    ...
    def test_get_children_only_returns_child_nodes_with_admin_permissions(self):
        ...
        admin_project.add_contributor(
            user, auth=Auth(admin_project.creator), permissions=permissions.ADMIN
        )
        admin_project.save()

        admin_component = NodeFactory(parent=admin_project)
        admin_component.add_contributor(
            user, auth=Auth(admin_component.creator), permissions=permissions.ADMIN
        )
        admin_component.save()

        read_and_write = NodeFactory(parent=admin_project)
        read_and_write.add_contributor(
            user, auth=Auth(read_and_write.creator), permissions=permissions.WRITE
        )
        read_and_write.save()
        read_only = NodeFactory(parent=admin_project)
        read_only.add_contributor(
            user, auth=Auth(read_only.creator), permissions=permissions.READ
        )
        read_only.save()
    ...
    def test_serialize_node_summary_private_fork_should_include_is_fork(self):
        ...
        res = serialize_node_summary(fork, auth=Auth(user), primary=True,)
    ...
    def test_serialize_node_summary_is_contributor_osf_group(self):
        ...
        res = _view_project(project, auth=Auth(user),)
        ...
        assert_equal(
            set(res['user']['permissions']), set([permissions.READ, permissions.WRITE])
        )
    ...
    def test_serialize_node_search_returns_only_visible_contributors(self):
        ...
        assert_equal(
            serialized_node['firstAuthor'], node.visible_contributors[0].family_name
        )
...
class TestViewProject(OsfTestCase):
    ...
    def test_view_project_pending_registration_for_write_contributor_does_not_contain_cancel_link(
        ...
        self.node.add_contributor(
            write_user, permissions=permissions.WRITE, auth=Auth(self.user), save=True
        )
        pending_reg = RegistrationFactory(project=self.node, archive=True)
        assert_true(pending_reg.is_pending_registration)
...
class TestViewProjectEmbeds(OsfTestCase):
    ...
    def test_view_project_embed_registrations_sorted_by_registered_date_descending(
        ...
        data = _view_project(
            node=self.project, auth=Auth(self.project.creator), embed_registrations=True
        )
    ...
    def test_view_project_embed_descendants(self):
        ...
        res = _view_project(
            self.project, auth=Auth(self.project.creator), embed_descendants=True
        )
...
class TestGetReadableDescendants(OsfTestCase):
    ...
    def test__get_readable_descendants(self):
        ...
        nodes, all_readable = _get_readable_descendants(
            auth=Auth(project.creator), node=project
        )
    ...
    def test__get_readable_descendants_masked_by_permissions(self):
        ...
        project1.add_contributor(
            userA, auth=Auth(self.user), permissions=permissions.READ
        )
        project1.add_contributor(
            userB, auth=Auth(self.user), permissions=permissions.READ
        )

        component2 = ProjectFactory(creator=self.user, title='Two')
        component2.add_contributor(
            userA, auth=Auth(self.user), permissions=permissions.READ
        )

        component3 = ProjectFactory(creator=self.user, title='Three')
        component3.add_contributor(
            userA, auth=Auth(self.user), permissions=permissions.READ
        )
        component3.add_contributor(
            userB, auth=Auth(self.user), permissions=permissions.READ
