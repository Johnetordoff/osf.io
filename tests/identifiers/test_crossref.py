...
class TestCrossRefClient:
    @responses.activate
    def test_crossref_create_identifiers(
        self, preprint, crossref_client, crossref_success_response
    ):
        responses.add(
            responses.Response(
                responses.POST,
    ...
    @responses.activate
    def test_crossref_update_identifier(
        self, preprint, crossref_client, crossref_success_response
    ):
        responses.add(
            responses.Response(
    ...
    def test_crossref_build_doi(self, crossref_client, preprint):
        ...
        assert crossref_client.build_doi(preprint) == settings.DOI_FORMAT.format(
            prefix=doi_prefix, guid=preprint._id
        )
    ...
    def test_crossref_build_metadata(self, crossref_client, preprint):
        ...
        # header
        assert (
            root.find('.//{%s}doi_batch_id' % crossref.CROSSREF_NAMESPACE).text
            == preprint._id
        )
        assert (
            root.find('.//{%s}depositor_name' % crossref.CROSSREF_NAMESPACE).text
            == crossref.CROSSREF_DEPOSITOR_NAME
        )
        assert (
            root.find('.//{%s}email_address' % crossref.CROSSREF_NAMESPACE).text
            == test_email
        )
        ...
        assert (
            root.find('.//{%s}group_title' % crossref.CROSSREF_NAMESPACE).text
            == preprint.provider.name
        )
        assert (
            root.find('.//{%s}title' % crossref.CROSSREF_NAMESPACE).text
            == preprint.title
        )
        assert root.find(
            './/{%s}item_number' % crossref.CROSSREF_NAMESPACE
        ).text == 'osf.io/{}'.format(preprint._id)
        assert (
            root.find('.//{%s}abstract/' % crossref.JATS_NAMESPACE).text
            == preprint.description
        )
        assert (
            root.find('.//{%s}license_ref' % crossref.CROSSREF_ACCESS_INDICATORS).text
            == 'https://creativecommons.org/licenses/by/4.0/legalcode'
        )
        assert root.find(
            './/{%s}license_ref' % crossref.CROSSREF_ACCESS_INDICATORS
        ).get('start_date') == preprint.date_published.strftime('%Y-%m-%d')

        assert (
            root.find('.//{%s}intra_work_relation' % crossref.CROSSREF_RELATIONS).text
            == preprint.article_doi
        )
        assert root.find(
            './/{%s}doi' % crossref.CROSSREF_NAMESPACE
        ).text == settings.DOI_FORMAT.format(
            prefix=preprint.provider.doi_prefix, guid=preprint._id
        )
        assert (
            root.find('.//{%s}resource' % crossref.CROSSREF_NAMESPACE).text
            == settings.DOMAIN + preprint._id
        )

        metadata_date_parts = [
            elem.text
            for elem in root.find('.//{%s}posted_date' % crossref.CROSSREF_NAMESPACE)
        ]
    ...
    @responses.activate
    @mock.patch(
        'osf.models.preprint.update_or_enqueue_on_preprint_updated', mock.Mock()
    )
    ...
    def test_process_crossref_name(self, crossref_client):
        ...
        assert meta == {
            'surname': imputed_names['family'],
            'given_name': imputed_names['given'],
        }
        ...
        meta = crossref_client._process_crossref_name(contributor)
        assert meta == {
            'surname': unparsable_fullname[: crossref.CROSSREF_SURNAME_LIMIT]
        }

    def test_metadata_for_single_name_contributor_only_has_surname(
        self, crossref_client, preprint
    ):
        ...
        assert (
            contributors.find('.//{%s}surname' % crossref.CROSSREF_NAMESPACE).text
            == 'Madonna'
        )
    ...
    def test_metadata_contributor_orcid(self, crossref_client, preprint):
        ...
        # verified ORCID
        contributor = preprint.creator
        contributor.external_identity = {'ORCID': {ORCID: 'VERIFIED'}}
        contributor.save()
        ...
        assert contributors.find(
            './/{%s}ORCID' % crossref.CROSSREF_NAMESPACE
        ).text == 'https://orcid.org/{}'.format(ORCID)
        assert contributors.find(
            './/{%s}ORCID' % crossref.CROSSREF_NAMESPACE
        ).attrib == {'authenticated': 'true'}

        # unverified (only in profile)
        contributor.external_identity = {}
        contributor.social = {'orcid': ORCID}
        contributor.save()
    ...
    def test_metadata_none_license_update(self, crossref_client, preprint):
        ...
        assert (
            root.find('.//{%s}license_ref' % crossref.CROSSREF_ACCESS_INDICATORS)
            is None
        )
        assert (
            root.find(
                './/{%s}program' % crossref.CROSSREF_ACCESS_INDICATORS
            ).getchildren()
            == []
        )
    ...
    def test_metadata_for_non_included_relation(self, crossref_client, preprint):
        ...
        xml_without_relation = crossref_client.build_metadata(
            preprint, include_relation=False
        )
        root_without_relation = lxml.etree.fromstring(xml_without_relation)
        assert (
            root_without_relation.find(
                './/{%s}intra_work_relation' % crossref.CROSSREF_RELATIONS
            )
            is None
        )
