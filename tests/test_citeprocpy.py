...
class Node:
    ...
    csl = {
        ...
        'author': [{'given': u'Henrique', 'family': u'Harman'}],
        'URL': 'localhost:5000/2nthu',
        'issued': {'date-parts': [[2016, 12, 6]]},
        'title': u'The study of chocolate in its many forms',
        'type': 'webpage',
        'id': u'2nthu',
    }
...
class TestCiteprocpy(OsfTestCase):
    ...
    def test_failing_citations(self):
        ...
        url_data_path = os.path.join(
            os.path.dirname(__file__), '../website/static/citeprocpy_test_data.json'
        )
...
class TestCiteprocpyMLA(OsfTestCase):
    ...
    def setUp(self):
        ...
        self.formated_date = date.strftime(self.MLA_DATE_FORMAT).format(
            month=self.MLA_MONTH_MAP[date.month]
        )
    ...
    def test_render_citations_mla_one_author(self):
        ...
        assert_equal(
            citation,
            u'Tordoff, John. “{}.” {}, {}. Web.'.format(
                self.preprint.title, self.preprint.provider.name, self.formated_date
            ),
        )
        ...
        assert_equal(
            citation,
            u'Tordoff, John, Junior. “{}.” {}, {}. Web.'.format(
                self.preprint.title, self.preprint.provider.name, self.formated_date
            ),
        )
    ...
    def test_citation_osf_provider(self):
        ...
        assert_equal(
            citation,
            u'Tordoff, John. “{}” {}, {}. Web.'.format(
                self.preprint.title, 'OSF Preprints', self.formated_date
            ),
        )
    ...
    def test_three_authors(self):
        ...
        assert_equal(
            citation,
            u'Tordoff, John, et al. “{}.” {}, {}. Web.'.format(
                self.preprint.title, self.preprint.provider.name, self.formated_date
            ),
        )
        ...
        assert_equal(
            citation,
            u'Tordoff, John, Jr., et al. “{}.” {}, {}. Web.'.format(
                self.preprint.title, self.preprint.provider.name, self.formated_date
            ),
        )
