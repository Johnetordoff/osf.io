...
class RegistrationRetractionModelsTestCase(OsfTestCase):
    ...
    def test__initiate_retraction_does_not_create_tokens_for_unregistered_admin(self):
        ...
        assert_equal(
            Contributor.objects.get(
                node=self.registration, user=unconfirmed_user
            ).permission,
            permissions.ADMIN,
        )
    ...
    def test__initiate_retraction_adds_admins_on_child_nodes(self):
        ...
        project.add_contributor(
            project_non_admin, auth=Auth(project.creator), save=True
        )
    ...
    def test_pending_retract(self):
        ...
        assert_equal(
            self.registration.retraction.justification, self.valid_justification
        )
        assert_equal(self.registration.retraction.initiated_by, self.user)
        assert_equal(
            self.registration.retraction.initiation_date.date(), timezone.now().date()
        )
    ...
    def test_long_justification_raises_ValidationValueError(self):
        with assert_raises(DataError):
            self.registration.retract_registration(
                self.user, self.invalid_justification
            )
            self.registration.save()
    ...
    def test_retraction_of_registration_pending_embargo_cancels_embargo(self):
        ...
        approval_token = self.registration.retraction.approval_state[self.user._id][
            'approval_token'
        ]
    ...
    def test_approval_adds_to_parent_projects_log(self):
        ...
        assert_equal(
            self.registration.registered_from.logs.count(), initial_project_logs + 2
        )
    ...
    def test_approval_of_registration_with_embargo_adds_to_parent_projects_log(self):
        ...
        assert_equal(
            self.registration.registered_from.logs.count(), initial_project_logs + 4
        )
    ...
    def test_two_approvals_with_two_admins_retracts(self):
        ...
        approval_token = self.registration.retraction.approval_state[self.admin2._id][
            'approval_token'
        ]
    ...
    def test_non_admin_rejection_token_raises_PermissionsError(self):
        ...
        rejection_token = self.registration.retraction.approval_state[self.user._id][
            'rejection_token'
        ]
        with assert_raises(PermissionsError):
            self.registration.retraction.disapprove_retraction(
    ...
    def test__on_complete_makes_project_and_components_public(self):
        ...
        grandchild = NodeFactory(
            creator=grandchild_admin, parent=child, is_public=False
        )  # noqa
...
class RegistrationWithChildNodesRetractionModelTestCase(OsfTestCase):
    def setUp(self):
        ...
        self.component = NodeFactory(
            creator=self.user, parent=self.project, title='Component'
        )
        self.subproject = ProjectFactory(
            creator=self.user, parent=self.project, title='Subproject'
        )
        self.subproject_component = NodeFactory(
            creator=self.user, parent=self.subproject, title='Subcomponent'
        )
...
class RegistrationRetractionApprovalDisapprovalViewsTestCase(OsfTestCase):
    def setUp(self):
        ...
        self.registration = RegistrationFactory(
            is_public=True, project=self.registered_from
        )
        self.registration.retract_registration(self.user)
        self.registration.save()
    ...
    def test_GET_approve_with_non_existant_sanction_returns_HTTPError_BAD_REQUEST(self):
        res = self.app.get(
            self.registration.web_url_for(
                'token_action', token=self.token_without_sanction
            ),
            auth=self.user.auth,
            expect_errors=True,
        )
...
class ComponentRegistrationRetractionViewsTestCase(OsfTestCase):
    def setUp(self):
        ...
        self.project = ProjectFactory(is_public=True, creator=self.user)
        self.component = NodeFactory(
            is_public=True, creator=self.user, parent=self.project, title='Component'
        )
        self.subproject = ProjectFactory(
            is_public=True, creator=self.user, parent=self.project, title='Subproject'
        )
        self.subproject_component = NodeFactory(
            is_public=True,
            creator=self.user,
        ...
        self.component_registration = self.registration._nodes.order_by(
            'created'
        ).first()
        self.subproject_registration = list(
            self.registration._nodes.order_by('created')
        )[1]
        self.subproject_component_registration = self.subproject_registration._nodes.order_by(
            'created'
        ).first()

    def test_POST_retraction_to_component_returns_HTTPError_BAD_REQUEST(self):
        res = self.app.post_json(
            self.component_registration.api_url_for(
    ...
    def test_POST_retraction_to_subproject_returns_HTTPError_BAD_REQUEST(self):
        res = self.app.post_json(
            self.subproject_registration.api_url_for(
                'node_registration_retraction_post'
            ),
            auth=self.auth,
            expect_errors=True,
    ...
    def test_POST_retraction_to_subproject_component_returns_HTTPError_BAD_REQUEST(
        ...
        res = self.app.post_json(
            self.subproject_component_registration.api_url_for(
                'node_registration_retraction_post'
            ),
            auth=self.auth,
            expect_errors=True,
...
class RegistrationRetractionViewsTestCase(OsfTestCase):
    def setUp(self):
        ...
        self.registration = RegistrationFactory(
            project=self.registered_from, is_public=True
        )
    ...
    def test_GET_retraction_page_when_pending_retraction_returns_HTTPError_BAD_REQUEST(
        ...
        res = self.app.get(
            self.retraction_get_url, auth=self.user.auth, expect_errors=True,
        )
        assert_equal(res.status_code, http_status.HTTP_400_BAD_REQUEST)
    ...
    def test_POST_retraction_does_not_send_email_to_unregistered_admins(
        ...
        self.registration.save()
        self.app.post_json(
            self.retraction_post_url, {'justification': ''}, auth=self.user.auth,
        )
    ...
    def test_POST_active_embargo_returns_HTTPOK(self):
        ...
        approval_token = self.registration.embargo.approval_state[self.user._id][
            'approval_token'
        ]
    ...
    def test_POST_retraction_by_non_admin_retract_HTTPError_UNAUTHORIZED(self):
        ...
        res = self.app.post_json(
            self.retraction_post_url, auth=self.group_mem.auth, expect_errors=True
        )
        assert_equal(res.status_code, http_status.HTTP_403_FORBIDDEN)
    ...
    def test_valid_POST_retraction_adds_to_parent_projects_log(self, mock_send):
        ...
        assert_equal(
            self.registration.registered_from.logs.count(), initial_project_logs + 1
        )
    ...
    def test_non_contributor_GET_approval_returns_HTTPError_UNAUTHORIZED(self):
        ...
        approval_url = self.registration.web_url_for(
            'token_action', token=approval_token
        )
    ...
    def test_non_contributor_GET_disapproval_returns_HTTPError_UNAUTHORIZED(self):
        ...
        disapproval_url = self.registration.web_url_for(
            'token_action', token=rejection_token
        )
        res = self.app.get(
            disapproval_url, auth=non_contributor.auth, expect_errors=True
        )
        assert_equal(res.status_code, http_status.HTTP_401_UNAUTHORIZED)
        assert_true(self.registration.is_pending_retraction)
        assert_false(self.registration.is_retracted)
        ...
        # group admin on node fails disapproval GET
        res = self.app.get(
            disapproval_url, auth=self.group_mem.auth, expect_errors=True
        )
        assert_equal(res.status_code, http_status.HTTP_401_UNAUTHORIZED)
