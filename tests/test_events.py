...
class TestEventExists(OsfTestCase):
    ...
    def test_get_file_updated(self):
        ...
        event = event_registry['file_updated'](
            self.user, self.node, 'file_updated', payload=file_payload
        )
        assert_is_instance(event, FileUpdated)

    def test_get_file_added(self):
        # Event gets FileAdded from file_added
        event = event_registry['file_added'](
            self.user, self.node, 'file_added', payload=file_payload
        )
        assert_is_instance(event, FileAdded)

    def test_get_file_removed(self):
        # Event gets FileRemoved from file_removed
        event = event_registry['file_removed'](
            self.user, self.node, 'file_removed', payload=file_deleted_payload
        )
        assert_is_instance(event, FileRemoved)

    def test_get_folder_created(self):
        # Event gets FolderCreated from folder_created
        event = event_registry['folder_created'](
            self.user, self.node, 'folder_created', payload=folder_created_payload
        )
    ...
    def test_get_file_moved(self):
        ...
        event = event_registry['addon_file_moved'](
            self.user, self.node, 'addon_file_moved', payload=file_moved_payload
        )
    ...
    def test_get_file_copied(self):
        ...
        event = event_registry['addon_file_copied'](
            self.user, self.node, 'addon_file_copied', payload=file_copied_payload
        )
    ...
    def test_get_file_renamed(self):
        ...
        event = event_registry['addon_file_renamed'](
            self.user, self.node, 'addon_file_renamed', payload=file_rename_payload
        )
...
class TestFileUpdated(OsfTestCase):
    def setUp(self):
        ...
        self.sub.save()
        self.event = event_registry['file_updated'](
            self.user_2, self.project, 'file_updated', payload=file_payload
        )

    def test_info_formed_correct(self):
        assert_equal('{}_file_updated'.format(wb_path), self.event.event_type)
        assert_equal(
            'updated file "<b>{}</b>".'.format(materialized.lstrip('/')),
            self.event.html_message,
        )
        assert_equal(
            'updated file "{}".'.format(materialized.lstrip('/')),
...
class TestFileAdded(NotificationTestCase):
    def setUp(self):
        ...
        self.event = event_registry['file_added'](
            self.user2, self.project, 'file_added', payload=file_payload
        )

    def test_info_formed_correct(self):
        assert_equal('{}_file_updated'.format(wb_path), self.event.event_type)
        assert_equal(
            'added file "<b>{}</b>".'.format(materialized.lstrip('/')),
            self.event.html_message,
        )
        assert_equal(
            'added file "{}".'.format(materialized.lstrip('/')), self.event.text_message
        )
...
class TestFileRemoved(NotificationTestCase):
    ...
    def test_info_formed_correct_file(self):
        ...
        assert_equal(
            'removed file "<b>{}</b>".'.format(materialized.lstrip('/')),
            self.event.html_message,
        )
        assert_equal(
            'removed file "{}".'.format(materialized.lstrip('/')),
            self.event.text_message,
        )
    ...
    def test_info_formed_correct_folder(self):
        ...
        assert_equal(
            u'removed folder "<b>{}/</b>".'.format(materialized.lstrip('/')),
            self.event.html_message,
        )
        assert_equal(
            u'removed folder "{}/".'.format(materialized.lstrip('/')),
            self.event.text_message,
        )
...
class TestFolderFileRenamed(OsfTestCase):
    ...
    def test_rename_file_html(self):
        ...
        assert_equal(
            self.event.html_message,
            'renamed file "<b>/One/Paper13.txt</b>" to "<b>/One/Paper14.txt</b>".',
        )
    ...
    def test_rename_folder_html(self):
        ...
        assert_equal(
            self.event.html_message,
            'renamed folder "<b>/One/Two/Three</b>" to "<b>/One/Two/Four</b>".',
        )
    ...
    def test_rename_file_text(self):
        ...
        assert_equal(
            self.event.text_message,
            'renamed file "/One/Paper13.txt" to "/One/Paper14.txt".',
        )
    ...
    def test_rename_folder_text(self):
        ...
        assert_equal(
            self.event.text_message,
            'renamed folder "/One/Two/Three" to "/One/Two/Four".',
        )
...
class TestFileMoved(NotificationTestCase):
    ...
    def test_perform_store_one_of_each(self, mock_store):
        ...
        self.private_node.add_contributor(
            self.user_3, permissions=WRITE, auth=self.auth
        )
...
class TestFileCopied(NotificationTestCase):
    ...
    def test_info_correct(self):
        ...
        assert_equal(
            (
                'copied file "<b>One/Paper13.txt</b>" from OSF Storage'
                ' in Consolidate to "<b>Two/Paper13.txt</b>" in OSF'
                ' Storage in Consolidate.'
            ),
            self.event.html_message,
        )
        assert_equal(
            (
                'copied file "One/Paper13.txt" from OSF Storage'
...
class TestCategorizeUsers(NotificationTestCase):
    ...
    def test_warn_user(self):
        ...
        moved, warn, removed = utils.categorize_users(
            ...
            self.event.source_node,
            self.event.event_type,
            self.event.node,
        )
        assert_equal(
            {email_transactional: [], email_digest: [self.user_3._id], 'none': []}, warn
        )
        assert_equal(
    ...
    def test_moved_user(self):
        ...
        assert_equal(
            {email_transactional: [self.user_3._id], email_digest: [], 'none': []},
            moved,
        )
    ...
    def test_node_permissions(self):
        ...
        warn = {
            email_transactional: [],
            email_digest: [self.user_3._id, self.user_4._id],
            'none': [],
        }
        subbed, remove = utils.subscriptions_node_permissions(
            self.private_node, warn, remove
        )
        assert_equal(
        ...
        assert_equal(
            {email_transactional: [], email_digest: [self.user_4._id], 'none': []},
            remove,
        )
...
class TestSubscriptionManipulations(OsfTestCase):
    def setUp(self):
        ...
        self.diff_1_3 = {
            email_transactional: ['d1234'],
            'none': ['g1234'],
            email_digest: ['a1234'],
        }
        self.union_1_2 = {
            'email_transactional': ['e1234', 'f1234', 'd1234', 'k1234'],
    ...
    def test_subscription_user_union(self):
        ...
        assert set(self.union_1_2['email_transactional']) == set(
        ...
        (
            ...
            OrderedDict(
                [
                    ...
                    (
                        u'etag',
                        u'10485efa4069bb94d50588df2e7466a079d49d4f5fd7bf5b35e7c0d5b12d76b7',
                    ),
                    (u'extra', OrderedDict([(u'downloads', 0), (u'version', 30)])),
                    (u'kind', u'file'),
...
file_deleted_payload = OrderedDict(
    [
        ...
        (
            u'metadata',
            OrderedDict([(u'materialized', materialized), (u'path', materialized)]),
        ),  # Deleted files don't get wb_paths
        ...
        (
            ...
            OrderedDict(
                [
                    (
                        u'etag',
                        u'5caf8ab73c068565297e455ebce37fd64b6897a2284ec9d7ecba8b6093082bcd',
                    ),
                    (u'extra', OrderedDict()),
                    (u'kind', u'folder'),
...
def file_move_payload(new_node, old_node):
    return OrderedDict(
        [
            ...
            (
                u'auth',
                OrderedDict([(u'email', 'Bob'), (u'id', 'bob2'), (u'name', 'Bob')]),
            ),
            (
                u'destination',
                OrderedDict(
                    [
                        ...
                        (
                            'url',
                            '/project/nhgts/files/osfstorage/5581cb50a24f710b0f4623f9/',
                        ),
                        (
                            'node',
                            {
                                'url': '/{}/'.format(new_node._id),
                                '_id': new_node._id,
                                'title': u'Consolidate2',
                            },
                        ),
                        ('addon', 'OSF Storage'),
                    ]
            ...
            (
                ...
                OrderedDict(
                    [
                        ...
                        (
                            u'etag',
                            u'16075ae3e546971003095beef8323584de40b1fcbf52ed4bb9e7f8547e322824',
                        ),
                        (u'extra', OrderedDict([(u'downloads', 0), (u'version', 30)])),
                        (u'kind', u'file'),
                        ...
                        (
                            'url',
                            '/project/nhgts/files/osfstorage/558ac45da24f714eff336d59/',
                        ),
                        (
                            'node',
                            {
            ...
            (
                ...
                OrderedDict(
                    [
                        ...
                        (
                            u'etag',
                            u'0e9bfddcb5a59956ae60e93f32df06b174ad33b53d8a2f2cd08c780cf34a9d93',
                        ),
                        (
                            u'extra',
                            OrderedDict(
                                    ...
                                    (
                                        ...
                                        OrderedDict(
                                            [
                                                (
                                                    u'md5',
                                                    u'79a64594dd446674ce1010007ac2bde7',
                                                ),
                                                (
                                                    u'sha256',
                                                    u'bf710301e591f6f5ce35aa8971cfc938b39dae0fedcb9915656dded6ad025580',
                                                ),
                                            ]
                                        ),
                                    ),
                        ...
                        (
                            'url',
                            '/project/wp6xv/files/osfstorage/55f07134a24f71b2a24f4812/',
                        ),
                        (
                            'node',
                            {
            ...
            (
                ...
                OrderedDict(
                    [
                        ...
                        (
                            'url',
                            '/project/wp6xv/files/osfstorage/Fibery/%21--i--2.pdf/',
                        ),
                        (
                            'node',
                            {
