...
def component_one_payload(component_one):
    return {'data': [{'type': 'nodes', 'id': component_one._id}]}
...
class TestViewOnlyLinksNodes:
    ...
    def test_view_only_links_nodes(
        self, app, user, read_only_user, read_write_user, non_contributor, url
    ):
...
class TestViewOnlyLinkNodesSet:
    ...
    def test_admin_can_set_multiple_nodes(
        ...
        payload = {
            'data': [
                {'type': 'nodes', 'id': component_one._id},
                {'type': 'nodes', 'id': component_two._id},
            ]
        }
    ...
    def test_set_nodes_does_not_duplicate_nodes(
        self, app, user, public_project, component_one, view_only_link, url
    ):
        payload = {
            'data': [
                {'type': 'nodes', 'id': public_project._id},
                {'type': 'nodes', 'id': component_one._id},
                {'type': 'nodes', 'id': component_one._id},
            ]
        }
    ...
    def test_set_node_not_component(self, app, user, project_two, url):
        ...
        """
        payload = {'data': [{'type': 'nodes', 'id': project_two._id},]}
        res = app.post_json_api(url, payload, auth=user.auth, expect_errors=True)
        assert res.status_code == 400
        assert res.json['errors'][0][
            'detail'
        ] == "The node {0} cannot be affiliated with this View Only Link because the node you're trying to affiliate is not descended from the node that the View Only Link is attached to.".format(
            project_two._id
        )
    ...
    def test_set_node_second_level_component_without_first_level_parent(
        ...
        """
        payload = {'data': [{'type': 'nodes', 'id': second_level_component._id},]}
        res = app.post_json_api(url, payload, auth=user.auth, expect_errors=True)
        view_only_link.reload()
        assert res.status_code == 201
    ...
    def test_set_node_second_level_component_with_first_level_parent(
        ...
        payload = {
            'data': [
                {'type': 'nodes', 'id': first_level_component._id},
                {'type': 'nodes', 'id': second_level_component._id},
            ]
        }
        res = app.post_json_api(url, payload, auth=user.auth)
        view_only_link.reload()
    ...
    def test_view_only_link_nodes_set_errors(
        ...
        payload = {'data': [{'type': 'nodes', 'id': 'abcde'}]}
        res = app.post_json_api(url, payload, auth=user.auth, expect_errors=True)
        assert res.status_code == 404

        #   test_type_required_in_payload
        payload = {'data': [{'id': component_one._id}]}
        res = app.post_json_api(url, payload, auth=user.auth, expect_errors=True)
        assert res.status_code == 400
        ...
        res = app.post_json_api(
            url, component_one_payload, auth=read_write_user.auth, expect_errors=True
        )
        assert res.status_code == 403

        #   test_read_only_contributor_cannot_set_nodes
        res = app.post_json_api(
            url, component_one_payload, auth=read_only_user.auth, expect_errors=True
        )
        assert res.status_code == 403

        #   test_logged_in_user_cannot_set_nodes
        res = app.post_json_api(
            url, component_one_payload, auth=non_contributor.auth, expect_errors=True
        )
        assert res.status_code == 403
...
class TestViewOnlyLinkNodesUpdate:
    ...
    def test_admin_can_update_nodes_multiple_nodes_to_add(
        ...
        update_payload['data'].append({'type': 'nodes', 'id': component_two._id})
        res = app.put_json_api(url, update_payload, auth=user.auth)
        view_only_link.reload()
        assert res.status_code == 200
    ...
    def test_admin_can_update_nodes_multiple_add_multiple_remove(
        ...
        payload = {
            'data': [
                {'type': 'nodes', 'id': public_project._id,},
                {'type': 'nodes', 'id': component_three._id},
                {'type': 'nodes', 'id': component_four._id},
            ]
        }
    ...
    def test_update_nodes_no_changes(
        ...
        payload = {'data': [{'type': 'nodes', 'id': public_project._id,}]}
    ...
    def test_update_nodes_top_level_node_not_included(
        ...
        """
        payload = {'data': [{'type': 'nodes', 'id': component_one._id}]}
        res = app.put_json_api(url, payload, auth=user.auth, expect_errors=True)
        assert res.status_code == 400
        assert res.json['errors'][0][
            'detail'
    ...
    def test_view_only_link_nodes_update_errors(
        ...
        #   test_id_required_in_payload
        payload = {
            'data': [{'type': 'nodes', 'id': public_project._id}, {'type': 'nodes'}]
        }
        res = app.put_json_api(url, payload, auth=user.auth, expect_errors=True)
        assert res.status_code == 400
        ...
        res = app.put_json_api(
            url, update_payload, auth=read_write_user.auth, expect_errors=True
        )
        assert res.status_code == 403

        #   test_read_only_contributor_cannot_update_nodes
        res = app.put_json_api(
            url, update_payload, auth=read_only_user.auth, expect_errors=True
        )
        assert res.status_code == 403

        #   test_logged_in_user_cannot_update_nodes
        res = app.put_json_api(
            url, update_payload, auth=non_contributor.auth, expect_errors=True
        )
        assert res.status_code == 403
