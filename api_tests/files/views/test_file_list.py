...
class TestNodeFileList:
    ...
    def deleted_file(self, user, node):
        deleted_file = api_utils.create_test_file(node, user, filename='file_two')
        deleted_file.delete(user=user, save=True)
        return deleted_file

    def test_does_not_return_trashed_files(self, app, user, node, file, deleted_file):
        res = app.get(
            '/{}nodes/{}/files/osfstorage/'.format(API_BASE, node._id), auth=user.auth
        )
...
class TestFileFiltering:
    ...
    def file_two(self, user, node):
        return api_utils.create_test_file(node, user, filename='file_two')

    @pytest.fixture()
    def file_three(self, user, node):
        return api_utils.create_test_file(node, user, filename='file_three')

    @pytest.fixture()
    def file_four(self, user, node):
        return api_utils.create_test_file(node, user, filename='file_four')

    def test_get_all_files(
        self, app, user, node, file_one, file_two, file_three, file_four
    ):
        res = app.get(
            '/{}nodes/{}/files/osfstorage/'.format(API_BASE, node._id), auth=user.auth
        )
    ...
    def test_filter_on_single_tag(
        ...
        # test_filter_on_tag
        res = app.get(
            '/{}nodes/{}/files/osfstorage/?filter[tags]=new'.format(API_BASE, node._id),
            auth=user.auth,
        )
        data = res.json.get('data')
        assert len(data) == 2
        ...
        res = app.get(
            '/{}nodes/{}/files/osfstorage/?filter[tags]=cat'.format(API_BASE, node._id),
            auth=user.auth,
        )
        assert len(res.json.get('data')) == 1

    def test_filtering_on_multiple_tags(self, app, user, node, file_one):
    ...
    def test_filtering_by_tags_returns_distinct(self, app, user, node, file_one):
