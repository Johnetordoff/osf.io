...
class TestNodeRequestListCreate(NodeRequestTestMixin):
    ...
    def create_payload(self):
        return {
            'data': {
                'attributes': {'comment': 'ASDFG', 'request_type': 'access'},
                'type': 'node-requests',
            }
        }

    def test_noncontrib_can_submit_to_public_node(
        self, app, project, noncontrib, url, create_payload
    ):
    ...
    def test_noncontrib_can_submit_to_private_node(
        self, app, project, noncontrib, url, create_payload
    ):
    ...
    def test_contributor_cannot_submit_to_contributed_node(
        self, app, url, write_contrib, create_payload
    ):
        res = app.post_json_api(
            url, create_payload, auth=write_contrib.auth, expect_errors=True
        )
    ...
    def test_write_contrib_cannot_view_requests(
        self, app, url, write_contrib, node_request
    ):
    ...
    def test_requester_cannot_submit_again(
        self, app, url, requester, node_request, create_payload
    ):
        res = app.post_json_api(
            url, create_payload, auth=requester.auth, expect_errors=True
        )
        assert res.status_code == 409
        assert (
            res.json['errors'][0]['detail']
            == 'Users may not have more than one access request per node.'
        )

    def test_requests_disabled_create(
        self, app, url, create_payload, project, noncontrib
    ):
        project.access_requests_enabled = False
        project.save()
        res = app.post_json_api(
            url, create_payload, auth=noncontrib.auth, expect_errors=True
        )
        assert res.status_code == 403
    ...
    def test_email_sent_to_all_admins_on_submit(
        self, mock_mail, app, project, noncontrib, url, create_payload, second_admin
    ):
        project.is_public = True
        project.save()
        res = app.post_json_api(url, create_payload, auth=noncontrib.auth)
    ...
    @mock.patch('website.mails.mails.send_mail')
    def test_email_not_sent_to_parent_admins_on_submit(
        self, mock_mail, app, project, noncontrib, url, create_payload, second_admin
    ):
    ...
    def test_request_followed_by_added_as_contrib(
        elf, app, project, noncontrib, admin, url, create_payload
    ):
        res = app.post_json_api(url, create_payload, auth=noncontrib.auth)
        assert res.status_code == 201
        assert project.requests.filter(
            creator=noncontrib, machine_state='pending'
        ).exists()

        project.add_contributor(noncontrib, save=True)
        assert project.is_contributor(noncontrib)
        assert not project.requests.filter(
            creator=noncontrib, machine_state='pending'
        ).exists()
        assert project.requests.filter(
            creator=noncontrib, machine_state='accepted'
        ).exists()

    def test_filter_by_machine_state(
        self, app, project, noncontrib, url, admin, node_request
    ):
...
class TestPreprintRequestListCreate(PreprintRequestTestMixin):
    ...
    def create_payload(self):
        return {
            'data': {
                'attributes': {'comment': 'ASDFG', 'request_type': 'withdrawal'},
                'type': 'preprint-requests',
            }
        }

    def test_noncontrib_cannot_submit(
        self,
        app,
        noncontrib,
        create_payload,
    ...
    def test_unauth_cannot_submit(
        ...
        for preprint in [pre_mod_preprint, post_mod_preprint, none_mod_preprint]:
            res = app.post_json_api(
                self.url(preprint), create_payload, expect_errors=True
            )
            assert res.status_code == 401

    def test_write_contributor_cannot_submit(
        self,
        app,
        write_contrib,
        create_payload,
    ...
    def test_admin_can_view_requests(
        self, app, admin, pre_request, post_request, none_request
    ):
    ...
    def test_noncontrib_and_write_contrib_cannot_view_requests(
        self, app, noncontrib, write_contrib, pre_request, post_request, none_request
    ):
        for request in [pre_request, post_request, none_request]:
            for user in [noncontrib, write_contrib]:
                res = app.get(
                    self.url(request.target), auth=user.auth, expect_errors=True
                )
                assert res.status_code == 403

    def test_unauth_cannot_view_requests(
        self, app, noncontrib, write_contrib, pre_request, post_request, none_request
    ):
        for request in [pre_request, post_request, none_request]:
    ...
    def test_requester_cannot_submit_again(
        self, app, admin, create_payload, pre_mod_preprint, pre_request
    ):
        res = app.post_json_api(
            self.url(pre_mod_preprint),
            create_payload,
            auth=admin.auth,
            expect_errors=True,
        )
        ...
        assert (
            res.json['errors'][0]['detail']
            == 'Users may not have more than one withdrawal request per preprint.'
        )

    @pytest.mark.skip('TODO: IN-284 -- add emails')
    @mock.patch('website.reviews.listeners.mails.send_mail')
    def test_email_sent_to_moderators_on_submit(
        self, mock_mail, app, admin, create_payload, moderator, post_mod_preprint
    ):
        res = app.post_json_api(
            self.url(post_mod_preprint), create_payload, auth=admin.auth
        )
        assert res.status_code == 201
        assert mock_mail.call_count == 1
