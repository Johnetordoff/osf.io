...
class TestCreateNodeRequestAction(NodeRequestTestMixin):
    ...
    def create_payload(self, _id=None, **attrs):
        ...
        if _id:
            payload['data']['relationships']['target'] = {
                'data': {'type': 'node-requests', 'id': _id}
            }
        return payload
    ...
    def test_requester_can_edit_comment(self, app, requester, url, node_request):
        ...
        payload = self.create_payload(
            node_request._id, trigger='edit_comment', comment='ASDFG'
        )
    ...
    def test_write_contrib_cannot_edit_comment(
        self, app, write_contrib, url, node_request
    ):
        initial_state = node_request.machine_state
        initial_comment = node_request.comment
        payload = self.create_payload(
    ...
    @mock.patch('website.mails.mails.send_mail')
    def test_email_not_sent_on_reject(
        self, mock_mail, app, requester, url, node_request
    ):
        initial_state = node_request.machine_state
        initial_comment = node_request.comment
        payload = self.create_payload(
    ...
    def test_set_permissions_on_approve(self, app, admin, url, node_request):
        assert node_request.creator not in node_request.target.contributors
        payload = self.create_payload(
            node_request._id, trigger='accept', permissions=permissions.ADMIN
        )
        res = app.post_json_api(url, payload, auth=admin.auth)
        assert res.status_code == 201
        node_request.reload()
        assert node_request.target.has_permission(
            node_request.creator, permissions.ADMIN
        )
    ...
    def test_accept_request_defaults_to_read_and_visible(
        self, app, admin, url, node_request
    ):
        assert node_request.creator not in node_request.target.contributors
        payload = self.create_payload(node_request._id, trigger='accept')
        res = app.post_json_api(url, payload, auth=admin.auth)
        ...
        assert node_request.creator in node_request.target.contributors
        assert node_request.target.has_permission(
            node_request.creator, permissions.READ
        )
...
class TestCreatePreprintRequestAction(PreprintRequestTestMixin):
    ...
    def create_payload(self, _id=None, **attrs):
        ...
        if _id:
            payload['data']['relationships']['target'] = {
                'data': {'type': 'preprint-requests', 'id': _id}
            }
        return payload

    def test_no_one_can_view(
        self, app, admin, write_contrib, noncontrib, moderator, url
    ):
        for user in [admin, write_contrib, noncontrib, moderator]:
            res = app.get(url, auth=user.auth, expect_errors=True)
            assert res.status_code == 405

    def test_nonmoderator_cannot_approve(
        self,
        app,
        admin,
        write_contrib,
        ...
        for request in [pre_request, post_request, none_request]:
            ...
            for user in [admin, write_contrib, noncontrib]:
                res = app.post_json_api(
                    url, payload, auth=user.auth, expect_errors=True
                )
                assert res.status_code == 403
                request.reload()
                assert initial_state == request.machine_state

    def test_nonmoderator_cannot_reject(
        self,
        app,
        admin,
        write_contrib,
    ...
    def test_submitter_can_edit_comment(
        self, app, admin, url, pre_request, post_request, none_request
    ):
        for request in [pre_request, post_request, none_request]:
            initial_state = request.machine_state
            initial_comment = request.comment
            payload = self.create_payload(
                request._id, trigger='edit_comment', comment='ASDFG'
            )
            res = app.post_json_api(url, payload, auth=admin.auth)
            assert res.status_code == 201
            request.reload()
    ...
    def test_moderator_can_approve_moderated_requests(
        self, app, moderator, url, pre_request, post_request
    ):
    ...
    def test_moderator_cannot_approve_or_reject_or_edit_comment_nonmoderated_requests(
        self, app, moderator, url, none_request
    ):
        ...
        payload = self.create_payload(
            none_request._id, trigger='edit_comment', comment='ASDFG'
        )
        res = app.post_json_api(url, payload, auth=moderator.auth, expect_errors=True)
        assert res.status_code == 403
        none_request.reload()
    ...
    def test_moderator_cannot_edit_comment_moderated_requests(
        self, app, moderator, url, pre_request, post_request
    ):
        for request in [pre_request, post_request]:
            initial_state = request.machine_state
            ...
            res = app.post_json_api(
                url, payload, auth=moderator.auth, expect_errors=True
            )
            assert res.status_code == 403
            request.reload()
            assert initial_state == request.machine_state
    ...
    def test_write_contrib_and_noncontrib_cannot_edit_comment(
        self,
        app,
        write_contrib,
        noncontrib,
    ...
    def test_email_sent_on_approve(
        self, mock_mail, app, moderator, url, pre_request, post_request
    ):
        for request in [pre_request, post_request]:
            initial_state = request.machine_state
            assert not request.target.is_retracted
    ...
    @mock.patch('website.reviews.listeners.mails.send_mail')
    def test_email_not_sent_on_edit_comment(
        self, mock_mail, app, moderator, url, pre_request, post_request
    ):
        for request in [pre_request, post_request]:
            initial_state = request.machine_state
    ...
    def test_auto_approve(
        self, app, auto_withdrawable_pre_mod_preprint, auto_approved_pre_request
    ):
