...
class CommentDetailMixin(object):
    ...
    def test_private_node_comments_related_auth(
        self, app, user, non_contrib, comment, private_url
    ):
    ...
    def test_comment_has_multiple_links(
        ...
        url_replies = res.json['data']['relationships']['replies']['links']['related'][
            'href'
        ]
        ...
        url_reports = res.json['data']['relationships']['reports']['links']['related'][
            'href'
        ]
    ...
    def test_private_node_comment_auth_misc(
        ...
        res = app.put_json_api(
            private_url, payload, auth=non_contrib.auth, expect_errors=True
        )
        assert res.status_code == 403
        assert (
            res.json['errors'][0]['detail']
    ...
    def test_public_node_comment_update_misc(
        self, app, user, contributor, non_contrib, public_url, public_comment_payload
    ):
        ...
        assert res.status_code == 200
        assert (
            public_comment_payload['data']['attributes']['content']
            == res.json['data']['attributes']['content']
        )
    ...
    def test_private_node_cannot_delete_comment_situation(
        ...
        res = app.delete_json_api(
            private_url, auth=contributor.auth, expect_errors=True
        )
        assert res.status_code == 403
        assert (
            res.json['errors'][0]['detail']
        ...
        res = app.delete_json_api(
            private_url, auth=non_contrib.auth, expect_errors=True
        )
        assert res.status_code == 403
        assert (
            res.json['errors'][0]['detail']
    ...
    def test_private_node_cannot_undelete_comment_situation(
        ...
        res = app.patch_json_api(
            url, payload, auth=contributor.auth, expect_errors=True
        )
        assert res.status_code == 403

        # test_private_node_non_contrib_cannot_undelete_comment
        res = app.patch_json_api(
            url, payload, auth=non_contrib.auth, expect_errors=True
        )
        assert res.status_code == 403
    ...
    def test_public_node_deleted_comments_auth_misc(
        self, app, user, contributor, non_contrib, public_project, public_comment
    ):
        public_comment.is_deleted = True
        public_comment.save()
...
class TestCommentDetailView(CommentDetailMixin):
    ...
    def test_comment_has_target_link_with_correct_type(
        ...
        expected_url = '/{}nodes/{}/'.format(API_BASE, public_project._id)
        target_type = res.json['data']['relationships']['target']['links']['related'][
            'meta'
        ]['type']
        expected_type = 'nodes'
        assert res.status_code == 200
    ...
    def test_registration_comment_has_usable_replies_relationship_link(
        ...
        replies_url = comments_res.json['data'][0]['relationships']['replies']['links'][
            'related'
        ]['href']
        replies_uri = test_utils.urlparse_drop_netloc(replies_url)
        app.get(replies_uri, auth=user.auth)
        node_url = comments_res.json['data'][0]['relationships']['node']['links'][
            'related'
        ]['href']
    ...
    def test_registration_comment_has_usable_node_relationship_link(
        self, app, user, registration, registration_url, registration_comment_reply
    ):
        res = app.get(registration_url, auth=user.auth)
        assert res.status_code == 200
        comments_url = res.json['data']['relationships']['comments']['links'][
...
class TestFileCommentDetailView(CommentDetailMixin):
    ...
    def public_comment(self, user, public_project, public_file):
        return CommentFactory(
            node=public_project, target=public_file.get_guid(), user=user
        )
    ...
    def registration_comment(self, user, registration, registration_file):
        return CommentFactory(
            node=registration, target=registration_file.get_guid(), user=user
        )
    ...
    def test_public_node_non_contrib_commenter_can_update_file_comment(
        ...
        comment = CommentFactory(
            node=project, target=test_file.get_guid(), user=non_contrib
        )
        url = '/{}comments/{}/'.format(API_BASE, comment._id)
        payload = set_up_payload(comment._id)
        res = app.put_json_api(url, payload, auth=non_contrib.auth)
...
class TestWikiCommentDetailView(CommentDetailMixin):
    ...
    def wiki(self, user, private_project):
        with mock.patch('osf.models.AbstractNode.update_search'):
            wiki = WikiFactory(user=user, node=private_project, page_name='not home')
            return wiki

    @pytest.fixture()
    def comment(self, user, private_project, wiki):
        return CommentFactory(
            node=private_project, target=Guid.load(wiki._id), user=user
        )
    ...
    def public_wiki(self, user, public_project):
        with mock.patch('osf.models.AbstractNode.update_search'):
            return WikiFactory(user=user, node=public_project,)

    @pytest.fixture()
    def public_comment(self, user, public_project, public_wiki):
        return CommentFactory(
            node=public_project, target=Guid.load(public_wiki._id), user=user
        )
    ...
    def registration_wiki(self, registration, user):
        with mock.patch('osf.models.AbstractNode.update_search'):
            return WikiFactory(user=user, node=registration,)

    @pytest.fixture()
    def registration_comment(self, user, registration, registration_wiki):
        return CommentFactory(
            node=registration, target=Guid.load(registration_wiki._id), user=user
        )
    ...
    def test_public_node_non_contrib_commenter_can_update_wiki_comment(
        ...
        comment = CommentFactory(
            node=project, target=Guid.load(wiki_page._id), user=non_contrib
        )
        url = '/{}comments/{}/'.format(API_BASE, comment._id)
        payload = set_up_payload(comment._id)
        res = app.put_json_api(url, payload, auth=non_contrib.auth)
