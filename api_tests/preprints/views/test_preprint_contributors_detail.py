...
class TestPreprintContributorDetail:
    ...
    @pytest.fixture()
    def preprint_published(self, user, title, description):
        return PreprintFactory(
            title=title, description=description, is_published=True, creator=user
        )

    @pytest.fixture()
    def preprint_unpublished(self, user, title, description):
        return PreprintFactory(
            title=title, description=description, is_published=False, creator=user
        )
    ...
    def test_get_contributor_detail_errors(
        ...
        res = app.get(
            url_unpublished_base.format(non_contrib._id),
            auth=user.auth,
            expect_errors=True,
        )
        ...
        res = app.get(
            url_unpublished_base.format('invalid'), auth=user.auth, expect_errors=True
        )
        assert res.status_code == 404
    ...
    def test_unregistered_contributor_detail_show_up_as_name_associated_with_preprint(
        ...
        preprint_published.add_unregistered_contributor(
            'Rheisen Dennis', 'reason@gmail.com', auth=Auth(user), save=True
        )
        ...
        assert res.status_code == 200
        assert (
            res.json['data']['embeds']['users']['data']['attributes']['full_name']
            == 'Rheisen Dennis'
        )
        assert (
    ...
    def test_detail_includes_index(self, app, user, preprint_published, url_published):
        res = app.get(url_published)
        data = res.json['data']
...
class TestPreprintContributorOrdering:
    ...
    def test_initial_order(self, app, user, contribs, preprint, contrib_user_id):
        res = app.get(
            '/{}preprints/{}/contributors/'.format(API_BASE, preprint._id),
            auth=user.auth,
        )
        assert res.status_code == 200
    ...
    def test_move_second_contributor_up_one_to_top(
        ...
        assert contrib_user_id(contributor_list[1]) == former_first_contributor._id
    ...
    def test_move_top_contributor_down_to_bottom(
        ...
        data = {
            'data': {
                'id': contributor_id,
                'type': 'contributors',
                'attributes': {'index': last_position},
            }
        }
        res_patch = app.patch_json_api(url, data, auth=user.auth)
        assert res_patch.status_code == 200
        ...
        assert res.status_code == 200
        contributor_list = res.json['data']
        assert contrib_user_id(contributor_list[last_position]) == contributor_to_move
        assert contrib_user_id(contributor_list[0]) == former_second_contributor._id
    ...
    def test_move_second_to_last_contributor_down_past_bottom(
        ...
        data = {
            'data': {
                'id': contributor_id,
                'type': 'contributors',
                'attributes': {'index': last_position + 10},
            }
        }
        res_patch = app.patch_json_api(url, data, auth=user.auth)
        assert res_patch.status_code == 200
    ...
    def test_move_top_contributor_down_to_second_to_last_position_with_negative_numbers(
        ...
        contributor_list = res.json['data']
        assert (
            contrib_user_id(contributor_list[last_position - 1]) == contributor_to_move
        )
        assert contrib_user_id(contributor_list[0]) == former_second_contributor._id
    ...
    def test_write_contributor_fails_to_move_top_contributor_down_one(
        ...
        res_patch = app.patch_json_api(
            url, data, auth=former_second_contributor.auth, expect_errors=True
        )
        ...
        contributor_list = res.json['data']
        assert contrib_user_id(contributor_list[0]) == contributor_to_move
        assert contrib_user_id(contributor_list[1]) == former_second_contributor._id
...
class TestPreprintContributorUpdate:
    ...
    def test_change_contrib_errors(self, app, user, contrib, preprint, url_contrib):

        #   test_change_contributor_no_id
        data = {
            'data': {
                'type': 'contributors',
                'attributes': {'permission': permissions.ADMIN, 'bibliographic': True},
            }
        }
        res = app.put_json_api(url_contrib, data, auth=user.auth, expect_errors=True)
        ...
        data = {
            'data': {
                'id': contrib_id,
                'type': 'contributors',
                'attributes': {'permission': 'invalid', 'bibliographic': 'invalid'},
            }
        }
        res = app.put_json_api(url_contrib, data, auth=user.auth, expect_errors=True)
        assert res.status_code == 400
        assert preprint.get_permissions(contrib) == [
            permissions.READ,
            permissions.WRITE,
        ]
        ...
        data = {
            'data': {
                'id': contrib._id,
                'type': 'contributors',
                'attributes': {'permission': permissions.READ, 'bibliographic': False},
            }
        }
        ...
        res = app.put_json_api(url_contrib, data, auth=contrib.auth, expect_errors=True)
        assert res.status_code == 403
    ...
    def test_change_admin_self_without_other_admin(
        ...
        data = {
            'data': {
                'id': contrib_id,
                'type': 'contributors',
                'attributes': {'permission': permissions.WRITE, 'bibliographic': True},
            }
        }
        res = app.put_json_api(url_creator, data, auth=user.auth, expect_errors=True)
        assert res.status_code == 400

        preprint.reload()
        assert preprint.get_permissions(user) == [
            permissions.READ,
            permissions.WRITE,
            permissions.ADMIN,
        ]
    ...
    def test_node_update_invalid_data(self, app, user, url_creator):
        res = app.put_json_api(
            url_creator, 'Incorrect data', auth=user.auth, expect_errors=True
        )
        assert res.status_code == 400
    ...
    def test_change_contributor_permission_and_bibliographic(
        ...
        with assert_latest_log(
            PreprintLog.PERMISSIONS_UPDATED, preprint, 1
        ), assert_latest_log(PreprintLog.MADE_CONTRIBUTOR_INVISIBLE, preprint):
            contrib_id = '{}-{}'.format(preprint._id, contrib._id)
            data = {
                'data': {
                    'id': contrib_id,
    ...
    def test_not_change_contributor(self, app, user, contrib, preprint, url_contrib):
        with assert_latest_log_not(PreprintLog.PERMISSIONS_UPDATED, preprint):
            ...
            data = {
                'data': {
                    'id': contrib_id,
                    'type': 'contributors',
                    'attributes': {'permission': None, 'bibliographic': True},
                }
            }
            res = app.put_json_api(url_contrib, data, auth=user.auth)
            assert res.status_code == 200
...
class TestPreprintContributorDelete:
    ...
    def test_remove_errors(
        ...
        res = app.delete(
            url_user_write_contrib, auth=user_non_contrib.auth, expect_errors=True
        )
        assert res.status_code == 403
        ...
        res = app.delete(url_user_non_contrib, auth=user.auth, expect_errors=True)
        assert res.status_code == 404
    ...
    def test_can_not_remove_only_bibliographic_contributor(
        ...
        preprint.add_permission(user_write_contrib, permissions.ADMIN, save=True)
    ...
    def test_remove_contributor_non_admin_is_forbidden(
        self, app, user_write_contrib, user_non_contrib, preprint, url_user_non_contrib
    ):
        preprint.add_contributor(
            user_non_contrib, permissions=permissions.WRITE, visible=True, save=True
        )

        res = app.delete(
            url_user_non_contrib, auth=user_write_contrib.auth, expect_errors=True
        )
        assert res.status_code == 403
    ...
    def test_remove_contributor_admin(
        self, app, user, user_write_contrib, preprint, url_user_write_contrib
    ):
    ...
    def test_remove_self_non_admin(
        self, app, user_non_contrib, preprint, url_user_non_contrib
    ):
        preprint.add_contributor(
            user_non_contrib, permissions=permissions.WRITE, visible=True, save=True
        )
