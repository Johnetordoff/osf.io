...
class TestPreprintContributorList(NodeCRUDTestCase):
    ...
    def test_total_bibliographic_contributor_count_returned_in_metadata(
        ...
        preprint_published.add_contributor(
            non_bibliographic_user, visible=False, auth=Auth(preprint_published.creator)
        )
        preprint_published.save()
        res = app.get(url_published, auth=user_two.auth)
        assert res.status_code == 200
    ...
    def test_contributors_order_is_the_same_over_multiple_requests(
        ...
        req_one = app.get(
            '{}?page=2'.format(url_published), auth=Auth(preprint_published.creator)
        )
        req_two = app.get(
            '{}?page=2'.format(url_published), auth=Auth(preprint_published.creator)
        )
...
class TestPreprintContributorAdd(NodeCRUDTestCase):
    ...
    def data_user_three(self, user_three):
        return {
            'data': {
                ...
                'attributes': {'bibliographic': True,},
                'relationships': {
                    'users': {'data': {'type': 'users', 'id': user_three._id,}}
                },
            }
        }
    ...
    def test_add_contributors_errors(
        ...
        res = app.post_json_api(url_published, data, auth=user.auth, expect_errors=True)
        assert res.status_code == 400
    ...
    def test_contributor_create_invalid_data(self, app, user_three, url_published):
        res = app.post_json_api(
            url_published, 'Incorrect data', auth=user_three.auth, expect_errors=True
        )
        assert res.status_code == 400
        assert res.json['errors'][0]['detail'] == exceptions.ParseError.default_detail
    ...
    def test_add_contributor_is_visible_by_default(
        ...
        with assert_latest_log(PreprintLog.CONTRIB_ADDED, preprint_published):
            ...
            res = app.post_json_api(
                url_published, data_user_two, auth=user.auth, expect_errors=True
            )
    ...
    def test_adds_non_bibliographic_contributor_unpublished_preprint_admin(
        ...
        with assert_latest_log(PreprintLog.CONTRIB_ADDED, preprint_unpublished):
            ...
            res = app.post_json_api(
                url_unpublished, data, auth=user.auth, expect_errors=True
            )
            assert res.status_code == 201
            assert res.json['data']['id'] == '{}-{}'.format(
                preprint_unpublished._id, user_two._id
    ...
    def test_adds_contributor_published_preprint_non_admin(
        ...
        res = app.post_json_api(
            url_published, data_user_three, auth=user_two.auth, expect_errors=True
        )
        assert res.status_code == 403
        preprint_published.reload()
    ...
    def test_adds_admin_contributor_unpublished_preprint_admin(
        ...
        with assert_latest_log(PreprintLog.CONTRIB_ADDED, preprint_unpublished):
            ...
            preprint_unpublished.reload()
            assert user_two in preprint_unpublished.contributors
            assert preprint_unpublished.get_permissions(user_two) == [
                permissions.READ,
                permissions.WRITE,
                permissions.ADMIN,
            ]
    ...
    def test_adds_invalid_permission_contributor_unpublished_preprint_admin(
        ...
        data = {
            'data': {
                'type': 'contributors',
                'attributes': {'bibliographic': True, 'permission': 'invalid',},
                'relationships': {
                    'users': {'data': {'id': user_two._id, 'type': 'users'}}
                },
            }
    ...
    def test_adds_already_existing_contributor_unpublished_preprint_admin(
        ...
        res = app.post_json_api(
            url_unpublished, data_user_two, auth=user.auth, expect_errors=True
        )
        assert res.status_code == 400
    ...
    def test_adds_contributor_unpublished_preprint_non_admin(
        ...
        res = app.post_json_api(
            url_unpublished, data_user_three, auth=user_two.auth, expect_errors=True
        )
        assert res.status_code == 403
    ...
    def test_add_unregistered_contributor_with_fullname(
        ...
        with assert_latest_log(PreprintLog.CONTRIB_ADDED, preprint_published):
            ...
            assert res.status_code == 201
            assert (
                res.json['data']['attributes']['unregistered_contributor'] == 'John Doe'
            )
            assert res.json['data']['attributes'].get('email') is None
            assert res.json['data']['embeds']['users']['data'][
                'id'
            ] in preprint_published.contributors.values_list('guids___id', flat=True)
    ...
    def test_add_unregistered_contributor_already_contributor(
        ...
        res = app.post_json_api(
            url_published, payload, auth=user.auth, expect_errors=True
        )
        preprint_published.reload()
        assert res.status_code == 400
    ...
    def test_add_contributor_index_returned(
        self, app, user, data_user_two, data_user_three, url_published
    ):
        res = app.post_json_api(url_published, data_user_two, auth=user.auth)
        assert res.status_code == 201
    ...
    def test_add_contributor_set_index_first(
        ...
        index = list(preprint_published.get_preprintcontributor_order()).index(
            contributor_obj.pk
        )
        assert index == 0
    ...
    def test_add_inactive_merged_user_as_contributor(self, app, user, url_published):
    ...
    def test_add_unconfirmed_user_by_guid(
        ...
        assert res.json['errors'][0][
            ...
            unconfirmed_user._id, preprint_published._id
        )
...
class TestPreprintContributorCreateValidation(NodeCRUDTestCase):
    ...
    def test_add_contributor_validation(self, preprint_published, validate_data):
        ...
        validate_data(
            NodeContributorsCreateSerializer(), preprint_published, user_id='abcde'
        )
        ...
        validate_data(
            NodeContributorsCreateSerializer(), preprint_published, full_name='Kanye'
        )
...
class TestPreprintContributorCreateEmail(NodeCRUDTestCase):
    ...
    def test_add_contributor_needs_preprint_filter_to_send_email(
        self, mock_mail, app, user, user_two, url_preprint_contribs
    ):
        ...
        assert (
            res.json['errors'][0]['detail']
            == 'default is not a valid email preference.'
        )
        assert mock_mail.call_count == 0
    ...
    def test_add_unregistered_contributor_without_email_no_email(
        ...
        url = '{}?send_email=preprint'.format(url_preprint_contribs)
        payload = {
            'data': {'type': 'contributors', 'attributes': {'full_name': 'Kanye West',}}
        }
    ...
    def test_contributor_added_not_sent_if_unpublished(
        ...
        url = '/{}preprints/{}/contributors/?send_email=preprint'.format(
            API_BASE, preprint_unpublished._id
        )
        payload = {
            'data': {
...
class TestPreprintContributorBulkCreate(NodeCRUDTestCase):
    ...
    def test_preprint_contributor_bulk_create_errors(
        ...
        res = app.post_json_api(
            url_published, auth=user.auth, expect_errors=True, bulk=True
        )
        assert res.status_code == 400
    ...
    def test_preprint_contributor_bulk_create_payload_errors(
        ...
        payload = 'sdf;jlasfd'
        res = app.post_json_api(
            url_published, payload, auth=user.auth, expect_errors=True, bulk=True
        )
        assert res.status_code == 400
        assert res.json['errors'][0]['detail'] == exceptions.ParseError.default_detail
...
class TestPreprintContributorBulkUpdate(NodeCRUDTestCase):
    ...
    @pytest.fixture()
    def preprint_published(self, user, user_two, user_three, title, description):
    ...
    @pytest.fixture()
    def preprint_unpublished(self, user, user_two, user_three, title, description):
    ...
    def test_bulk_update_contributors_errors(
        ...
        res = app.put_json_api(
            url_published, empty_payload, auth=user.auth, expect_errors=True, bulk=True
        )
        assert res.status_code == 400
        assert (
        ...
        assert (
            res.json['errors'][0]['detail']
            == '"true and false" is not a valid boolean.'
        )
        ...
        res = app.put_json_api(
            url_published,
            {
                'data': [
                    payload_published_two,
                    {
                        'id': make_contrib_id(preprint_published._id, user._id),
                        'type': 'contributors',
                        'attributes': {
                            'permission': permissions.ADMIN,
                            'bibliographic': False,
    ...
    def test_bulk_update_contributors_published_preprints_logged_in(
        ...
        data = res.json['data']
        assert_equals(
            [data[0]['attributes']['permission'], data[1]['attributes']['permission']],
    ...
    def test_bulk_delete_contributors_errors(
        ...
        res = app.delete_json_api(
            url_published,
            {
                'data': [
                    {
                        'id': make_contrib_id(preprint_published._id, user_four._id),
                        'type': 'contributors',
                    }
                ]
            },
        ...
        res = app.delete_json_api(
            url_published,
            {'data': [{'id': make_contrib_id(preprint_published._id, user_two._id)}]},
            auth=user.auth,
            expect_errors=True,
            bulk=True,
        )
        ...
        res = app.delete_json_api(
            url_published, new_payload, auth=user.auth, expect_errors=True, bulk=True
        )
        assert res.status_code == 400
        assert (
            res.json['errors'][0]['detail'] == 'Could not find all objects to delete.'
