...
class TestSchemaResponseActionDetailGETBehavior:
    ...
    @pytest.fixture()
    def url(self, action):
        return f'/v2/schema_responses/{action.target._id}/actions/{action._id}/'

    def test_schema_response_action_detail(self, app, url, action, schema_response, admin_user):
...
class TestSchemaResponseActionDetailUnsupportedMethods:
    ...
    def test_cannot_POST(self, app, role):
        ...
        resp = app.post_json_api(make_api_url(schema_response), auth, expect_errors=True)
        assert resp.status_code == 405

    @pytest.mark.parametrize('role', USER_ROLES)
    def test_cannot_PUT(self, app, role):
        auth, schema_response, _, _ = configure_test_preconditions(role=role)
        resp = app.put_json_api(make_api_url(schema_response), auth, expect_errors=True)
        assert resp.status_code == 405

    @pytest.mark.parametrize('role', USER_ROLES)
    def test_cannot_PATCH(self, app, role):
        auth, schema_response, _, _ = configure_test_preconditions(role=role)
        resp = app.patch_json_api(make_api_url(schema_response), auth, expect_errors=True)
        assert resp.status_code == 405

    @pytest.mark.parametrize('role', USER_ROLES)
    def test_cannot_DELETE(self, app, role):
        auth, schema_response, _, _ = configure_test_preconditions(role=role)
        resp = app.delete_json_api(make_api_url(schema_response), auth, expect_errors=True)
        assert resp.status_code == 405
