...
def post_payload(type_payload='tokens', scopes=None, name='A shiny updated token'):
    if not scopes:
    ...
    return {
        'data': {
            ...
            'relationships': {'scopes': {'data': [{'type': 'scopes', 'id': scopes}]}},
        }
    }
...
class TestTokenDetailScopesAsRelationships:
    ...
    def url_token_detail(self, user_one, token_user_one):
        path = 'tokens/{}/?version={}'.format(
            token_user_one._id, SCOPES_RELATIONSHIP_VERSION
        )
    ...
    def url_token_list(self):
        return api_v2_url(
            'tokens/?version={}'.format(SCOPES_RELATIONSHIP_VERSION), base_route='/'
        )
    ...
    def test_token_detail_who_can_view(
        self, app, url_token_detail, user_one, user_two, token_user_one
    ):
    ...
    def test_updating_one_field_should_not_blank_others_on_patch_update(
        ...
        res = app.patch_json_api(
            url_token_detail,
            {
                'data': {
                    ...
                    'type': 'tokens',
                    'relationships': {
                        'scopes': {'data': [{'type': 'scopes', 'id': read_scope.name}]}
                    },
                }
            },
            auth=user_one.auth,
        ...
        assert_dict_contains_subset({'name': new_name,}, res.json['data']['attributes'])
        assert res.json['data']['id'] == user_one_token._id
        assert (
            res.json['data']['relationships']['owner']['data']['id']
            == user_one_token.owner._id
        )
    ...
    def test_update_token_does_not_return_token_id(
        ...
        res = app.put_json_api(
            url_token_detail, correct, auth=user_one.auth, expect_errors=True
        )
        assert res.status_code == 200
        assert 'token_id' not in res.json['data']['attributes']
    ...
    @mock.patch('framework.auth.cas.CasClient.revoke_tokens')
    def test_update_token_add_scope(
        self, mock_revoke, app, user_one, token_user_one, url_token_detail
    ):
        mock_revoke.return_value = True
    ...
    def test_token_detail_crud_with_wrong_payload(
        ...
        res = app.delete(url_token_detail, auth=user_two.auth, expect_errors=True)
        assert res.status_code == 403

        # test_create_with_nonexistant_scope_fails
        injected_scope = post_payload(name='A shiny invalid token', scopes='osf.admin')
        res = app.post_json_api(
            url_token_list, injected_scope, auth=user_one.auth, expect_errors=True
        )
        assert res.status_code == 404
        ...
        nonsense_scope = post_payload(
            name='A shiny invalid token', scopes=fake_scope.name
        )
        res = app.post_json_api(
            url_token_list, nonsense_scope, auth=user_one.auth, expect_errors=True
        )
        ...
        injected_scope = post_payload(name='A shiny invalid token', scopes='osf.admin')
        res = app.put_json_api(
            url_token_detail, injected_scope, auth=user_one.auth, expect_errors=True
        )
        assert res.status_code == 404
        ...
        res = app.put_json_api(
            url_token_detail, nonsense_scope, auth=user_one.auth, expect_errors=True
        )
        assert res.status_code == 400
        ...
        res = app.put_json_api(
            url_token_detail, incorrect_type, auth=user_one.auth, expect_errors=True
        )
        ...
        res = app.put_json_api(
            url_token_detail, missing_type, auth=user_one.auth, expect_errors=True
        )
        assert res.status_code == 400
        ...
        res = app.put_json_api(
            url_token_detail, payload, auth=user_one.auth, expect_errors=True
        )
        assert res.status_code == 400
...
def post_attributes_payload(
    type_payload='tokens', scopes='osf.full_write', name='A shiny updated token'
):
    return {
        'data': {'type': type_payload, 'attributes': {'name': name, 'scopes': scopes,}}
    }
...
class TestTokenDetailScopesAsAttributes:
    ...
    def test_token_detail_who_can_view(
        ...
        assert res.json['data']['id'] == token_user_one._id
        assert (
            res.json['data']['attributes']['scopes']
            == token_user_one.scopes.first().name
        )
    ...
    def test_update_token(
        self, mock_revoke, app, user_one, url_token_detail, write_scope
    ):
        mock_revoke.return_value = True
        correct = post_attributes_payload(scopes='osf.full_write')
        res = app.put_json_api(
    ...
    def test_token_detail_crud_with_wrong_payload(
        self, app, url_token_list, url_token_detail, token_user_one, user_one, user_two
    ):
        ...
        injected_scope = post_attributes_payload(
            name='A shiny invalid token', scopes='osf.admin'
        )
        res = app.post_json_api(
            url_token_list, injected_scope, auth=user_one.auth, expect_errors=True
        )
        ...
        nonsense_scope = post_attributes_payload(
            name='A shiny invalid token', scopes='osf.nonsense'
        )
        res = app.post_json_api(
            url_token_list, nonsense_scope, auth=user_one.auth, expect_errors=True
        )
