...
class TestDraftRegistrationDetailEndpoint(TestDraftRegistrationDetail):
    ...
    def url_draft_registrations(self, project_public, draft_registration):
        return '/{}draft_registrations/{}/'.format(API_BASE, draft_registration._id)
    ...
    def test_cannot_view_draft(
        self, app, project_public, user_non_contrib, url_draft_registrations
    ):
    ...
    def test_detail_view_returns_editable_fields(
        ...
        assert (
            Node.load(relationships['branched_from']['data']['id'])
            == draft_registration.branched_from
        )
    ...
    def test_detail_view_returns_editable_fields_no_specified_node(self, app, user):
        ...
        draft_registration = DraftRegistrationFactory(
            initiator=user, branched_from=None
        )
        url = '/{}draft_registrations/{}/'.format(API_BASE, draft_registration._id)
        ...
        assert (
            DraftNode.load(res.json['data']['id']) == draft_registration.branched_from
        )

    def test_draft_registration_perms_checked_on_draft_not_node(
        ...
        res = app.get(
            url_draft_registrations, auth=draft_admin.auth, expect_errors=True
        )
        assert res.status_code == 403
...
@pytest.mark.django_db
class TestDraftRegistrationUpdateWithNode(
    TestDraftRegistrationUpdate, TestUpdateEditableFieldsTestCase
):
    @pytest.fixture()
    def url_draft_registrations(self, project_public, draft_registration):
        return '/{}draft_registrations/{}/'.format(API_BASE, draft_registration._id)
    ...
    def draft_registration(
        self, user, user_read_contrib, user_write_contrib, project_public, schema
    ):
        draft_registration = DraftRegistrationFactory(
    ...
    def schema_open_ended(self):
        return RegistrationSchema.objects.get(
            name='Open-Ended Registration', schema_version=3
        )

    @pytest.fixture
    def draft_registration_open_ended(self, user, schema_open_ended):
        return DraftRegistrationFactory(
            initiator=user, registration_schema=schema_open_ended, branched_from=None
        )
    ...
    def test_update_editable_fields(
        ...
        res = app.put_json_api(
            url_draft_registrations, editable_fields_payload, auth=user.auth
        )
        assert res.status_code == 200
        attributes = res.json['data']['attributes']
        ...
        assert (
            'draft_registrations/{}/subjects'.format(draft_registration._id)
            in relationships['subjects']['links']['related']['href']
        )
        assert (
            'draft_registrations/{}/relationships/subjects'.format(
                draft_registration._id
            )
            in relationships['subjects']['links']['self']['href']
        )

        affiliated_institutions = draft_registration.affiliated_institutions.values_list(
            'id', flat=True
        )
        assert len(affiliated_institutions) == 1
        assert affiliated_institutions[0] == institution_one.id
        assert (
            'draft_registrations/{}/institutions'.format(draft_registration._id)
            in relationships['affiliated_institutions']['links']['related']['href']
        )
        assert (
            'draft_registrations/{}/relationships/institutions'.format(
                draft_registration._id
            )
            in relationships['affiliated_institutions']['links']['self']['href']
        )

        assert (
            'draft_registrations/{}/contributors'.format(draft_registration._id)
            in relationships['contributors']['links']['related']['href']
        )
    ...
    def test_update_upload(
        ...
        res = app.patch_json_api(
            url_draft_registration_open_ended, upload_payload, auth=user.auth
        )
        assert res.status_code == 200
        assert (
            res.json['data']['attributes']['registration_responses']['uploader'][0][
                'file_name'
            ]
            == 'Cafe&LunchMenu.pdf'
        )
    ...
    def test_invalid_editable_category(
        ...
        assert res.status_code == 400
        assert (
            res.json['errors'][0]['detail'] == '"Not a category" is not a valid choice.'
        )

    def test_cannot_edit_node(
        self, app, user, url_draft_registrations, draft_registration
    ):
        ...
        payload = {
            'data': {
                ...
                'type': 'draft_registrations',
                'relationships': {
                    'branched_from': {'data': {'id': node._id, 'type': 'nodes'}}
                },
            }
        }
        res = app.put_json_api(
...
class TestDraftRegistrationPatchNew(TestDraftRegistrationPatch):
    ...
    def test_write_contributor_can_update_draft(
        self, app, user_write_contrib, schema, payload, url_draft_registrations
    ):
        res = app.patch_json_api(
            url_draft_registrations, payload, auth=user_write_contrib.auth
        )
...
class TestDraftPreregChallengeRegistrationMetadataValidationNew(
    TestDraftPreregChallengeRegistrationMetadataValidation
):
    @pytest.fixture()
