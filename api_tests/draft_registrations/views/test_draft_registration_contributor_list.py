...
class DraftRegistrationCRUDTestCase(NodeCRUDTestCase):
    ...
    def project_private(self, user, title, description, category):
        return DraftRegistrationFactory(
            title=title, description=description, category=category, initiator=user
        )


class TestDraftRegistrationContributorList(
    DraftRegistrationCRUDTestCase, TestNodeContributorList
):
    @pytest.fixture()
    ...
    def test_contributors_order_is_the_same_over_multiple_requests(
        ...
        req_one = app.get('{}?page=2'.format(url_public), auth=user.auth)
        req_two = app.get('{}?page=2'.format(url_public), auth=user.auth)
    ...
    def test_permissions_work_with_many_users(
        ...
        users = {permissions.ADMIN: [user._id], permissions.WRITE: []}
        for i in range(0, 25):
...
class TestDraftRegistrationContributorAdd(
    DraftRegistrationCRUDTestCase, TestNodeContributorAdd
):
    @pytest.fixture()
    def url_public(self, project_public):
...
class TestDraftRegistrationContributorCreateValidation(
    DraftRegistrationCRUDTestCase, TestNodeContributorCreateValidation
):
    @pytest.fixture()
...
class TestDraftContributorCreateEmail(
    DraftRegistrationCRUDTestCase, TestNodeContributorCreateEmail
):
    @pytest.fixture()
...
class TestDraftContributorBulkCreate(
    DraftRegistrationCRUDTestCase, TestNodeContributorBulkCreate
):
    @pytest.fixture()
    def url_public(self, project_public):
        return '/{}draft_registrations/{}/contributors/?send_email=false'.format(
...
class TestDraftContributorBulkUpdated(
    DraftRegistrationCRUDTestCase, TestNodeContributorBulkUpdate
):
    @pytest.fixture()
    def project_public(self, user, user_two, user_three, title, description, category):
        project_public = DraftRegistrationFactory(initiator=user)
        project_public.add_contributor(
...
class TestDraftRegistrationContributorBulkPartialUpdate(
    DraftRegistrationCRUDTestCase, TestNodeContributorBulkPartialUpdate
):
    @pytest.fixture()
    def project_public(self, user, user_two, user_three, title, description, category):
        project_public = DraftRegistrationFactory(initiator=user)
...
class TestDraftRegistrationContributorBulkDelete(
    DraftRegistrationCRUDTestCase, TestNodeContributorBulkDelete
):
    @pytest.fixture()
    def url_public(self, project_public):
        return '/{}draft_registrations/{}/contributors/'.format(
...
class TestDraftRegistrationContributorFiltering(
    DraftRegistrationCRUDTestCase, TestNodeContributorFiltering
):
    @pytest.fixture()
