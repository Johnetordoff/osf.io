...
class TestMove:
    ...
    def test_move_checked_out_file_in_folder(
        self, app, root_node, user, folder, folder_two, move_url
    ):
        ...
        assert res.status_code == 400
        assert (
            'Cannot move file as it is checked out.' in res.json['errors'][0]['detail']
        )

    def test_move_checkedout_file_two_deep_in_folder(
        self, app, root_node, user, folder, folder_two, move_url
    ):
    ...
    def test_move_file_out_of_node(
        self, app, user, move_url, root_node, node, node_two, node_two_root_node, folder
    ):
    ...
    def test_can_rename_file_in_quickfiles_node(
        self,
        app,
        node,
        user,
    ...
    def test_blank_destination_file_name(
        self, app, move_url, user, root_node, folder, file
    ):
        signed_payload = sign_payload(
            {
                'source': file._id,
        ...
        signed_payload = sign_payload(
            {
                ...
                'target': root_node._id,
                'user': user._id,
                'destination': {'target': folder.target._id, 'name': 'hello.txt',},
            }
        )
        res = app.post_json(move_url, signed_payload, expect_errors=True)
        assert res.status_code == 400
    ...
    def test_invalid_payload(self, app, move_url):
        signed_payload = {'key': 'incorrectly_formed_payload'}
    ...
    def test_storage_usage_move_within_node(self, app, node, signed_payload, move_url):
        ...
        assert res.status_code == 200
        assert (
            node.storage_usage is None
        )  # this is intentional, the cache shouldn't be touched

    def test_storage_usage_move_between_nodes(
        self, app, node, node_two, file, root_node, user, node_two_root_node, move_url
    ):
        """
        ...
        assert (
            node.storage_usage is None
        )  # the cache starts expired, but there is 1337 bytes in there
...
class TestMovePreprint:
    ...
    def test_move_primary_file_out_of_node(
        self,
        app,
        user,
        move_url,
    ...
    def test_within_preprint_move(
        self, app, user, move_url, file, preprint, folder, root_node
    ):
        preprint.primary_file = file
        preprint.save()
        signed_payload = sign_payload(
    ...
    def test_preprint_in_params_does_not_exist(
        self, app, file, root_node, user, folder
    ):
        move_url = '/_/wb/hooks/{}/move/'.format('12345')
        signed_payload = sign_payload(
            {
...
class TestCopy:
    ...
    def test_copy_checked_out_file_in_folder(
        self, app, root_node, user, folder, folder_two, copy_url
    ):
        file = folder.append_file("No I don't wanna go")
        file.checkout = user
        file.save()
    ...
    def test_copy_checkedout_file_two_deep_in_folder(
        self, app, root_node, user, folder, folder_two, copy_url
    ):
        folder_nested = folder.append_folder('Nested')
        file = folder_nested.append_file("No I don't wanna go")
    ...
    def test_copy_preprint_file_out_of_node(
        self, app, user, copy_url, root_node, node, node_two, node_two_root_node, folder
    ):
        file = folder.append_file("No I don't wanna go")
    ...
    def test_copy_file_out_of_node(
        self, app, user, copy_url, root_node, node, node_two, node_two_root_node, folder
    ):
        node.preprint_file = root_node.append_file('far')
        node.save()
    ...
    def test_within_node_copy_while_preprint(
        self, app, user, copy_url, file, node, folder, root_node
    ):
        node.preprint_file = file
        node.save()
        signed_payload = sign_payload(
    ...
    def test_can_copy_file_out_of_quickfiles_node(
        self,
        app,
        quickfiles_copy_url,
        quickfiles_file,
    ...
    def test_blank_destination_file_name(
        self, app, copy_url, user, root_node, folder, file
    ):
        signed_payload = sign_payload(
            {
                'source': file._id,
    ...
    def test_storage_usage_copy_within_node(
        self, app, node, file, signed_payload, copy_url
    ):
        """
    ...
    def test_storage_usage_copy_between_nodes(
        self, app, node, node_two, file, user, node_two_root_node, copy_url
    ):
        """
        ...
        assert (
            node.storage_usage is None
        )  # The node cache starts expired, but there is 1337 bytes in there
...
class TestCopyPreprint:
    ...
    def test_copy_preprint_file_out_of_preprint(
        self,
        app,
        user,
        copy_url,
    ...
    def test_copy_file_out_of_preprint(
        self,
        app,
        user,
        copy_url,
    ...
    def test_within_preprint_copy_while_preprint(
        self, app, user, copy_url, file, preprint, folder, root_node
    ):
        preprint.primary_file = file
        preprint.save()
        signed_payload = sign_payload(
