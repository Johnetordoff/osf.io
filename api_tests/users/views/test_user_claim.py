...
class TestClaimUser:
    ...
    def test_claim_auth_failure(
        ...
        # no record locator
        payload = self.payload(email='david@david.son')
        res = app.post_json_api(_url, payload, auth=claimer.auth, expect_errors=True)
        assert res.status_code == 400
        assert res.json['errors'][0]['detail'] == 'Must specify record "id".'
        ...
        res = app.post_json_api(_url, payload, auth=referrer.auth, expect_errors=True)
        assert res.status_code == 400
    ...
    def test_claim_auth_success(
        ...
        res = app.post_json_api(
            url.format(unreg_user._id), self.payload(id=project._id), auth=claimer.auth
        )
        assert res.status_code == 204
        assert mock_mail.call_count == 2
