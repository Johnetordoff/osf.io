...
class TestApplicationDetail:
    ...
    def test_update_application(self, app, user, user_app, user_app_url, make_payload):
        ...
        res = app.put_json_api(
            user_app_url, valid_payload, auth=user.auth, expect_errors=True
        )
        assert res.status_code == 200

        #   test_update_application_incorrect_type_payload
        res = app.put_json_api(
            user_app_url, incorrect_type_payload, auth=user.auth, expect_errors=True
        )
        assert res.status_code == 409

        #   test_update_application_incorrect_id_payload
        res = app.put_json_api(
            user_app_url, incorrect_id_payload, auth=user.auth, expect_errors=True
        )
        assert res.status_code == 409

        #   test_update_application_no_type
        res = app.put_json_api(
            user_app_url, missing_type_payload, auth=user.auth, expect_errors=True
        )
        assert res.status_code == 400

        #   test_update_application_no_id
        res = app.put_json_api(
            user_app_url, missing_id_payload, auth=user.auth, expect_errors=True
        )
        assert res.status_code == 400
        ...
        res = app.put_json_api(
            user_app_url, payload, auth=user.auth, expect_errors=True
        )
        assert res.status_code == 400
