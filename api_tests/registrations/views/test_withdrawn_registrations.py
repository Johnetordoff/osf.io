...
class TestWithdrawnRegistrations(NodeCRUDTestCase):
    ...
    def test_withdrawn_registrations_display_limited_relationship_fields(
        ...
        url_withdrawn = '/{}registrations/{}/?version=2.14'.format(
            API_BASE, registration._id
        )
        res = app.get(url_withdrawn, auth=user.auth)
    ...
    def test_child_inherits_withdrawl_justication_and_date_withdrawn(
        ...
        assert res.status_code == 200
        assert (
            res.json['data']['attributes']['withdrawal_justification']
            == withdrawn_registration_with_child.justification
        )
        formatted_date_retracted = withdrawn_registration_with_child.date_retracted.isoformat().replace(
            '+00:00', 'Z'
        )
        assert (
            res.json['data']['attributes']['date_withdrawn'] == formatted_date_retracted
        )
