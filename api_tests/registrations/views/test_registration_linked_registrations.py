...
class LinkedRegistrationsTestCase(ApiTestCase):
    def setUp(self):
        ...
        public_node.add_contributor(
            self.read_contributor, permissions=READ, auth=Auth(self.admin_contributor)
        )
        public_node.add_pointer(
            self.public_linked_registration, auth=Auth(self.admin_contributor)
        )
        public_node.add_pointer(
            self.private_linked_registration, auth=Auth(self.rw_contributor)
        )
        public_node.save()
        draft_reg = DraftRegistrationFactory(
            branched_from=public_node, initiator=self.admin_contributor
        )
        ...
        private_node.save()
        draft_reg = DraftRegistrationFactory(
            branched_from=private_node, initiator=self.admin_contributor
        )
...
class TestRegistrationLinkedRegistrationsList(LinkedRegistrationsTestCase):
    ...
    def test_unauthenticated_can_view_public_registration_linked_registrations(self):
        ...
        assert_not_in(self.private_linked_registration._id, linked_registration_ids)
    ...
    def test_rw_contributor_can_view_private_registration_linked_registrations(self):
        res = self.make_request(
            registration_id=self.private_registration._id, auth=self.rw_contributor.auth
        )
        assert_equal(res.status_code, 200)
        linked_registration_ids = [r['id'] for r in res.json['data']]
        assert_in(self.public_linked_registration._id, linked_registration_ids)
        assert_in(self.private_linked_registration._id, linked_registration_ids)
    ...
    def test_unauthenticated_cannot_view_private_registration_linked_registrations(
        ...
        res = self.make_request(
            registration_id=self.private_registration._id, expect_errors=True
        )
...
class TestRegistrationsLinkedRegistrationsRelationship(LinkedRegistrationsTestCase):
    def setUp(self):
    ...
    def test_public_registration_unauthenticated_user_can_view_linked_registrations_relationship_2_13(
        ...
        res = self.make_request(
            registration_id=self.public_registration._id, version='2.13'
        )
        assert_equal(res.status_code, 200)
        linked_registration_ids = [r['id'] for r in res.json['data']]
        assert_in(self.public_linked_registration._id, linked_registration_ids)
    ...
    def test_cannot_create_linked_registrations_relationship(self):
        res = self.app.post_json_api(
            self.public_url, {}, auth=self.admin_contributor.auth, expect_errors=True
        )
