...
from osf_tests.factories import NodeFactory, AuthUserFactory, RegistrationFactory
...
class TestNodeRelationshipNodeLinks:
    ...
    def public_linking_node_source(self, contributor, private_node, public_node):
        public_linking_node_source = NodeFactory(is_public=True, creator=contributor)
        public_linking_node_source.add_pointer(private_node, auth=Auth(contributor))
        public_linking_node_source.add_pointer(public_node, auth=Auth(contributor))
    ...
    def payload(self, admin_node):
        def payload(node_ids=None):
            ...
            return {
                'data': [
                    {'type': 'linked_nodes', 'id': node_id} for node_id in node_ids
                ]
            }
    ...
    def test_node_relationship_node_links(
        ...
        assert (
            res.json['data']['relationships']['linked_nodes']['links']['related'][
                'meta'
            ]['count']
            == 2
        )
        ...
        res = app.post_json_api(
            url, payload([contributor_node._id]), auth=user.auth, expect_errors=True
        )
        ...
        #   post_public_node
        res = app.post_json_api(
            url, payload([public_node._id]), auth=user.auth, expect_errors=True
        )
        ...
        #   post_private_node
        res = app.post_json_api(
            url, payload([other_node._id]), auth=user.auth, expect_errors=True
        )
        ...
        res = app.post_json_api(
            url,
            payload([other_node._id, contributor_node._id]),
            auth=user.auth,
            expect_errors=True,
        )
        ...
        res = app.post_json_api(
            url, payload([private_node._id]), auth=user.auth, expect_errors=True
        )
        ...
        res = app.put_json_api(url, new_payload, auth=user.auth, expect_errors=True)
        assert res.status_code == 405
        ...
        res = app.delete_json_api(
            url,
            payload([private_node._id, admin_node._id]),
            auth=user.auth,
            expect_errors=True,
        )
        assert res.status_code == 405
        ...
        #   node_doesnt_exist
        res = app.post_json_api(
            url, payload(['aquarela']), auth=user.auth, expect_errors=True
        )
        ...
        res = app.post_json_api(
            url,
            {'data': [{'type': 'not_linked_nodes', 'id': contributor_node._id}]},
            auth=user.auth,
            expect_errors=True,
        )
        ...
        res = app.post_json_api(
            public_url, payload([public_node._id]), expect_errors=True
        )
        ...
        res = app.post_json_api(
            public_url, payload([public_node._id]), auth=user.auth, expect_errors=True
        )

        assert res.status_code == 405

        #   creates_private_linked_node_relationship_logged_out
        res = app.post_json_api(url, payload([other_node._id]), expect_errors=True)
        ...
        res = app.put_json_api(
            public_url, payload([private_node._id]), auth=user.auth, expect_errors=True
        )
...
class TestNodeLinkedNodes:
    ...
    @pytest.fixture()
    def url(self, linking_node):
        return '/{}registrations/{}/linked_nodes/'.format(API_BASE, linking_node._id)

    @pytest.fixture()
    def node_ids(self, linking_node):
        return list(linking_node.nodes_pointer.values_list('guids___id', flat=True))
    ...
    def test_linked_nodes_only_return_viewable_nodes(
        self, app, auth, private_node_one, private_node_two, public_node, node_ids
    ):
        ...
        res = app.get(
            '/{}registrations/{}/linked_nodes/'.format(
    ...
    def test_linked_nodes_doesnt_return_deleted_nodes(
        self, app, user, url, private_node_one, private_node_two, public_node, node_ids
    ):
