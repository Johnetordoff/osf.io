...
class TestRegistrationForksList:
    ...
    def private_registration_url(self, private_registration):
        return '/{}registrations/{}/forks/'.format(API_BASE, private_registration._id)

    @pytest.fixture()
    def public_registration_url(self, public_registration):
        return '/{}registrations/{}/forks/'.format(API_BASE, public_registration._id)
    ...
    def test_can_access_public_registration_forks_list_when_unauthenticated(
        ...
        assert data['attributes']['title'] == 'Fork of ' + public_registration.title
    ...
    def test_authentication(
        ...
        fork_contributors = data['embeds']['contributors']['data'][0]['embeds'][
            'users'
        ]['data']
        ...
        assert (
            forked_children['id']
            == private_registration.forks.first().get_nodes(is_node_link=False)[0]._id
        )
        ...
        expected_logs = list(private_registration.logs.values_list('action', flat=True))
        ...
        res = app.get(
            private_registration_url, auth=non_contributor.auth, expect_errors=True
        )
...
class TestRegistrationForkCreate:
    ...
    def fork_data_with_title(self):
        return {'data': {'type': 'nodes', 'attributes': {'title': 'My Forked Project'}}}
    ...
    def test_create_fork_from_public_registration_with_new_title(
        ...
        res = app.post_json_api(
            public_registration_url, fork_data_with_title, auth=user.auth
        )
        ...
        assert (
            ...
            == fork_data_with_title['data']['attributes']['title']
        )
        assert not data['attributes']['registration']
        assert data['attributes']['fork'] is True
    ...
    def test_create_fork_from_private_registration_with_new_title(
        ...
        res = app.post_json_api(
            private_registration_url, fork_data_with_title, auth=user.auth
        )
        assert res.status_code == 201
        data = res.json['data']
    ...
    def test_can_fork_public_registration_logged_in(
        self, app, user_two, public_registration, public_registration_url, fork_data
    ):
        res = app.post_json_api(public_registration_url, fork_data, auth=user_two.auth)
        assert res.status_code == 201
        data = res.json['data']
        assert data['id'] == public_registration.forks.first()._id
        assert data['attributes']['title'] == 'Fork of ' + public_registration.title
    ...
    def test_cannot_fork_private_registration_logged_in_non_contributor(
        ...
        res = app.post_json_api(
            private_registration_url, fork_data, auth=user_two.auth, expect_errors=True
        )
        assert res.status_code == 403
        assert (
            res.json['errors'][0]['detail']
    ...
    def test_can_fork_private_registration_logged_in_contributor(
        ...
        data = res.json['data']
        assert data['attributes']['title'] == 'Fork of ' + private_registration.title
        assert not data['attributes']['registration']
        assert data['attributes']['fork'] is True
    ...
    def test_fork_private_components_no_access(
        ...
        NodeFactory(parent=public_registration, creator=user_two, is_public=False)
    ...
    def test_fork_components_you_can_access(
        ...
        assert (
            res.json['data']['embeds']['children']['data'][0]['id']
            == new_component.forks.first()._id
        )
    ...
    def test_fork_node_links_you_can_access(
        ...
        res = app.post_json_api(url, fork_data, auth=user.auth)
        assert (
            res.json['data']['embeds']['node_links']['data'][1]['embeds'][
                'target_node'
            ]['data']['id']
            == pointer._id
        )
