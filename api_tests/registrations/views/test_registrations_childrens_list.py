...
def registration_with_children_url(registration_with_children):
    return '/{}registrations/{}/children/'.format(
        API_BASE, registration_with_children._id,
    )
...
def registration_with_children_approved(user, registration_with_children):
    ...
    approval_token = registration_with_children.registration_approval.approval_state[
        user._id
    ]['approval_token']
...
def registration_with_children_approved_url(registration_with_children_approved):
    return '/{}registrations/{}/children/'.format(
        API_BASE, registration_with_children_approved._id,
    )


@pytest.mark.django_db
class TestRegistrationsChildrenList:
    def test_registrations_children_list(
        self, user, app, registration_with_children, registration_with_children_url
    ):
        (
            component_one,
            component_two,
            component_three,
            component_four,
        ) = registration_with_children.nodes
    ...
    def test_return_registrations_list_no_auth_approved(
        ...
        (
            ...
            component_three,
            component_four,
        ) = registration_with_children_approved.nodes
    ...
    def test_registrations_list_no_auth_unapproved(
        self, user, app, registration_with_children, registration_with_children_url
    ):
    ...
    def test_registration_children_no_auth_vol(
        ...
        node_url = '/{}registrations/{}/?related_counts=children&view_only={}'.format(
            API_BASE, registration_with_children._id, view_only_link.key
        )
        ...
        view_only_link_url = '{}?view_only={}'.format(
            registration_with_children_url, view_only_link.key
        )
...
class TestRegistrationChildrenListFiltering:
    def test_registration_child_filtering(self, app, user, registration_with_children):
        ...
        url = '/{}registrations/{}/children/?filter[title]={}'.format(
            API_BASE, registration_with_children._id, component_one.title
        )
