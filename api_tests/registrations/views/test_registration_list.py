...
class TestRegistrationList(ApiTestCase):
    ...
    def test_return_public_registrations_logged_out(self):
        ...
        assert_equal(
            urlparse(url).path, '/{}nodes/{}/'.format(API_BASE, self.public_project._id)
        )
    ...
    def test_return_registrations_logged_in_contributor(self):
        ...
        registered_from_one = urlparse(
            res.json['data'][0]['relationships']['registered_from']['links']['related'][
                'href'
            ]
        ).path
        registered_from_two = urlparse(
            res.json['data'][1]['relationships']['registered_from']['links']['related'][
                'href'
            ]
        ).path
    ...
    def test_return_registrations_logged_in_non_contributor(self):
        ...
        assert_equal(
            registered_from, '/{}nodes/{}/'.format(API_BASE, self.public_project._id)
        )
...
class TestRegistrationFiltering(ApiTestCase):
    def setUp(self):
        ...
        self.project_three = ProjectFactory(
            title='Three', description='', is_public=True, creator=self.user_two
        )
    ...
    def test_filtering_tags_exact(self):
        ...
        res = self.app.get(
            '/{}registrations/?filter[tags]=cat'.format(API_BASE),
    ...
    def test_filtering_on_multiple_tags(self):
        ...
        res = self.app.get(
            '/{}registrations/?filter[tags]=cat&filter[tags]=sand'.format(API_BASE),
            auth=self.user_one.auth,
        )
        assert_equal(len(res.json.get('data')), 1)
...
class TestNodeRegistrationCreate(DraftRegistrationTestCase):
    ...
    def url_registrations_ver(self, project_public, url_registrations):
        return '{}?version={}'.format(
            url_registrations, CREATE_REGISTRATION_FIELD_CHANGE_VERSION
        )
    ...
    @pytest.fixture()
    def payload_with_children(
        self, draft_registration, project_public_child, project_public_grandchild
    ):
        return {
            'data': {
                'type': 'registrations',
                'attributes': {
                    'draft_registration': draft_registration._id,
                    'children': [
                        project_public_child._id,
                        project_public_grandchild._id,
                    ],
                    'registration_choice': 'immediate',
                },
            }
    ...
    @pytest.fixture()
    def payload_with_grandchildren_but_no_children(
        self, draft_registration, project_public_child, project_public_grandchild
    ):
        return {
            'data': {
    ...
    def test_draft_registration_retains_editable_fields_after_reg(
        ...
        assert (
            attributes['node_license']['copyright_holders']
            == node_license_rec.copyright_holders
        )
    ...
    def test_admin_can_create_registration_with_specific_children(
        ...
        project_public,
        project_public_child,
        project_public_excluded_sibling,
        project_public_grandchild,
        url_registrations,
    ):
        res = app.post_json_api(
            url_registrations, payload_with_children, auth=user.auth
        )
        data = res.json['data']['attributes']
        assert res.status_code == 201
        assert data['registration'] is True
    ...
    def test_admin_400_with_bad_child_node_guid(
        ...
        app,
        user,
        payload_with_bad_child_node_guid,
        url_registrations,
    ):
        res = app.post_json_api(
            url_registrations,
    ...
    def test_admin_cant_register_grandchildren_without_children(
        ...
        app,
        user,
        payload_with_grandchildren_but_no_children,
        url_registrations,
        project_public_grandchild,
    ):
        res = app.post_json_api(
    ...
    def test_old_workflow_node_editable_metadata_copied(
        ...
        project_public.save()
        res = app.post_json_api(
            url_registrations, payload, auth=user.auth, expect_errors=True
        )
        assert res.status_code == 201
    ...
    def test_cannot_create_registration(
        ...
        res = app.post_json_api(
            url_registrations, payload, auth=user_write_contrib.auth, expect_errors=True
        )
        assert res.status_code == 403

        # def test_read_only_contributor_cannot_create_registration(self):
        res = app.post_json_api(
            url_registrations, payload, auth=user_read_contrib.auth, expect_errors=True
        )
        assert res.status_code == 403
        ...
        res = app.post_json_api(
            url_registrations, payload, auth=group_mem.auth, expect_errors=True
        )
        assert res.status_code == 403
    ...
    def test_registration_draft_must_be_specified(
        ...
        payload = {
            'data': {
                'type': 'registrations',
                'attributes': {'registration_choice': 'immediate'},
            }
        }
        res = app.post_json_api(
            url_registrations, payload, auth=user.auth, expect_errors=True
        ...
        assert (
            ...
            in res.json['errors'][0]['source']['pointer']
        )
    ...
    def test_required_top_level_questions_must_be_answered_on_draft(
        ...
        schema = RegistrationSchema.objects.get(
            name='Test Schema', schema=test_schema, schema_version=SCHEMA_VERSION
        )

        draft_registration = DraftRegistrationFactory(
            initiator=user, registration_schema=schema, branched_from=project_public
        )
        ...
        assert (
            res.json['errors'][0]['detail']
            == "For your registration the 'Title' field is required"
        )
    ...
    def test_required_second_level_questions_must_be_answered_on_draft(
        ...
        assert (
            res.json['errors'][0]['detail']
            == "For your registration your response to the 'Manipulated variables' field is invalid."
        )
    ...
    def test_multiple_choice_in_registration_schema_must_match_one_of_choices(
        ...
        assert (
            res.json['errors'][0]['detail']
            == "For your registration your response to the 'I judge the replication to be a(n)'"
    ...
    def test_invalid_registration_choice(
        ...
        assert res.status_code == 400
        assert (
            res.json['errors'][0]['source']['pointer']
    ...
    def test_embargo_must_be_less_than_four_years(
        ...
        today = timezone.now()
        five_years = (today + dateutil.relativedelta.relativedelta(years=5)).strftime(
            '%Y-%m-%dT%H:%M:%S'
        )
        payload = {
            'data': {
    ...
    def test_embargo_registration(
        ...
        today = timezone.now()
        next_week = (today + dateutil.relativedelta.relativedelta(months=1)).strftime(
            '%Y-%m-%dT%H:%M:%S'
        )
        payload = {
            'data': {
    ...
    def test_new_API_version_requires_draft_registration_id_on_creation(
        ...
        payload = {
            'data': {
                'type': 'registrations',
                'attributes': {'draft_registration': draft_registration._id,},
            }
        }
        res = app.post_json_api(
            url_registrations_ver, payload, auth=user.auth, expect_errors=True
        )
        assert res.status_code == 400
        assert (
            res.json['errors'][0]['source']['pointer']
            == '/data/attributes/draft_registration_id'
        )
        assert res.json['errors'][0]['detail'] == 'This field is required.'
    ...
    def test_new_API_version_no_embargo_is_immediate_by_default(
        ...
        payload = {
            'data': {
                'type': 'registrations',
                'attributes': {'draft_registration_id': draft_registration._id,},
            }
        }
        res = app.post_json_api(
            url_registrations_ver, payload, auth=user.auth, expect_errors=True
    ...
    def test_new_API_version_uses_embargo_end_date(
        ...
        today = timezone.now()
        three_years = (today + dateutil.relativedelta.relativedelta(years=3)).strftime(
            '%Y-%m-%dT%H:%M:%S'
        )
    ...
    def test_new_API_version_uses_included_node_ids_instead_of_children(
        ...
        res = app.post_json_api(
            url_registrations_ver, payload_with_children, auth=user.auth
        )
        data = res.json['data']['attributes']
        assert res.status_code == 201
        assert data['registration'] is True
    ...
    def test_cannot_register_draft_that_is_pending_review(
        ...
        with mock.patch.object(
            DraftRegistration, 'is_pending_review', mock.PropertyMock(return_value=True)
        ):
            res = app.post_json_api(
                url_registrations, payload, auth=user.auth, expect_errors=True
            )
    ...
    def test_cannot_register_draft_that_has_already_been_approved(
        ...
        with mock.patch.object(
            DraftRegistration, 'requires_approval', mock.PropertyMock(return_value=True)
        ), mock.patch.object(
            DraftRegistration, 'is_approved', mock.PropertyMock(return_value=True)
        ):
            res = app.post_json_api(
                url_registrations, payload, auth=user.auth, expect_errors=True
            )
    ...
    def test_cannot_register_draft_that_has_orphan_files(
        ...
        draft_registration.registration_metadata = {
            'qwhatever': {
                'value': 'file 1',
                'extra': [{'nodeId': 'badid', 'selectedFileName': 'file 1',}],
            }
        }
        draft_registration.save()
        res = app.post_json_api(
        ...
        assert (
            res.json['errors'][0]['detail']
            == 'All files attached to this form must be registered to complete the'
    ...
    def test_assert_file_validity_on_registration_metadata(
        ...
        prereg_schema = RegistrationSchema.objects.get(
            name='Prereg Challenge', schema_version=SCHEMA_VERSION
        )
        ...
        prereg_draft_registration.update_registration_responses(
            prereg_registration_responses
        )
        ...
        file_view_url = urljoin(
            settings.DOMAIN,
            '/project/{}/files/osfstorage/{}'.format(
                prereg_draft_registration.branched_from._id, file._id,
            ),
        )
        prereg_registration_responses['q7.uploader'] = [
            {
                'file_name': file_name,
                'file_id': file._id,
                'file_hashes': {'sha256': 'incorrect_sha',},
                'file_urls': {'html': file_view_url,},
            }
        ]
...
class TestRegistrationCreate(TestNodeRegistrationCreate):
    ...
    def url_registrations_ver(self, project_public):
        return '/{}registrations/?version={}'.format(
            API_BASE, CREATE_REGISTRATION_FIELD_CHANGE_VERSION
        )
    ...
    def test_need_admin_perms_on_node_and_draft(
        ...
        payload_ver['data']['attributes'][
            'draft_registration_id'
        ...
        res = app.post_json_api(
            url_registrations_ver, payload_ver, auth=user.auth, expect_errors=True
        )
        assert res.status_code == 403
        assert (
            res.json['errors'][0]['detail']
        ...
        assert draft_registration.branched_from.is_admin_contributor(user) is False
        assert (
            draft_registration.branched_from.has_permission(user, permissions.ADMIN)
            is True
        )
        assert draft_registration.has_permission(user, permissions.ADMIN) is True
        res = app.post_json_api(
...
class TestRegistrationBulkUpdate:
    ...
    def empty_payload(self, registration_one, registration_two):
        return {
            'data': [
                {'id': registration_one._id, 'type': 'registrations', 'attributes': {}},
                {'id': registration_two._id, 'type': 'registrations', 'attributes': {}},
            ]
        }
        ...
        return {
            'data': [
                ...
                },
                {
                    'id': registration_two._id,
                    'type': 'registrations',
                    'attributes': {'title': 'Nerds II: Attack of the Nerds',},
                },
            ]
        }
    ...
    def test_bulk_update_errors(
        ...
        res = app.put_json_api(
            url,
            {
                'data': [
                    ...
                    {'type': 'registrations', 'attributes': {'public': True}},
                ]
            },
            auth=user.auth,
            expect_errors=True,
        ...
        res = app.put_json_api(
            url,
            {
                'data': [
                    public_payload['data'][1],
                    {'id': registration_one._id, 'attributes': {'public': True}},
                ]
            },
            auth=user.auth,
            expect_errors=True,
        ...
        res = app.put_json_api(
            url, registration_update_list, auth=user.auth, expect_errors=True, bulk=True
        )
        assert (
        ...
        res = app.put_json_api(
            url, private_payload, auth=user.auth, bulk=True, expect_errors=True
        )
        assert res.status_code == 400
        assert (
            res.json['errors'][0]['detail']
    ...
    def test_bulk_update_embargo_logged_in_read_only_contrib(
        ...
        res = app.put_json_api(
            url, public_payload, auth=read_contrib.auth, expect_errors=True, bulk=True
        )
        assert res.status_code == 403
        assert (
            res.json['errors'][0]['detail']
    ...
    def test_bulk_update_embargo_logged_in_contrib(
        ...
        assert (
            res.json['data'][0]['attributes']['pending_embargo_termination_approval']
            is True
        )
        assert (
            res.json['data'][1]['attributes']['pending_embargo_termination_approval']
            is True
        )
