...
class TestRegistrationIdentifierList:
    ...
    def test_identifier_list_returns_correct_number_and_referent(
        ...
        paths = [
            urlparse(item['relationships']['referent']['links']['related']['href']).path
    ...
    def test_identifier_filter_by_category(
        self, app, registration, identifier_registration, url_registration_identifiers
    ):
...
class TestNodeIdentifierList:
    ...
    def test_registration_not_allowed_from_nodes_endpoint(self, app, registration):
...
class TestPreprintIdentifierList:
    ...
    def test_identifier_list_returns_correct_categories_and_values(
        ...
        values_in_response = [
            identifier['attributes']['value'] for identifier in data_preprint_identifier
        ]
...
class TestNodeIdentifierCreate:
    ...
    def identifier_url(self, resource):
        return '/{}{}s/{}/identifiers/'.format(
            API_BASE, resource.__class__.__name__.lower(), resource._id
        )

    @pytest.fixture()
    def identifier_payload(self):
        return {'data': {'type': 'identifiers', 'attributes': {'category': 'doi'}}}

    @pytest.fixture()
    def ark_payload(self):
        return {'data': {'type': 'identifiers', 'attributes': {'category': 'ark'}}}
    ...
    def test_create_identifier(
        ...
        res = app.post_json_api(
            identifier_url, identifier_payload, auth=user.auth, expect_errors=True
        )
        ...
        assert res.json['data']['attributes']['value'] == resource.get_identifier_value(
            'doi'
        )
        ...
        assert res.status_code == 400
        assert (
            res.json['errors'][0]['detail'] == 'A DOI already exists for this resource.'
        )
