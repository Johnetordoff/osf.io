...
def make_registration_payload(*node_ids):
    data = [{'type': 'registrations', 'id': id_} for id_ in node_ids]
    return {'data': data}
...
class TestInstitutionRelationshipNodes:
    ...
    def test_auth_get_nodes(
        self, app, user, node_one, node_public, node_private, url_institution_nodes
    ):
    ...
    def test_user_with_nodes_and_permissions(
        self, user, app, node_two, url_institution_nodes, institution
    ):
        res = app.post_json_api(
            url_institution_nodes, make_payload(node_two._id), auth=user.auth
        )
    ...
    def test_user_does_not_have_node(
        self, app, node, url_institution_nodes, user, institution
    ):
        res = app.post_json_api(
            url_institution_nodes,
    ...
    def test_user_is_admin(
        self, app, node_two, url_institution_nodes, user, institution
    ):
        res = app.post_json_api(
            url_institution_nodes, make_payload(node_two._id), auth=user.auth
        )
    ...
    def test_user_is_read_write(self, app, node, url_institution_nodes, institution):
        ...
        node.save()
        res = app.post_json_api(
            url_institution_nodes, make_payload(node._id), auth=user.auth
        )
    ...
    def test_add_some_existant_others_not(
        self, institution, node_one, user, node_two, app, url_institution_nodes
    ):
    ...
    def test_only_add_existent_with_mixed_permissions(
        self, institution, node_one, node_public, app, url_institution_nodes, user
    ):
        assert institution in node_one.affiliated_institutions.all()
    ...
    def test_only_add_existent_with_permissions(
        self, user, node_one, node_two, institution, app, url_institution_nodes
    ):
    ...
    def test_delete_user_is_admin(
        self, app, url_institution_nodes, node_one, user, institution
    ):
        res = app.delete_json_api(
            url_institution_nodes, make_payload(node_one._id), auth=user.auth
        )
        node_one.reload()
        assert res.status_code == 204
    ...
    def test_delete_user_is_read_write(
        self, app, node_private, user, url_institution_nodes, institution
    ):
        node_private.add_contributor(user)
        node_private.save()

        res = app.delete_json_api(
            url_institution_nodes, make_payload(node_private._id), auth=user.auth
        )
        node_private.reload()
    ...
    def test_delete_user_is_read_only(
        self, node_private, user, app, url_institution_nodes, institution
    ):
...
class TestInstitutionRelationshipRegistrations:
    ...
    def test_add_incorrect_permissions(
        ...
        assert (
            institution not in registration_no_affiliation.affiliated_institutions.all()
        )
    ...
    def test_add_some_with_permissions_others_without(
        ...
        res = app.post_json_api(
            ...
            make_registration_payload(
                registration_no_owner._id, registration_no_affiliation._id
            ),
            expect_errors=True,
            auth=admin.auth,
        )
