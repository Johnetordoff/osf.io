...
class TestDeprecatedMetaSchemaDetail:
    def test_deprecated_metaschemas_routes(self, app, user, schema):
        ...
        url = '/{}metaschemas/registrations/{}/?version=2.8'.format(
            API_BASE, schema._id
        )
        res = app.get(url, auth=user.auth)
        assert res.status_code == 200
        ...
        url = '/{}metaschemas/registrations/{}/?version=2.9'.format(
            API_BASE, schema._id
        )
        res = app.get(url, auth=user.auth, expect_errors=True)
        assert res.status_code == 404
        assert (
            res.json['errors'][0]['detail']
            == 'This route has been deprecated. It was last available in version 2.8'
        )
...
class TestRegistrationSchemaDetail:
    ...
    def test_registration_schema_schema_blocks(self, app, user, schema):
        ...
        url = '/{}schemas/registrations/{}/schema_blocks/{}/'.format(
            API_BASE, schema._id, schema_block_id
        )
        res = app.get(url, auth=user.auth)
        assert res.status_code == 200
