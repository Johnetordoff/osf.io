...
class TestPreprintProviderModeratorDetail:
    ...
    def test_detail_not_authorized(
        ...
        res = app.patch_json_api(
            url.format(moderator._id),
            self.update_payload(
                user_id=moderator._id, permission_group=permissions.ADMIN
            ),
    ...
    def test_detail_cannot_remove_last_admin(self, app, url, admin, provider):
        ...
        res = app.delete_json_api(
            url.format(admin._id), auth=admin.auth, expect_errors=True
        )
        assert res.status_code == 400
        assert 'last admin' in res.json['errors'][0]['detail']
    ...
    def test_moderator_deletes(self, app, url, moderator, admin, provider):
        res = app.delete_json_api(
            url.format(admin._id), auth=moderator.auth, expect_errors=True
        )
        assert res.status_code == 403
    ...
    def test_admin_delete_admin(self, app, url, moderator, admin, provider):
        ...
        assert (
            res.json['data']['attributes']['permission_group'] == permissions.ADMIN
        )  # Sanity check
