...
class TestPreprintProviderSubjects(ProviderSubjectsMixin):
    ...
    def test_no_rules_with_grandparent_filter(self, app, lawless_url, subA):
        res = app.get(lawless_url + 'filter[parent]={}'.format(subA._id))

        assert res.status_code == 200
        assert res.json['links']['meta']['total'] == 3

    def test_rules_enforced_with_grandparent_filter(self, app, ruled_url, subA):
        res = app.get(ruled_url + 'filter[parent]={}'.format(subA._id))
...
class TestPreprintProviderSpecificSubjectsForDeprecatedEndpoint(
    ProviderSpecificSubjectsMixin
):
    provider_class = PreprintProviderFactory
...
class TestPreprintProviderSpecificSubjects(ProviderSpecificSubjectsMixin):
    ...
    @pytest.fixture()
    def url_1(self, provider_1):
        return '/{}providers/preprints/{}/subjects/?page[size]=15&'.format(
            API_BASE, provider_1._id
        )
...
class TestPreprintProviderHighlightedTaxonomies:
    ...
    def url_deprecated(self, provider):
        return '/{}preprint_providers/{}/taxonomies/highlighted/'.format(
            API_BASE, provider._id
        )

    @pytest.fixture()
    def url(self, provider):
        return '/{}providers/preprints/{}/taxonomies/highlighted/'.format(
            API_BASE, provider._id
        )

    def test_taxonomy_other_ordering(
        self, app, url, provider, subj_a, subj_aa, other_subj, z_subj
    ):
        res = app.get(url)
    ...
    def test_mapped_subjects_filter_wrong_provider(
        self, app, url_deprecated, url, subj_aa
    ):
...
class TestPreprintProviderHighlightedSubjects(
    TestPreprintProviderHighlightedTaxonomies
):
    @pytest.fixture()
    def url(self, provider):
        return '/{}providers/preprints/{}/subjects/highlighted/'.format(
            API_BASE, provider._id
        )
...
class TestCustomTaxonomy:
    ...
    def a_subj(self, bepress_subj, asdf_provider):
        return SubjectFactory(
            text='Aaa Text', bepress_subject=bepress_subj, provider=asdf_provider
        )
    ...
    def z_subj(self, bepress_subj, asdf_provider):
        return SubjectFactory(
            text='Zzz Text', bepress_subject=bepress_subj, provider=asdf_provider
        )
    ...
    def test_taxonomy_other_ordering(
        self, app, url, asdf_provider, a_subj, other_subj, z_subj
    ):
    ...
    def test_taxonomy_share_title(
        ...
        bepress_res = app.get(url_deprecated.format(API_BASE, osf_provider._id))
        asdf_res = app.get(url_deprecated.format(API_BASE, asdf_provider._id))
