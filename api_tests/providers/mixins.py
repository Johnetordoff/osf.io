...
class ProviderExistsMixin(ProviderMixinBase):
    ...
    def test_provider_exists(
        self, app, provider_url, fake_url, provider_list_url, provider_list_url_fake
    ):
        ...
        res = app.get(provider_list_url_fake, expect_errors=True)
    ...
    def test_has_highlighted_subjects_flag(
        self, app, provider, provider_two, provider_url, provider_url_two
    ):
        SubjectFactory(provider=provider, text='A', highlighted=True)
        ...
        assert (
            res_subjects['links']['related']['meta']['has_highlighted_subjects'] is True
        )
        ...
        res_subjects = res.json['data']['relationships']['highlighted_taxonomies']
        assert (
            res_subjects['links']['related']['meta']['has_highlighted_subjects']
            is False
        )
...
class ProviderSubjectsMixin(ProviderMixinBase):
    ...
    def test_no_rules_with_parents_filter(self, app, lawless_url, subB, subI, subM):
        res = app.get(lawless_url + 'filter[parents]={}'.format(subB._id))

        assert res.status_code == 200
        assert res.json['links']['meta']['total'] == 1
        assert res.json['data'][0]['attributes']['text'] == 'F'

        res = app.get(lawless_url + 'filter[parents]={}'.format(subI._id))

        assert res.status_code == 200
        assert res.json['links']['meta']['total'] == 2

        res = app.get(lawless_url + 'filter[parents]={}'.format(subM._id))

        assert res.status_code == 200
        assert res.json['links']['meta']['total'] == 2

    def test_rules_enforced_with_parents_filter(self, app, ruled_url, subB, subI, subM):
        res = app.get(ruled_url + 'filter[parents]={}'.format(subB._id))
        ...
        res = app.get(ruled_url + 'filter[parents]={}'.format(subI._id))
        ...
        res = app.get(ruled_url + 'filter[parents]={}'.format(subM._id))

    def test_no_rules_with_parent_filter(self, app, lawless_url, subB, subI, subM):
        res = app.get(lawless_url + 'filter[parent]={}'.format(subB._id))

        assert res.status_code == 200
        assert res.json['links']['meta']['total'] == 1
        assert res.json['data'][0]['attributes']['text'] == 'F'

        res = app.get(lawless_url + 'filter[parent]={}'.format(subI._id))

        assert res.status_code == 200
        assert res.json['links']['meta']['total'] == 2

        res = app.get(lawless_url + 'filter[parent]={}'.format(subM._id))

        assert res.status_code == 200
        assert res.json['links']['meta']['total'] == 2

    def test_rules_enforced_with_parent_filter(self, app, ruled_url, subB, subI, subM):
        res = app.get(ruled_url + 'filter[parent]={}'.format(subB._id))
        ...
        res = app.get(ruled_url + 'filter[parent]={}'.format(subI._id))
        ...
        res = app.get(ruled_url + 'filter[parent]={}'.format(subM._id))
    ...
    def test_no_rules_with_grandparent_filter(self, app, lawless_url, subA):
        res = app.get(lawless_url + 'filter[parents]={}'.format(subA._id))

        assert res.status_code == 200
        assert res.json['links']['meta']['total'] == 3

    def test_rules_enforced_with_grandparent_filter(self, app, ruled_url, subA):
        res = app.get(ruled_url + 'filter[parents]={}'.format(subA._id))
...
class ProviderSpecificSubjectsMixin(ProviderMixinBase):
    ...
    def test_mapped_subjects_are_not_shared_list(self, app, url_1, url_2):
        ...
        assert (
            len(
                set([d['attributes']['text'] for d in res_1.json['data']])
    ...
    def test_mapped_subjects_are_not_shared_filter(
        self, app, url_1, url_2, root_subject_1, root_subject_2
    ):
        res_1 = app.get(url_1 + 'filter[parent]={}'.format(root_subject_1._id))
        res_2 = app.get(url_2 + 'filter[parent]={}'.format(root_subject_2._id))
    ...
    def test_mapped_subjects_filter_wrong_provider(
        self, app, url_1, url_2, root_subject_1, root_subject_2
    ):
        res_1 = app.get(url_1 + 'filter[parent]={}'.format(root_subject_2))
        res_2 = app.get(url_2 + 'filter[parent]={}'.format(root_subject_1))
...
class ProviderCustomTaxonomyMixin(ProviderMixinBase):
    ...
    def other_subj(self, bepress_subj, asdf_provider):
        return SubjectFactory(
            text='Other Text', bepress_subject=bepress_subj, provider=asdf_provider
        )
    ...
    def test_taxonomy_share_title(
        self, app, url, osf_provider, asdf_provider, bepress_subj, other_subj
    ):
        bepress_res = app.get(url.format(API_BASE, osf_provider._id))
        asdf_res = app.get(url.format(API_BASE, asdf_provider._id))

        assert len(bepress_res.json['data']) == len(asdf_res.json['data']) == 1
        assert (
            bepress_res.json['data'][0]['attributes']['share_title']
            == osf_provider.share_title
        )
        assert (
            asdf_res.json['data'][0]['attributes']['share_title']
            == asdf_provider.share_title
        )
...
class ProviderCustomSubjectMixin(ProviderMixinBase):
    ...
    def test_taxonomy_share_title(
        ...
        assert len(bepress_res.json['data']) == len(asdf_res.json['data']) == 1
        assert (
            bepress_res.json['data'][0]['attributes']['taxonomy_name']
            == osf_provider.share_title
        )
        assert (
            asdf_res.json['data'][0]['attributes']['taxonomy_name']
            == asdf_provider.share_title
        )
...
class ProviderHighlightedSubjectsMixin(ProviderMixinBase):
    ...
    def subj_aa(self, provider, subj_a):
        return SubjectFactory(
            provider=provider, text='AA', parent=subj_a, highlighted=True
        )
...
class ProviderListViewTestBaseMixin(ProviderMixinBase):
    ...
    def test_provider_list(self, app, url, user, provider_one, provider_two):
...
@pytest.mark.django_db
class ProviderSubmissionListViewTestBaseMixin(
    ProviderMixinBase, ProviderSubmissionMixinBase
):
    @pytest.fixture()
    def user_one(self):
        return AuthUserFactory()
    ...
    def submission_provider(self, user_one):
        ...
        submission_provider.primary_collection.collected_type_choices = [
            '',
            'asdf',
            'fdsa',
        ]
    ...
    def test_no_permissions(
        ...
        assert (
            not (
                collection_with_provider.guid_links.all()
                | collection_without_provider.guid_links.all()
            )
            .filter(_id=submission_three._id)
            .exists()
        )
    ...
    def test_with_permissions(
        ...
        res = app.post_json_api(
            url,
            payload(
                guid=submission_two._id, status='asdf', subjects=[[subject_one._id]]
            ),
            auth=user_one.auth,
        )
        assert res.status_code == 201

        submission_three = self.submission_class(
            creator=user_two
        )  # user_one does not have referent perm

        res = app.post_json_api(
            url,
            payload(
                guid=submission_three._id, status='asdf', subjects=[[subject_one._id]]
            ),
            auth=user_one.auth,
        )
        assert res.status_code == 201
        ...
        assert (
            ...
            .filter(_id__in=[submission_two._id, submission_three._id])
            .exists()
        )
        assert (
            primary_collection.guid_links.filter(
                _id__in=[submission_two._id, submission_three._id]
            ).count()
            == 2
        )
    ...
    def test_choice_restrictions(
        ...
        res = app.post_json_api(
            url,
            payload(
                guid=submission_two._id, status='one', subjects=[[subject_one._id]]
            ),
            auth=user_one.auth,
            expect_errors=True,
        )
    ...
    def test_filters(
        ...
        res = app.get(
            '{}?filter[id]={}'.format(url, submission_one._id), auth=user_one.auth
        )
        assert res.status_code == 200
        assert len(res.json['data']) == 1
        res = app.get(
            '{}?filter[id]={}'.format(url, submission_two._id), auth=user_one.auth
        )
        assert res.status_code == 200
        ...
        # Sanity
        res = app.get(
            '{}?filter[subjects]={}'.format(url, subject_one._id), auth=user_one.auth
        )
        assert res.status_code == 200
        assert len(res.json['data']) == 0
...
class ProviderLicensesViewTestBaseMixin(ProviderMixinBase):
    ...
    def test_provider_has_no_acceptable_licenses_and_no_default(
        self, app, provider, licenses, url, license_one
    ):
    ...
    def test_provider_has_a_default_license_but_no_acceptable_licenses(
        self, app, provider, licenses, license_two, url
    ):
        provider.licenses_acceptable.clear()
    ...
    def test_provider_has_acceptable_licenses_but_no_default(
        self, app, provider, licenses, license_one, license_two, license_three, url
    ):
    ...
    def test_provider_has_both_acceptable_and_default_licenses(
        self, app, provider, licenses, license_one, license_two, license_three, url
    ):
