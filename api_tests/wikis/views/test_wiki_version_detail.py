...
class TestWikiVersionDetailView(ApiWikiTestCase):
    def _set_up_public_project_with_wiki_page(self, project_options=None):
        ...
        with mock.patch('osf.models.AbstractNode.update_search'):
            ...
            self.public_wiki_version = WikiVersionFactory(
                wiki_page=self.public_wiki_page, user=self.user
            )
        self.public_url = '/{}wikis/{}/versions/{}/'.format(
            API_BASE,
            self.public_wiki_page._id,
            str(self.public_wiki_version.identifier),
        )
        return self.public_wiki_version

    def _set_up_private_project_with_wiki_page(self):
        self.private_project = ProjectFactory(creator=self.user)
        self.private_wiki_version = self._add_project_wiki_version(
            self.private_project, self.user
        )
        self.private_url = '/{}wikis/{}/versions/{}/'.format(
            API_BASE,
            self.private_wiki_version.wiki_page._id,
            str(self.private_wiki_version.identifier),
        )
    ...
    def _set_up_public_registration_with_wiki_page(self):
        ...
        self.public_registration_wiki_version = WikiVersion.objects.get_for_node(
            self.public_registration, 'home'
        )
        self.public_registration.save()
        self.public_registration_url = '/{}wikis/{}/versions/{}/'.format(
            API_BASE,
            self.public_registration_wiki_version.wiki_page._id,
            str(self.public_registration_wiki_version.identifier),
        )
    ...
    def _set_up_private_registration_with_wiki_page(self):
        ...
        self.private_registration_wiki_version = WikiVersion.objects.get_for_node(
            self.private_registration, 'home'
        )
        self.private_registration.save()
        self.private_registration_url = '/{}wikis/{}/versions/{}/'.format(
            API_BASE,
            self.private_registration_wiki_version.wiki_page._id,
            str(self.private_registration_wiki_version.identifier),
        )
    ...
    def test_private_node_logged_in_non_contributor_cannot_view_wiki_version(self):
        self._set_up_private_project_with_wiki_page()
        res = self.app.get(
            self.private_url, auth=self.non_contributor.auth, expect_errors=True
        )
    ...
    def test_private_node_user_with_anonymous_link_can_view_wiki_version(self):
        ...
        private_link.save()
        url = (
            furl.furl(self.private_url)
            .add(query_params={'view_only': private_link.key})
            .url
        )
    ...
    def test_public_registration_logged_in_non_contributor_cannot_view_wiki_version(
        self,
    ):
        self._set_up_public_registration_with_wiki_page()
        res = self.app.get(
    ...
    def test_user_cannot_view_withdrawn_registration_wiki_versions(self):
        ...
        with mock.patch('osf.models.AbstractNode.update_search'):
            ...
            withdrawal.save()
        res = self.app.get(
            self.public_registration_url, auth=self.user.auth, expect_errors=True
        )
        assert_equal(res.status_code, 403)
        assert_equal(
            res.json['errors'][0]['detail'],
    ...
    def test_private_registration_logged_in_non_contributor_cannot_view_wiki_version(
        self,
    ):
        self._set_up_private_registration_with_wiki_page()
        res = self.app.get(
    ...
    def test_wiki_version_has_wiki_page_link(self):
        ...
        expected_url = '/{}wikis/{}/'.format(
            API_BASE, self.public_wiki_version.wiki_page._id
        )
        assert_equal(res.status_code, 200)
        assert_equal(urlparse(url).path, expected_url)
    ...
    def test_wiki_version_has_download_link(self):
        ...
        expected_url = '/{}wikis/{}/versions/{}/content'.format(
            API_BASE,
            self.public_wiki_version.wiki_page._id,
            str(self.public_wiki_version.identifier),
        )
        assert_equal(res.status_code, 200)
    ...
    def test_wiki_version_invalid_identifier_not_found(self):
        self._set_up_public_project_with_wiki_page()
        url = '/{}wikis/{}/versions/{}/'.format(
            API_BASE, self.public_wiki_page._id, 500
        )
    ...
    def test_deleted_wiki_version_not_returned(self):
        ...
        res = self.app.get(url)
        assert_equal(res.status_code, 200)
        self.public_wiki_version.wiki_page.deleted = datetime.datetime(
            2017, 3, 16, 11, 00, tzinfo=pytz.utc
        )
    ...
    def test_public_node_wiki_version_relationship_links(self):
        self._set_up_public_project_with_wiki_page()
        res = self.app.get(self.public_url)
        expected_wiki_page_relationship_url = '{}wikis/{}/'.format(
            API_BASE, self.public_wiki_version.wiki_page._id
        )
        expected_user_relationship_url = '{}users/{}/'.format(API_BASE, self.user._id)
        assert_in(
            expected_wiki_page_relationship_url,
            res.json['data']['relationships']['wiki_page']['links']['related']['href'],
        )
        assert_in(
            expected_user_relationship_url,
            res.json['data']['relationships']['user']['links']['related']['href'],
        )
    ...
    def test_private_node_wiki_version_relationship_links(self):
        ...
        res = self.app.get(self.private_url, auth=self.user.auth)
        expected_wiki_page_relationship_url = '{}wikis/{}/'.format(
            API_BASE, self.private_wiki_version.wiki_page._id
        )
        expected_user_relationship_url = '{}users/{}/'.format(API_BASE, self.user._id)
        assert_in(
            expected_wiki_page_relationship_url,
    ...
    def test_public_registration_wiki_version_relationship_links(self):
        ...
        expected_wiki_page_url = '{}wikis/{}/'.format(
            API_BASE,
            WikiPage.objects.get_for_node(self.public_registration, 'home')._id,
        )
        expected_user_relationship_url = '{}users/{}/'.format(API_BASE, self.user._id)
        assert_in(
            expected_wiki_page_url,
            res.json['data']['relationships']['wiki_page']['links']['related']['href'],
        )
        assert_in(
            expected_user_relationship_url,
    ...
    def test_private_registration_wiki_version_relationship_links(self):
        ...
        expected_wiki_page_url = '{}wikis/{}/'.format(
            API_BASE,
            WikiPage.objects.get_for_node(self.private_registration, 'home')._id,
        )
        expected_user_relationship_url = '{}users/{}/'.format(API_BASE, self.user._id)
        assert_in(
            expected_wiki_page_url,
