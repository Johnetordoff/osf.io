...
class WikiCRUDTestCase:
    ...
    def project_public(self, user_creator):
        project_public = ProjectFactory(is_public=True, creator=user_creator)
    ...
    def project_private(self, user_creator):
        project_private = ProjectFactory(is_public=False, creator=user_creator)
    ...
    def wiki_private(self, project_private, user_creator):
        wiki_page = WikiFactory(
            node=project_private, user=user_creator, page_name='foo'
        )
        WikiVersionFactory(wiki_page=wiki_page, user=user_creator)
        return wiki_page
...
class TestWikiDetailView(ApiWikiTestCase):
    def _set_up_public_project_with_wiki_page(self, project_options=None):
        ...
        with mock.patch('osf.models.AbstractNode.update_search'):
            ...
            self.public_wiki = WikiVersionFactory(
    ...
    def _set_up_public_registration_with_wiki_page(self):
        ...
        self.public_registration_wiki_id = WikiPage.objects.get_for_node(
    ...
    def _set_up_private_registration_with_wiki_page(self):
        ...
        self.private_registration_wiki_id = WikiPage.objects.get_for_node(
    ...
    def test_wiki_has_comments_link(self):
        ...
        CommentFactory(
            ...
            target=Guid.load(self.public_wiki_page._id),
            user=self.user,
        )
        res = self.app.get(url)
        assert_equal(res.status_code, 200)
    ...
    def test_deleted_wiki_not_returned(self):
        ...
        res = self.app.get(url)
        assert_equal(res.status_code, 200)
        self.public_wiki_page.deleted = datetime.datetime(
...
class TestWikiDelete(WikiCRUDTestCase):
    ...
    def test_delete_private_wiki_page(
        self, app, user_write_contributor, url_wiki_private
    ):
    ...
    def test_do_not_delete_private_wiki_page(
        ...
        res = app.delete(
            url_wiki_private, auth=user_read_contributor.auth, expect_errors=True
        )
        assert res.status_code == 403

        # test_do_not_delete_private_wiki_page_as_non_contributor
        res = app.delete(
            url_wiki_private, auth=user_non_contributor.auth, expect_errors=True
        )
        assert res.status_code == 403
    ...
    def test_do_not_delete_registration_wiki_page(
        ...
        app,
        user_creator,
        url_registration_wiki_public,
        url_registration_wiki_private,
    ):
        # test_do_not_delete_wiki_on_public_registration
        res = app.delete(
            url_registration_wiki_public, auth=user_creator.auth, expect_errors=True
        )
        assert res.status_code == 405

        # test_do_not_delete_wiki_on_embargoed_registration
        res = app.delete(
            url_registration_wiki_private, auth=user_creator.auth, expect_errors=True
        )
        assert res.status_code == 405
...
class TestWikiUpdate(WikiCRUDTestCase):
    def test_rename_public_wiki_page(
        ...
        res = app.patch_json_api(
            url_wiki_public, payload, auth=user_write_contributor.auth
        )
    ...
    def test_do_not_rename_public_wiki_page(
        ...
        res = app.patch_json_api(
            url_wiki_home, payload, auth=user_creator.auth, expect_errors=True
        )
        assert res.status_code == 400
        ...
        res = app.patch_json_api(
            url_wiki_public, payload, auth=user_non_contributor.auth, expect_errors=True
        )
        assert res.status_code == 403
    ...
    def test_rename_private_wiki_page(
        ...
        res = app.patch_json_api(
            url_wiki_private, payload, auth=user_write_contributor.auth
        )
        assert res.status_code == 200
        assert res.json['data']['attributes']['name'] == new_name
