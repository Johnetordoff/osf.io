...
class TestWikiVersionList:
    ...
    def public_registration_url(self, public_registration):
        return '/{}wikis/{}/versions/'.format(
            API_BASE, WikiPage.objects.get_for_node(public_registration, 'home')._id
        )
    ...
    def private_registration_url(self, private_registration):
        return '/{}wikis/{}/versions/'.format(
            API_BASE, WikiPage.objects.get_for_node(private_registration, 'home')._id
        )

    def test_return_wiki_versions(
        self,
        app,
        user,
        non_contrib,
        ...
        assert (
            str(
                WikiVersion.objects.get_for_node(
                    private_registration, 'home'
                ).identifier
            )
            in wiki_ids
        )

    def test_wiki_versions_not_returned_for_withdrawn_registration(
    ...
    def test_not_returned(
        ...
        assert len(node_relationships) == 1
        assert (
            WikiPage.objects.get_for_node(public_registration, 'home')._id
            in node_relationships[0]
        )
...
class TestWikiVersionCreate(WikiCRUDTestCase):
    ...
    def test_update_public_wiki_page_as_contributor(
        self, app, user_write_contributor, wiki_public, url_wiki_versions_public
    ):
    ...
    def test_do_not_update_public_wiki_page(
        ...
        res = app.post_json_api(url_wiki_versions_public, payload, expect_errors=True)
        assert res.status_code == 401

    def test_update_private_wiki_page(
        self, app, wiki_private, user_write_contributor, url_wiki_versions_private
    ):
        new_content = fake.text()
        res = app.post_json_api(
    ...
    def test_do_not_update_private_wiki_page(
        ...
        res = app.post_json_api(url_wiki_versions_private, payload, expect_errors=True)
        assert res.status_code == 401
    ...
    def test_do_not_update_disabled_public_wiki_page(
        self, app, user_creator, wiki_public, project_public, url_wiki_versions_public
    ):
