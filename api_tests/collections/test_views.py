...
from api_tests.subjects.mixins import (
    UpdateSubjectsMixin,
    SubjectsFilterMixin,
    SubjectsListMixin,
...
class TestCollectionList:
    ...
    def test_user_get_own_collections(
        self, app, user_one, user_two, collection_deleted, collection
    ):
...
class TestCollectionCreate:
    ...
    def data_collection(self, title_collection):
        return {
            'data': {'type': 'collections', 'attributes': {'title': title_collection,}}
        }
    ...
    def test_create_collection_fails(
        self, app, data_collection, user_one, title_collection
    ):
        ...
        res = app.post_json_api(
            url_collection_list, data_collection, expect_errors=True
        )
        ...
        res = app.post_json_api(
            url_collection_list, data_collection, auth=user_one.auth
        )
        ...
        res = app.get(
            '{}?filter[title]={}'.format(url_collection_list, title_collection),
            auth=user_one.auth,
        )
        ids = [each['id'] for each in res.json['data']]
        assert pid in ids
        ...
        collection = {'data': {'attributes': {'title': title_collection,}}}
        res = app.post_json_api(
            url_collection_list, collection, auth=user_one.auth, expect_errors=True
        )
        assert res.status_code == 400
        ...
        collection = {
            'data': {'attributes': {'title': title_collection,}, 'type': 'Wrong type.'}
        }
        res = app.post_json_api(
            url_collection_list, collection, auth=user_one.auth, expect_errors=True
        )
        ...
        assert (
            ...
            == 'This resource has a type of "collections", but you set the json body\'s type field to "Wrong type.". You probably need to change the type field to match the resource\'s type.'
        )

        # test_creates_collection_properties_not_nested
        project = {'data': {'title': title_collection, 'type': 'collections'}}
        res = app.post_json_api(
            url_collection_list, project, auth=user_one.auth, expect_errors=True
        )
        assert res.status_code == 400
        ...
        collection = {
            'data': {'type': 'collections', 'attributes': {'bookmarks': True,}}
        }
        res = app.post_json_api(
            url_collection_list, collection, auth=user_one.auth, expect_errors=True
        )
        ...
        project = {'data': {'type': 'collections', 'attributes': {'title': 'A' * 513,}}}
        res = app.post_json_api(
            url_collection_list, project, auth=user_one.auth, expect_errors=True
        )
        assert res.status_code == 400
    ...
    def test_create_bookmark_collection(
        ...
        collection = {
            'data': {
                'type': 'collections',
                'attributes': {'title': 'Bookmarks', 'bookmarks': True,},
            }
        }
    ...
    def test_create_bookmark_collection_with_wrong_title(
        ...
        collection = {
            'data': {
                'type': 'collections',
                'attributes': {'title': 'Not Bookmarks', 'bookmarks': True,},
            }
        }
        res = app.post_json_api(url_collection_list, collection, auth=user_one.auth)
        assert res.status_code == 201
    ...
    def test_create_collection_creates_collection_and_sanitizes_html(
        ...
        res = app.post_json_api(
            url_collection_list,
            {'data': {'attributes': {'title': title,}, 'type': 'collections'}},
            auth=user_one.auth,
        )
...
class TestCollectionFiltering:
    ...
    def test_collection_filtering(
        self, app, user_one, user_two, collection_one, collection_two, collection_three
    ):
        ...
        assert len(errors) == 1
        assert (
            errors[0]['detail'] == "'notafield' is not a valid field for this endpoint."
        )
...
class TestCollectionDetail:
    ...
    def test_collection_detail_returns(
        self, app, url_collection_detail, user_one, user_two, collection
    ):
        ...
        node_links_url = res.json['data']['relationships']['node_links']['links'][
            'related'
        ]['href']
...
class TestCollectionUpdate(CollectionCRUDTestCase):
    def test_update_collection_logged_in(
        self, app, url_collection_detail, collection, new_title_collection, user_one
    ):
        res = app.put_json_api(
            url_collection_detail,
            {
                'data': {
                    'id': collection._id,
                    'type': 'collections',
                    'attributes': {'title': new_title_collection,},
                }
            },
            auth=user_one.auth,
        )
    ...
    def test_update_collection_sanitizes_html_properly(
        ...
        new_title = (
            '<strong>Super</strong><script>alert("even cooler")</script> Cool Project'
        )
        res = app.put_json_api(
            url_collection_detail,
            {
    ...
    def test_update_collection_should_fail(
        ...
        project = {
            'data': {
                'type': 'collections',
                'id': collection._id,
                'attributes': {'title': 'A' * 513, 'category': 'project',},
            }
        }
        res = app.put_json_api(
            url_collection_detail, project, auth=user_one.auth, expect_errors=True
        )
        assert res.status_code == 400
        assert res.json['errors'][0]['detail'] == 'Title cannot exceed 512 characters.'
...
class TestCollectionDelete(CollectionCRUDTestCase):
    ...
    def test_cannot_delete_invalid_collection(
        ...
        res = app.delete(
            url_fake_collection_detail, auth=user_one.auth, expect_errors=True
        )
        assert res.status_code == 404
        assert 'detail' in res.json['errors'][0]
    ...
    def test_do_not_delete_collection_unauthorized(
        ...
        res = app.delete_json_api(
            url_collection_detail, auth=user_two.auth, expect_errors=True
        )
        collection.reload()
        assert res.status_code == 403
    ...
    def test_delete_collection_authorized(
        self, app, url_collection_detail, user_one, collection
    ):
        res = app.delete_json_api(
            url_collection_detail, auth=user_one.auth, expect_errors=True
        )
        collection.reload()
...
def make_post_payload(target_node_id, outer_type='node_links', inner_type='nodes'):
    payload = {'data': {'relationships': {'nodes': {'data': {'id': target_node_id,}}}}}
    if outer_type:
...
class TestCollectionNodeLinkCreate:
    ...
    def test_create_node_pointer_already_connected(
        ...
        assert (
            "Target Node '{}' already pointed to by '{}'.".format(
                project_private_user_one._id, collection_one._id
            )
            == error['detail']
        )
    ...
    def test_non_mutational_collection_nodelink_create_tests(
        ...
        res = app.post_json_api(
            url_collection_nodelinks, payload, auth=user_one.auth, expect_errors=True
        )
        assert res.status_code == 400
        assert res.json['errors'][0]['detail'] == 'This field may not be null.'
        assert res.json['errors'][0]['source']['pointer'] == '/data/type'
        ...
        payload = make_post_payload(
            project_public_user_one._id, outer_type='wrong_type'
        )
        res = app.post_json_api(
            url_collection_nodelinks, payload, auth=user_one.auth, expect_errors=True
        )
        ...
        assert (
            res.json['errors'][0]['detail']
            == 'This resource has a type of "node_links", but you set the json body\'s type field to "wrong_type". You probably need to change the type field to match the resource\'s type.'
        )
        ...
        res = app.post_json_api(
            url_collection_nodelinks, payload, auth=user_two.auth, expect_errors=True
        )
        assert res.status_code == 400
        assert res.json['errors'][0]['source']['pointer'] == '/data/relationships'
        assert (
            res.json['errors'][0]['detail']
            == 'Request must include /data/relationships.'
        )
        ...
        res = app.post_json_api(
            url_collection_nodelinks,
            make_post_payload(project_private_user_two._id),
            auth=user_two.auth,
            expect_errors=True,
        )
        assert res.status_code == 403
        ...
        res = app.post_json_api(
            url_collection_nodelinks,
            make_post_payload(collection_one._id),
            auth=user_one.auth,
            expect_errors=True,
        )
        res_json = res.json
        ...
        assert 'detail' in error
        assert (
            "Target Node '{}' not found.".format(collection_one._id) == error['detail']
        )
        assert 'source' in error
        assert 'pointer' in error['source']
        assert '/data/relationships/node_links/data/id' == error['source']['pointer']
...
class TestCollectionNodeLinkDetail:
    ...
    def node_link_private(self, user_one, collection, project_private):
        return collection.collect_object(project_private, user_one).guid.referent

    @pytest.fixture()
    def node_link_public(self, user_one, collection, project_public):
        return collection.collect_object(project_public, user_one).guid.referent

    @pytest.fixture()
    def registration_link_private(self, user_one, registration_private, collection):
        return collection.collect_object(registration_private, user_one).guid.referent

    @pytest.fixture()
    def registration_link_public(self, user_one, registration_public, collection):
        return collection.collect_object(registration_public, user_one).guid.referent
    ...
    def test_can_not_return_deleted_collection_public_registration_pointer(
        ...
        res = app.get(
            url_registration_link_public, auth=user_one.auth, expect_errors=True
        )
        assert res.status_code == 404

    def test_return_deleted_private_node_pointer(
        self, app, collection, url_node_link_private, user_one, project_private
    ):
        res = app.delete(url_node_link_private, auth=user_one.auth)
    ...
    def test_return_deleted_private_registration_pointer(
        self, app, collection, url_registration_link_private, user_one, project_private
    ):
        res = app.delete(url_registration_link_private, auth=user_one.auth)
        project_private.reload()
        assert res.status_code == 204
        ...
        res = app.get(
            url_registration_link_private, auth=user_one.auth, expect_errors=True
        )
        assert res.status_code == 404
    ...
    def test_non_mutational_collection_nodelink_detail_tests(
        ...
        res = app.get(
            url_registration_link_private, auth=user_two.auth, expect_errors=True
        )
        assert res.status_code == 403
        assert 'detail' in res.json['errors'][0]
...
class TestReturnDeletedCollection:
    def test_return_deleted_collection(self, app):
        ...
        CollectionFactory(creator=user, title='A boring collection')
...
class TestCollectionBulkCreate:
    ...
    def collection_one(self, title_one, user_one):
        return {'type': 'collections', 'attributes': {'title': title_one,}}

    @pytest.fixture()
    def collection_two(self, title_two):
        return {'type': 'collections', 'attributes': {'title': title_two,}}

    @pytest.fixture()
    def collection_empty(self):
        return {'type': 'collections', 'attributes': {'title': '',}}
    ...
    def test_bulk_create_logged_in(
        ...
        assert (
            res.json['data'][0]['attributes']['title']
            == collection_one['attributes']['title']
        )
        assert (
            res.json['data'][1]['attributes']['title']
            == collection_two['attributes']['title']
        )
        assert res.content_type == 'application/vnd.api+json'
    ...
    def test_bulk_create_collections_blank_request(
        ...
        res = app.post_json_api(
            url_collections, auth=user_one.auth, expect_errors=True, bulk=True
        )
        assert res.status_code == 400

    def test_bulk_create_error_formatting(
        self, app, url_collections, collection_empty, user_one
    ):
        res = app.post_json_api(
            url_collections,
            {'data': [collection_empty, collection_empty]},
            bulk=True,
            auth=user_one.auth,
            expect_errors=True,
        )
    ...
    def test_non_mutational_collection_bulk_create_tests(
        ...
        res = app.post_json_api(
            url_collections, payload, auth=user_one.auth, expect_errors=True, bulk=True
        )
        assert res.status_code == 400
        ...
        payload = {
            'data': [
                collection_one,
                {'type': 'Incorrect type.', 'attributes': {'title': title_one}},
            ]
        }
        res = app.post_json_api(
            url_collections, payload, auth=user_one.auth, expect_errors=True, bulk=True
...
class TestCollectionBulkUpdate:
    ...
    def test_non_mutational_collection_bulk_update_tests(
            ...
            'data': [
                {
                    'id': collection_one._id,
                    'type': 'collections',
                    'attributes': {'title': "This shouldn't update."},
                },
                {
                    'id': collection_two._id,
                    'type': 'collections',
        ...
        res = app.put_json_api(
            url, payload, auth=user_one.auth, expect_errors=True, bulk=True
        )
        assert res.status_code == 400
        res = app.get(url, auth=user_one.auth)
        ...
        res = app.put_json_api(
            url_collections, payload_collection, expect_errors=True, bulk=True
        )
        assert res.status_code == 401
        assert (
            res.json['errors'][0]['detail']
        ...
        res = app.put_json_api(
            url_collections,
            {
                'data': [
                    ...
                    {'type': 'collections', 'attributes': {'title': title_new}},
                ]
            },
            auth=user_one.auth,
            expect_errors=True,
        ...
        res = app.put_json_api(
            url_collections,
            {
                'data': [
                    payload_collection['data'][1],
                    {'id': collection_one._id, 'attributes': {'title': title_new}},
                ]
            },
            auth=user_one.auth,
            expect_errors=True,
    ...
    def test_bulk_update_collections_logged_in(
        ...
        res = app.put_json_api(
            url_collections, payload_collection, auth=user_one.auth, bulk=True
        )
        assert res.status_code == 200
...
class TestNodeBulkDelete:
    ...
    def payload_one(self, collection_one, collection_two):
        return {
            'data': [
                {'id': collection_one._id, 'type': 'collections'},
    ...
    def test_bulk_delete_collections_logged_in(
        ...
        res = app.delete_json_api(
            url_collections, payload_one, auth=user_one.auth, bulk=True
        )
        assert res.status_code == 204
    ...
    def test_bulk_delete_collections_logged_out(
        ...
        res = app.delete_json_api(
            url_collections, payload_one, expect_errors=True, bulk=True
        )
        assert res.status_code == 401
        assert (
            res.json['errors'][0]['detail']
    ...
    def test_bulk_delete_collections_logged_in_non_contributor(
        self, app, url_collections, payload_two, user_two, user_one, url_project_private
    ):
    ...
    def test_non_mutational_node_bulk_delete_tests(
        ...
        payload = {'data': [{'id': collection_one._id}, {'id': collection_two._id}]}
        res = app.delete_json_api(
            url_collections, payload, auth=user_one.auth, expect_errors=True, bulk=True
        )
        assert res.status_code == 400
        ...
        res = app.delete_json_api(
            url_collections,
            {'data': {'id': collection_one._id, 'type': 'collections'}},
            auth=user_one.auth,
            expect_errors=True,
            bulk=True,
        )
        ...
        res = app.delete_json_api(
            url_collections,
            {'data': [{'type': 'Wrong type', 'id': collection_one._id}]},
            auth=user_one.auth,
            expect_errors=True,
            bulk=True,
        )
        ...
        new_payload = {
            'data': [payload_one['data'][0], {'id': '12345', 'type': 'collections'}]
        }
        res = app.delete_json_api(
            url_collections,
            new_payload,
...
class TestCollectionLinksBulkCreate:
    ...
    def payload_collection_one(
        self,
        project_one_pointer_private,
        project_two_pointer_private,
        reg_one_pointer_private,
        reg_two_pointer_private,
    ):
        return {
            'data': [
                {
                ...
                },
                {
                    ...
                    'relationships': {
                        'target_node': {
                            'data': {'id': reg_one_pointer_private._id, 'type': 'nodes'}
                        }
                    },
                },
                {
                    ...
                    'relationships': {
                        'target_node': {
                            'data': {'id': reg_two_pointer_private._id, 'type': 'nodes'}
                        }
                    },
                },
            ]
            ...
            'data': [
                {
                    'type': 'node_links',
                    'relationships': {
                        'nodes': {'data': {'id': project_user_two._id, 'type': 'nodes'}}
                    },
                },
                {
                    'type': 'node_links',
                    'relationships': {
                        'nodes': {
                            'data': {'id': registration_user_two._id, 'type': 'nodes'}
                        }
                    },
                },
            ]
    ...
    def test_bulk_creates_collection_node_pointer_logged_in_contrib(
        ...
        res = app.post_json_api(
            url_collection_one, payload_collection_one, auth=user_one.auth, bulk=True
        )
        assert res.status_code == 201
        assert res.content_type == 'application/vnd.api+json'
        res_json = res.json['data']
    ...
    def test_bulk_creates_node_pointers_collection_to_non_contributing_node(
        ...
        res = app.post_json_api(
            url_collection_one, payload_user_two, auth=user_one.auth, bulk=True
        )
        assert res.status_code == 201
        assert res.content_type == 'application/vnd.api+json'
        res_json = res.json['data']
    ...
    def test_bulk_creates_node_pointer_already_connected(
        ...
        assert (
            "Target Node '{}' already pointed to by '{}'.".format(
                project_one_pointer_private._id, collection_one._id
            )
    ...
    def test_bulk_create_node_links_blank_request(
        ...
        res = app.post_json_api(
            url_collection_one, auth=user_one.auth, expect_errors=True, bulk=True
        )
        assert res.status_code == 400
    ...
    def test_non_mutational_collection_links_bulk_create_tests(
        ...
        res = app.post_json_api(
            url_collection_one, payload_collection_one, expect_errors=True, bulk=True
        )
        assert res.status_code == 401
        assert 'detail' in res.json['errors'][0]
            ...
            'data': [
                {
                    'relationships': {
                        'nodes': {
                            'data': {'type': 'nodes', 'id': collection_user_two._id}
                        }
                    }
                }
            ]
        ...
        assert (
            res.json['errors'][0]['detail']
            == 'This resource has a type of "node_links", but you set the json body\'s type field to "Wrong type.". You probably need to change the type field to match the resource\'s type.'
        )
...
class TestBulkDeleteCollectionNodeLinks:
    ...
    def collection_one_pointer_one(self, collection_one, project_pointer_one, user_one):
        return collection_one.collect_object(project_pointer_one, user_one)

    @pytest.fixture()
    def collection_one_pointer_two(self, collection_one, project_pointer_two, user_one):
        return collection_one.collect_object(project_pointer_two, user_one)

    @pytest.fixture()
    def payload_collection_one(
        self, collection_one_pointer_one, collection_one_pointer_two
    ):
        return {
            'data': [
    ...
    def collection_two_pointer_one(
        self, collection_two, project_two_pointer_one, user_one
    ):
        return collection_two.collect_object(
            project_two_pointer_one, user_one
        ).guid.referent

    @pytest.fixture()
    def collection_two_pointer_two(
        self, project_two_pointer_two, user_one, collection_two
    ):
        return collection_two.collect_object(
            project_two_pointer_two, user_one
        ).guid.referent

    @pytest.fixture()
    def payload_collection_two(
        self, collection_two_pointer_one, collection_two_pointer_two
    ):
        return {
            'data': [
    ...
    def test_bulk_deletes_collection_node_pointers_succeeds_as_owner(
        ...
        res = app.delete_json_api(
            url_collection_two, payload_collection_two, auth=user_one.auth, bulk=True
        )
        collection_two.reload()
        assert res.status_code == 204
    ...
    def test_non_mutational_bulk_delete_collection_nodelinks_tests(
        ...
        res = app.delete_json_api(
            url_collection_two, auth=user_one.auth, expect_errors=True, bulk=True
        )
        assert res.status_code == 400
        ...
        payload = {'data': [{'type': 'node_links'}, {'type': 'node_links'}]}
        res = app.delete_json_api(
            url_collection_two,
            payload,
            auth=user_one.auth,
        ...
        res = app.delete_json_api(
            url_collection_two, payload_collection_two, expect_errors=True, bulk=True
        )
        assert res.status_code == 401
        assert 'detail' in res.json['errors'][0]
...
class TestCollectionRelationshipNodeLinks:
    ...
    def collection_private(
        self, user_one, node_private, registration_private, auth_user_one
    ):
        collection_private = CollectionFactory(creator=user_one)
        collection_private.collect_object(node_private, user_one)
        collection_private.collect_object(registration_private, user_one)
    ...
    def collection_public(
        ...
        collection_public.collect_object(registration_private, user_two)
    ...
    def make_payload(self, node_admin):
        def payload(node_ids=None, deprecated_type=True):
            ...
            env_linked_nodes = [
                {'type': 'linked_nodes' if deprecated_type else 'nodes', 'id': node_id}
    ...
    def test_get_relationship_linked_nodes(
        self, app, url_private_linked_nodes, user_one, collection_private, node_private
    ):
    ...
    def test_get_relationship_linked_nodes_2_13(
        ...
        res = app.get(
            '{}?version=2.13'.format(url_private_linked_nodes), auth=user_one.auth
        )
        assert res.status_code == 200
        assert collection_private.linked_nodes_self_url in res.json['links']['self']
        assert collection_private.linked_nodes_related_url in res.json['links']['html']
    ...
    def test_get_relationship_linked_registrations(
        ...
        assert res.status_code == 200
        assert (
            collection_private.linked_registrations_self_url
            in res.json['links']['self']
        )
        assert (
            collection_private.linked_registrations_related_url
            in res.json['links']['html']
        )
    ...
    def test_get_relationship_linked_registrations_2_13(
        ...
        res = app.get(
            '{}?version=2.13'.format(url_private_linked_regs), auth=user_one.auth
        )
        assert res.status_code == 200
        assert (
            collection_private.linked_registrations_self_url
    ...
    def test_post_contributing_node(
        ...
        res = app.post_json_api(
            url_private_linked_nodes,
            make_payload([node_contributor._id]),
            auth=user_one.auth,
        )
    ...
    def test_post_contributing_node_2_13(
        ...
        res = app.post_json_api(
            ...
            make_payload([node_contributor._id], False),
            auth=user_one.auth,
        )
    ...
    def test_post_node_already_linked(
        self, app, user_one, url_private_linked_nodes, make_payload, node_private
    ):
        res = app.post_json_api(
            url_private_linked_nodes,
    ...
    def test_delete_with_put_empty_array(
        ...
        payload['data'].pop()
        res = app.put_json_api(url_private_linked_nodes, payload, auth=user_one.auth)
        assert res.status_code == 200
    ...
    def test_delete_linked_registration(
        ...
        payload = {
            'data': [{'id': registration_private._id, 'type': 'linked_registrations'}]
        }
        ...
        assert collection_private.guid_links.filter(
            _id=registration_private._id
        ).exists()
        res = app.delete_json_api(url_private_linked_regs, payload, auth=user_one.auth)
        assert res.status_code == 204
        collection_private.reload()
        assert not collection_private.guid_links.filter(
            _id=registration_private._id
        ).exists()
    ...
    def test_delete_linked_registration_213(
        ...
        payload = {'data': [{'id': registration_private._id, 'type': 'registrations'}]}
    ...
    def test_node_links_and_relationship_represent_same_nodes(
        ...
        res_node_links = app.get(
            '/{}collections/{}/node_links/'.format(API_BASE, collection_private._id),
            auth=user_one.auth,
        )
        node_links_id = []
    ...
    def test_non_mutational_collection_relationship_nodeLinks_tests(
        ...
        res = app.post_json_api(
            url_private_linked_nodes,
            make_payload([node._id, node_contributor._id]),
            auth=user_one.auth,
            expect_errors=True,
        )
        assert res.status_code == 403
        ...
        res = app.post_json_api(
            '{}?version=2.13'.format(url_private_linked_nodes),
            {'data': [{'type': 'linked_nodes', 'id': node_contributor._id}]},
            auth=user_one.auth,
            expect_errors=True,
        )
        assert res.status_code == 409
        ...
        res = app.post_json_api(
            url_public_linked_nodes, make_payload([node_public._id]), expect_errors=True
        )
        assert res.status_code == 401
        ...
        res = app.post_json_api(
            url_private_linked_nodes, make_payload([node._id]), expect_errors=True
        )
        assert res.status_code == 401
...
class TestCollectionRelationshipPreprintLinks:
    ...
    @pytest.fixture()
    def collection_private(self, user_one, preprint_private, auth_user_one):
        collection_private = CollectionFactory(creator=user_one)
        collection_private.collect_object(preprint_private, user_one)
        return collection_private

    @pytest.fixture()
    def collection_public(self, preprint_private, user_two, preprint_public):
    ...
    def make_payload(self, preprint_admin):
        def payload(preprint_ids=None, deprecated_type=True):
            ...
            env_linked_preprints = [
                {
                    'type': 'linked_preprints' if deprecated_type else 'preprints',
                    'id': preprint_id,
                }
    ...
    def test_get_relationship_linked_preprints_2_13(
        ...
        res = app.get(
            '{}?version=2.13'.format(url_private_linked_preprints), auth=user_one.auth
        )
        assert res.status_code == 200
        assert collection_private.linked_preprints_self_url in res.json['links']['self']
        assert res.json['data'][0]['id'] == preprint_private._id
    ...
    def test_post_contributing_preprint(
        ...
        res = app.post_json_api(
            url_private_linked_preprints,
            make_payload([preprint_contributor._id]),
            auth=user_one.auth,
        )
    ...
    def test_post_contributing_preprint_2_13(
        ...
        res = app.post_json_api(
            ...
            make_payload([preprint_contributor._id], False),
            auth=user_one.auth,
        )
    ...
    def test_delete_with_put_empty_array(
        ...
        payload['data'].pop()
        res = app.put_json_api(
            url_private_linked_preprints, payload, auth=user_one.auth
        )
        assert res.status_code == 200
        assert res.json['data'] == payload['data']
    ...
    def test_delete_multiple(
        ...
        res = app.delete_json_api(
            url_private_linked_preprints,
            make_payload([preprint_private._id, preprint_admin._id]),
            auth=user_one.auth,
        )
        assert res.status_code == 204
    ...
    def test_non_mutational_collection_relationship_nodeLinks_tests(
        ...
        res = app.post_json_api(
            url_private_linked_preprints,
            make_payload([preprint._id, preprint_contributor._id]),
            auth=user_one.auth,
            expect_errors=True,
        )
        assert res.status_code == 403
        ...
        res = app.post_json_api(
            ...
            {
                'data': [
                    {'type': 'not_linked_preprints', 'id': preprint_contributor._id}
                ]
            },
            auth=user_one.auth,
            expect_errors=True,
        ...
        res = app.post_json_api(
            '{}?version=2.13'.format(url_private_linked_preprints),
            {'data': [{'type': 'linked_preprints', 'id': preprint_contributor._id}]},
            auth=user_one.auth,
            expect_errors=True,
        )
        assert res.status_code == 409
...
class TestCollectionLinkedPreprints:
    ...
    def url_collection_linked_preprints(self, collection):
        return '/{}collections/{}/linked_preprints/'.format(API_BASE, collection._id)

    @pytest.fixture()
    def id_linked_preprints(self, collection):
        return list(collection.guid_links.values_list('_id', flat=True))

    def test_linked_preprints_returns_everything(
        self, app, url_collection_linked_preprints, user_one, id_linked_preprints
    ):
    ...
    def test_linked_preprints_only_return_viewable_preprints(
        ...
        res = app.get(
            '/{}collections/{}/linked_preprints/'.format(API_BASE, collection._id),
            auth=user.auth,
        )
    ...
    def test_attempt_to_return_linked_preprints_logged_out(
        ...
        res = app.get(url_collection_linked_preprints, auth=None, expect_errors=True)
        assert res.status_code == 401
...
class TestCollectionLinkedNodes:
    ...
    def test_linked_nodes_only_return_viewable_nodes(
        ...
        nodes_returned = [linked_node['id'] for linked_node in res.json['data']]
        registrations_returned = [
            linked_registration['id'] for linked_registration in res.json['data']
        ]
        assert len(nodes_returned) + len(registrations_returned) == len(id_linked_nodes)
        ...
        assert (
            len(nodes_returned) + len(registrations_returned)
            == len(id_linked_nodes) - 2
        )
...
class TestCollectedMetaList:
    ...
    @pytest.fixture()
    def collection_with_three_cgm(
        self, user_one, project_one, project_two, project_three
    ):
        c = CollectionFactory(
            creator=user_one,
            status_choices=['', 'two', 'asdf'],
            collected_type_choices=['', 'asdf'],
        )
    ...
    def payload(self):
        def make_collection_payload(**attributes):
            return {'data': {'type': 'collected-metadata', 'attributes': attributes,}}
    ...
    def test_no_permissions(
        self,
        app,
        collection_with_three_cgm,
        collection_with_one_cgm,
        user_two,
        project_four,
        url,
        payload,
    ...
    def test_filters(
        ...
        res = app.get(
            '{}?filter[id]={}'.format(
                url.format(collection_with_three_cgm._id), project_two._id
            ),
            auth=user_one.auth,
        )
        assert res.status_code == 200
        assert len(res.json['data']) == 1
        res = app.get(
            '{}?filter[status]=two'.format(url.format(collection_with_three_cgm._id)),
            auth=user_one.auth,
        )
        assert res.status_code == 200
        assert len(res.json['data']) == 1
        res = app.get(
            '{}?filter[collected_type]=asdf'.format(
                url.format(collection_with_three_cgm._id)
            ),
            auth=user_one.auth,
        )
        ...
        # Sanity
        res = app.get(
            '{}?filter[subjects]={}'.format(
                url.format(collection_with_three_cgm._id), subject_one._id
            ),
            auth=user_one.auth,
        )
        assert res.status_code == 200
...
class TestCollectedMetaSubjectFiltering(SubjectsFilterMixin):
    ...
    def resource(self, user, collection, project_one, subject_one):
        cgm = collection.collect_object(
            project_one, user, status='one', collected_type='asdf'
        )
    ...
    def resource_two(self, user, collection, project_two, subject_two):
        cgm = collection.collect_object(
            project_two, user, status='one', collected_type='asdf'
        )
...
class TestCollectedMetaSubjectsList(SubjectsListMixin):
    ...
    def url(self, collection, resource):
        return '/{}collections/{}/collected_metadata/{}/subjects/'.format(
            API_BASE, collection._id, resource.guid._id
        )
    ...
    def resource(
        ...
        cgm = collection.collect_object(
            project_one, user_admin_contrib, status='one', collected_type='asdf'
        )
        return cgm
...
class TestUpdateCollectedMetaSubjects(UpdateSubjectsMixin):
    ...
    @pytest.fixture()
    def url(self, collection, resource):
        return '/{}collections/{}/collected_metadata/{}/'.format(
            API_BASE, collection._id, resource.guid._id
        )
...
class TestCollectedMetaSubjectsRelationship(SubjectsRelationshipMixin):
    ...
    @pytest.fixture()
    def url(self, collection, resource):
        return '/{}collections/{}/collected_metadata/{}/relationships/subjects/'.format(
            API_BASE, collection._id, resource.guid._id
        )
...
class TestCollectedMetaDetail:
    ...
    def cgm(self, user_one, collection, project_one, subject_one):
        cgm = collection.collect_object(
            project_one, user_one, status='one', collected_type='asdf'
        )
    ...
    def url(self, collection, cgm):
        return '/{}collections/{}/collected_metadata/{}/'.format(
            API_BASE, collection._id, cgm.guid._id
        )
    ...
    def test_no_permissions(
        self, app, user_two, project_one, collection, cgm, url, payload
    ):
        # Private
        res = app.get(url, expect_errors=True)
        assert res.status_code == 401
        ...
        res = app.patch_json_api(url, payload(status='asdf'), expect_errors=True)
        assert res.status_code == 401
        cgm.reload()
        assert cgm.status != 'asdf'

        res = app.patch_json_api(
            url, payload(status='asdf'), auth=user_two.auth, expect_errors=True
        )
        assert res.status_code == 403
        cgm.reload()
        assert cgm.status != 'asdf'
        ...
        related_url = res.json['data']['relationships']['subjects']['links']['related'][
            'href'
        ]
        expected_url = '{}subjects/'.format(url)
        assert urlparse(related_url).path == expected_url
        self_url = res.json['data']['relationships']['subjects']['links']['self'][
            'href'
        ]
        ...
        project_one.add_contributor(
            user_two, save=True
        )  # has referent (read, write) perms

        res = app.patch_json_api(url, payload(status='asdf'), auth=user_two.auth)
        assert res.status_code == 200
        cgm.reload()
        assert cgm.status == 'asdf'
        ...
        project_one.add_contributor(
            user_two, permissions=ADMIN, save=True
        )  # has referent admin perms
        res = app.delete_json_api(url, auth=user_two.auth,)
        assert res.status_code == 204

    def test_get_collection_metadata_project_belongs_to_multiple_collections(
        self, app, collection, second_collection, project_one, user_one, url
    ):
        res = app.get(url, auth=user_one.auth, expect_errors=True)
        assert res.status_code == 200

        invalid_collection_url = '/{}collections/{}/collected_metadata/{}/'.format(
            API_BASE, 'abcde', project_one._id
        )
        invalid_project_url = '/{}collections/{}/collected_metadata/{}/'.format(
            API_BASE, collection._id, 'abcde'
        )
    ...
    def test_with_permissions(
        self, app, collection, cgm, user_one, user_two, url, payload
    ):
        res = app.get(url, auth=user_one.auth, expect_errors=True)
        assert res.status_code == 200

        res = app.patch_json_api(url, payload(status='asdf'), auth=user_one.auth)
        assert res.status_code == 200
        cgm.reload()
        assert cgm.status == 'asdf'
        ...
        res = app.patch_json_api(url, payload(status='fdsa'), auth=user_one.auth)
        assert res.status_code == 200
        cgm.reload()
    ...
    def test_cannot_update_restricted_fields(
        self, app, collection, cgm, user_one, user_two, url, payload, project_one
    ):
        collection_two = CollectionFactory(creator=user_one)
        guid_two = ProjectFactory(creator=user_one).guids.first()

        res = app.patch_json_api(
            url, payload(guid=guid_two._id), auth=user_one.auth, expect_errors=True
        )
        assert res.status_code == 200
        ...
        res = app.patch_json_api(
            url, payload(creator=user_two._id), auth=user_one.auth, expect_errors=True
        )
        assert res.status_code == 200
