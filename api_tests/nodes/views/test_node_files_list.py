...
class TestNodeFilesList(ApiTestCase):
    ...
    def test_returns_osfstorage_folder_version_two(self):
        fobj = (
            self.project.get_addon('osfstorage').get_root().append_folder('NewFolder')
        )
        fobj.save()
        res = self.app.get(
    ...
    def test_returns_folder_metadata_not_children(self):
        folder = GithubFolder(name='Folder', target=self.project, path='/Folder/')
        folder.save()
    ...
    def test_handles_notfound_waterbutler_request(self):
        ...
        url = '/{}nodes/{}/files/{}/'.format(
            API_BASE, self.project._id, invalid_provider
        )
        res = self.app.get(url, auth=self.user.auth, expect_errors=True)
        assert_equal(res.status_code, 404)
        assert_in('detail', res.json['errors'][0])
...
class TestNodeStorageProviderDetail(ApiTestCase):
    ...
    def test_can_view_if_contributor(self):
        ...
        assert_equal(res.status_code, 200)
        assert_equal(
            res.json['data']['id'], '{}:osfstorage'.format(self.private_project._id)
        )
    ...
    def test_can_view_if_public(self):
        ...
        assert_equal(res.status_code, 200)
        assert_equal(
            res.json['data']['id'], '{}:osfstorage'.format(self.public_project._id)
        )
