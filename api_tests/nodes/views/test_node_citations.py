...
from osf_tests.factories import ProjectFactory, AuthUserFactory, OSFGroupFactory
...
def private_project(admin_contributor, write_contrib, read_contrib, disabled_contrib):
    ...
    private_project.add_contributor(
        write_contrib, permissions=WRITE, auth=Auth(admin_contributor)
    )
    private_project.add_contributor(
        read_contrib, permissions=READ, auth=Auth(admin_contributor)
    )
    private_project.add_contributor(
        disabled_contrib, permissions=READ, auth=Auth(admin_contributor)
    )
...
class NodeCitationsMixin:
    def test_node_citations(
        ...
        post_res = app.post_json_api(
            public_url, {}, auth=admin_contributor.auth, expect_errors=True
        )
...
class TestCustomCitations:
    ...
    def test_custom_citation(
        self,
        app,
        admin_contributor,
        private_project,
        private_url,
        custom_citation_payload,
    ):

        #   test_admin_can_view_private_project_citations
        res = app.put_json_api(
            private_url, custom_citation_payload, auth=admin_contributor.auth
        )
        assert res.status_code == 200
