...
class TestNodeImplicitContributors:
    def test_list_and_filter_implicit_contributors(
        ...
        url = '/{}nodes/{}/implicit_contributors/?filter[given_name]={}'.format(
            API_BASE, component._id, implicit_contributor.given_name
        )
        res = app.get(url, auth=admin_contributor.auth)
        assert res.status_code == 200
        assert res.content_type == 'application/vnd.api+json'
        ...
        url = '/{}nodes/{}/implicit_contributors/?filter[given_name]=NOT_EVEN_A_NAME'.format(
