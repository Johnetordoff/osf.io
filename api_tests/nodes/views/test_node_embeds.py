...
class TestNodeEmbeds:
    ...
    def root_node(self, auth, write_contrib_one, write_contrib_two, make_public_node):
        ...
        root_node.add_contributor(write_contrib_one, WRITE, auth=auth, save=True)
        root_node.add_contributor(write_contrib_two, WRITE, auth=auth, save=True)
        return root_node

    @pytest.fixture()
    def child_one(
        self, auth, write_contrib_one, write_contrib_two, make_public_node, root_node
    ):
        child_one = make_public_node(parent=root_node)
        child_one.add_contributor(write_contrib_one, WRITE, auth=auth, save=True)
        child_one.add_contributor(write_contrib_two, WRITE, auth=auth, save=True)
        return child_one
    ...
    def test_node_embeds(
        ...
        formatted_ids = [
            '{}-{}'.format(child_one._id, contrib._id) for contrib in all_contribs
        ]
        embed_contrib_ids = [
        ...
        assert (
            res.json['data']['embeds']['parent']['errors'][0]['detail']
        ...
        assert res.status_code == 200
        assert (
            res.json['data']['embeds']['contributors']['meta']['total_bibliographic']
            == 3
        )
