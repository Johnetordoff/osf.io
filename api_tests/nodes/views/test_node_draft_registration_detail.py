...
class TestDraftRegistrationDetail(DraftRegistrationTestCase):
    ...
    def test_admin_can_view_draft(
        ...
        assert (
            schema._id
            in data['relationships']['registration_schema']['links']['related']['href']
        )
    ...
    def test_cannot_view_draft(
        ...
        res = app.get(
            url_draft_registrations, auth=user_read_contrib.auth, expect_errors=True
        )
        assert res.status_code == 403

        #   test_read_write_contributor_cannot_view_draft
        res = app.get(
            url_draft_registrations, auth=user_write_contrib.auth, expect_errors=True
        )
        assert res.status_code == 403

        #   test_logged_in_non_contributor_cannot_view_draft
        res = app.get(
            url_draft_registrations, auth=user_non_contrib.auth, expect_errors=True
        )
        assert res.status_code == 403
    ...
    def test_draft_registration_serializer_usage(
        self, app, user, project_public, draft_registration
    ):
...
class TestDraftRegistrationUpdate(DraftRegistrationTestCase):
            ...
            'data': {
                ...
                'attributes': {
                    'registration_metadata': {
                        ...
                        'looked': {'value': 'No'},
                        'comments': {'value': 'This is my first registration.'},
                    }
                },
            }
        }
    ...
    def test_id_required_in_payload(self, app, user, url_draft_registrations):
        payload = {
            'data': {
                'type': 'draft_registrations',
                'attributes': {'registration_metadata': {}},
            }
        }
        res = app.put_json_api(
            url_draft_registrations, payload, auth=user.auth, expect_errors=True
        )
        assert res.status_code == 400
        errors = res.json['errors'][0]
    ...
    def test_admin_can_update_draft(
        self, app, user, schema, project_public, payload, url_draft_registrations
    ):
        ...
        assert (
            data['attributes']['registration_metadata']
    ...
    def test_cannot_update_draft(
        ...
        res = app.put_json_api(
            url_draft_registrations, payload, expect_errors=True, auth=group_mem.auth
        )
        assert res.status_code == 403
    ...
    def test_registration_metadata_question_values_must_be_dictionaries(
        ...
        assert (
            errors['detail']
            == "For your registration your response to the 'Data collection status'"
    ...
    def test_question_in_registration_metadata_must_be_in_schema(
        ...
        assert (
            errors['detail']
            == "For your registration the 'datacompletion' field is extraneous and not"
    ...
    def test_multiple_choice_question_value_must_match_value_in_schema(
        ...
        assert (
            errors['detail']
            == "For your registration your response to the 'Data collection status' field"
            ' is invalid, your response must be one of the provided options.'
        )

    def test_cannot_update_registration_schema(
        self, app, user, schema, payload, schema_prereg, url_draft_registrations
    ):
        payload['data']['relationships'] = {
            'registration_schema': {
                'data': {'id': schema_prereg._id, 'type': 'registration_schema'}
            }
        }
        res = app.put_json_api(
            url_draft_registrations, payload, auth=user.auth, expect_errors=True
        ...
        assert (
            schema._id
            in res.json['data']['relationships']['registration_schema']['links'][
                'related'
            ]['href']
        )
    ...
    def test_required_metaschema_questions_not_required_on_update(
        ...
        payload = {
            'data': {
                ...
                'type': 'draft_registrations',
                'attributes': {
                    'registration_metadata': {'q3': {'value': 'New response'}}
                },
            }
        }
        ...
        assert res.status_code == 200
        assert (
            res.json['data']['attributes']['registration_metadata']['q3']['value']
            == 'New response'
        )
    ...
    def test_required_registration_responses_questions_not_required_on_update(
        ...
        registration_responses = {'q1': 'First question answered'}
        ...
        assert res.status_code == 200
        assert (
            res.json['data']['attributes']['registration_metadata']['q1']['value']
            == registration_responses['q1']
        )
        assert (
            res.json['data']['attributes']['registration_responses']['q1']
            == registration_responses['q1']
        )
    ...
    def test_registration_responses_must_be_a_dictionary(
        ...
        payload_with_registration_responses['data']['attributes'][
            'registration_responses'
        ] = 'Registration data'

        res = app.put_json_api(
            url_draft_registrations,
            payload_with_registration_responses,
            auth=user.auth,
            expect_errors=True,
        )
        errors = res.json['errors'][0]
    ...
    def test_registration_responses_question_values_should_not_be_dicts(
        ...
        payload_with_registration_responses['data']['attributes'][
            'registration_responses'
        ]['datacompletion'] = {'value': 'No, data collection has not begun'}
    ...
    def test_question_in_registration_responses_must_be_in_schema(
        ...
        payload_with_registration_responses['data']['attributes'][
            'registration_responses'
        ]['q11'] = {'value': 'No, data collection has not begun'}
        ...
        assert (
            errors['detail']
            == "Additional properties are not allowed ('q11' was unexpected)"
        )
    ...
    def test_multiple_choice_question_value_in_registration_responses_must_match_value_in_schema(
        ...
        payload_with_registration_responses['data']['attributes'][
            'registration_responses'
        ]['datacompletion'] = 'Nope, data collection has not begun'
...
class TestDraftRegistrationPatch(DraftRegistrationTestCase):
    ...
    def test_admin_can_update_draft(
        self, app, user, schema, payload, url_draft_registrations
    ):
    ...
    def test_cannot_update_draft(
        ...
        res = app.patch_json_api(
            url_draft_registrations, payload, auth=group_mem.auth, expect_errors=True
        )
        assert res.status_code == 403
...
class TestDraftRegistrationDelete(DraftRegistrationTestCase):
    ...
    def test_admin_can_delete_draft(
        self, app, user, url_draft_registrations, project_public
    ):
        res = app.delete_json_api(url_draft_registrations, auth=user.auth)
        assert res.status_code == 204
    ...
    def test_cannot_delete_draft(
        ...
        res = app.delete_json_api(
            url_draft_registrations, expect_errors=True, auth=group_mem.auth
        )
        assert res.status_code == 403
    ...
    def test_draft_that_has_been_registered_cannot_be_deleted(
        ...
        res = app.delete_json_api(
            url_draft_registrations, auth=user.auth, expect_errors=True
        )
        assert res.status_code == 403
        assert (
...
class TestDraftPreregChallengeRegistrationMetadataValidation(DraftRegistrationTestCase):
    ...
    def test_first_level_open_ended_answer_must_have_correct_key(
        ...
        assert (
            res.json['errors'][0]['detail']
            == "For your registration your response to the 'Title' field is invalid."
        )

    def test_first_level_open_ended_answer_must_be_of_correct_type(
        self, app, user, payload, url_draft_registrations
    ):
        payload['data']['attributes']['registration_metadata']['q1'] = {'value': 12345}
        res = app.put_json_api(
            url_draft_registrations, payload, auth=user.auth, expect_errors=True
        )
        assert res.status_code == 400
    ...
    def test_first_level_open_ended_answer_not_expecting_more_nested_data(
        ...
        payload['data']['attributes']['registration_metadata']['q1'] = {
            'value': {'question': {'value': 'This is my answer.'}}
        }
        res = app.put_json_api(
            url_draft_registrations, payload, auth=user.auth, expect_errors=True
        )
    ...
    def test_second_level_answers(self, app, user, payload, url_draft_registrations):
        ...
        assert res.status_code == 200
        assert (
            res.json['data']['attributes']['registration_metadata']['q7']['value'][
                'question'
            ]['value']
            == 'This is my answer.'
        )
    ...
    def test_second_level_open_ended_answer_must_have_correct_key(
        ...
        payload['data']['attributes']['registration_metadata']['q7'] = {
            'value': {'questions': {'value': 'This is my answer.'}}
        }
        res = app.put_json_api(
            url_draft_registrations, payload, auth=user.auth, expect_errors=True
        )
    ...
    def test_third_level_open_ended_answer_must_have_correct_key(
        ...
        payload['data']['attributes']['registration_metadata']['q7'] = {
            'value': {'question': {'values': 'This is my answer.'}}
        }
        res = app.put_json_api(
            url_draft_registrations, payload, auth=user.auth, expect_errors=True
        )
    ...
    def test_second_level_open_ended_answer_must_have_correct_type(
        ...
        payload['data']['attributes']['registration_metadata']['q7'] = {
            'value': {'question': 'This is my answer'}
        }
        res = app.put_json_api(
            url_draft_registrations, payload, auth=user.auth, expect_errors=True
        )
        ...
        assert (
            res.json['errors'][0]['detail']
            == "For your registration your response to the 'Data collection procedures'"
            ' field is invalid.'
        )
    ...
    def test_uploader_metadata(
                    ...
                    'extra': [
                        {
                            ...
                            'viewUrl': '/project/{}/files/osfstorage/{}'.format(
                                project_public._id, draft_registration_prereg._id
                            ),
        ...
        assert (
            ...
            == 'Screen Shot 2016-03-30 at 7.02.05 PM.png'
        )
    ...
    def test_multiple_choice_questions_incorrect_choice(
        ...
        assert (
            res.json['errors'][0]['detail']
            == "For your registration your response to the 'Blinding' field is invalid, your "
    ...
    def test_multiple_choice_questions(
        ...
        assert res.status_code == 200
        assert res.json['data']['attributes']['registration_metadata']['q15'][
            'value'
