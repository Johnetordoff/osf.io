...
class TestNodeLinksList:
    ...
    def test_non_mutational_node_links_list_tests(
        ...
        assert (
            ...
            == 'This feature is deprecated as of version 2.1'
        )
...
class TestNodeLinkCreate:
    ...
    def make_payload(self):
        ...
        def payload(id='rheis'):
            return {
                'data': {
                    'type': 'node_links',
                    'relationships': {'nodes': {'data': {'id': id, 'type': 'nodes'}}},
                }
            }
    ...
    def test_add_node_link(self, app, user, public_pointer_project, public_url):
        ...
        data = {
            'data': {
                'type': 'node_links',
                'attributes': {'id': public_pointer_project._id},
            }
        }
        res = app.post_json_api(public_url, data, auth=user.auth, expect_errors=True)
        assert res.status_code == 400
        ...
        data = {
            'data': {
                'type': 'node_links',
                'relationships': {
                    'data': {'id': public_pointer_project._id, 'type': 'nodes'}
                },
            }
        }
        res = app.post_json_api(public_url, data, auth=user.auth, expect_errors=True)
        ...
        data = {
            'data': {
                'type': 'node_links',
                'relationships': {
                    'nodes': {'id': public_pointer_project._id, 'type': 'nodes'}
                },
            }
        }
        res = app.post_json_api(public_url, data, auth=user.auth, expect_errors=True)
        ...
        data = {
            'data': {
                'type': 'node_links',
                'relationships': {
                    'nodes': {'data': {'id': public_pointer_project._id}}
                },
            }
        }
        res = app.post_json_api(public_url, data, auth=user.auth, expect_errors=True)
        ...
        data = {
            'data': {
                'type': 'node_links',
                'relationships': {'nodes': {'data': {'type': 'nodes'}}},
            }
        }
        res = app.post_json_api(public_url, data, auth=user.auth, expect_errors=True)
        assert res.status_code == 400
        ...
        data = {
            'data': {
                'type': 'node_links',
                'relationships': {'nodes': {'data': {'type': 'nodes', 'id': '12345'}}},
            }
        }
        res = app.post_json_api(public_url, data, auth=user.auth, expect_errors=True)
        assert res.status_code == 400
        ...
        data = {
            'data': {
                ...
                'relationships': {
                    'nodes': {
                        'data': {'type': 'Incorrect!', 'id': public_pointer_project._id}
                    }
                },
            }
        }
    ...
    def test_create_node_link_invalid_data(self, app, user, public_url):
        res = app.post_json_api(
            public_url, 'Incorrect data', auth=user.auth, expect_errors=True
        )
        assert res.status_code == 400
        assert res.json['errors'][0]['detail'] == exceptions.ParseError.default_detail
    ...
    def test_creates_node_link_target_not_nested(
        ...
        payload = {'data': {'type': 'node_links', 'id': private_pointer_project._id}}
        res = app.post_json_api(
            public_url, payload, auth=user_two.auth, expect_errors=True
        )
        assert res.status_code == 400
        assert res.json['errors'][0]['source']['pointer'] == '/data/relationships'
        assert (
    ...
    def test_creates_public_node_pointer_logged_in(
        ...
        with assert_latest_log(NodeLog.POINTER_CREATED, public_project):
            res = app.post_json_api(
                public_url, public_payload, auth=user_two.auth, expect_errors=True
            )
            assert res.status_code == 403
            assert 'detail' in res.json['errors'][0]
            ...
            res = app.post_json_api(
                public_url, public_payload, auth=group_mem.auth, expect_errors=True
            )
            assert res.status_code == 403
    ...
    def test_creates_private_node_pointer_group_member(
        ...
        private_payload = make_payload(id=private_pointer_project._id)
        res = app.post_json_api(private_url, private_payload, auth=group_mem.auth)
        assert res.status_code == 201
    ...
    def test_creates_private_node_pointer_logged_in_non_contributor(
        ...
        private_payload = make_payload(id=private_pointer_project._id)
        res = app.post_json_api(
            private_url, private_payload, auth=user_two.auth, expect_errors=True
        )
        assert res.status_code == 403
        assert 'detail' in res.json['errors'][0]
    ...
    def test_create_node_pointer_non_contributing_node_to_contributing_node(
        ...
        res = app.post_json_api(
            private_url, user_two_payload, auth=user_two.auth, expect_errors=True
        )
        assert res.status_code == 403
        assert 'detail' in res.json['errors'][0]

    def test_create_node_pointer_contributing_node_to_non_contributing_node(
        self, app, user, user_two_project, private_project, private_url, make_payload
    ):
    ...
    def test_create_pointer_non_contributing_node_to_fake_node(
        ...
        res = app.post_json_api(
            private_url, fake_payload, auth=user_two.auth, expect_errors=True
        )
        assert res.status_code == 403
        assert 'detail' in res.json['errors'][0]
    ...
    def test_create_pointer_contributing_node_to_fake_node(
        ...
        fake_payload = make_payload()
        res = app.post_json_api(
            private_url, fake_payload, auth=user.auth, expect_errors=True
        )
        assert res.status_code == 400
        assert 'detail' in res.json['errors'][0]
    ...
    def test_create_fake_node_pointing_to_contributing_node(
        ...
        private_payload = make_payload(id=private_pointer_project._id)
        res = app.post_json_api(
            fake_url, private_payload, auth=user.auth, expect_errors=True
        )
        assert res.status_code == 404
        assert 'detail' in res.json['errors'][0]

        res = app.post_json_api(
            fake_url, private_payload, auth=user_two.auth, expect_errors=True
        )
        assert res.status_code == 404
        assert 'detail' in res.json['errors'][0]

    def test_create_node_pointer_to_itself(
        self, app, user, public_project, public_url, make_payload
    ):
        point_to_itself_payload = make_payload(id=public_project._id)
        res = app.post_json_api(
            public_url, point_to_itself_payload, auth=user.auth, expect_errors=True
        )
        assert res.status_code == 400
    ...
    def test_create_node_pointer_errors(
        ...
        point_to_itself_payload = make_payload(id=public_project._id)
        res = app.post_json_api(
            public_url, point_to_itself_payload, auth=user_two.auth, expect_errors=True
        )
        assert res.status_code == 403
        assert 'detail' in res.json['errors'][0]
        ...
        with assert_latest_log(NodeLog.POINTER_CREATED, public_project):
            ...
            res = app.post_json_api(
                public_url, public_payload, auth=user.auth, expect_errors=True
            )
            assert res.status_code == 400
            assert 'detail' in res.json['errors'][0]
        ...
        payload = {
            'data': {
                'relationships': {
                    'nodes': {'data': {'id': user_two_project._id, 'type': 'nodes'}}
                }
            }
        }
        res = app.post_json_api(
            private_url, payload, auth=user.auth, expect_errors=True
        )
        assert res.status_code == 400
...
class TestNodeLinksBulkCreate:
    ...
    def test_bulk_create_errors(
        ...
        res = app.post_json_api(
            public_url, auth=user.auth, expect_errors=True, bulk=True
        )
        assert res.status_code == 400
        ...
        res = app.post_json_api(
            public_url, payload, auth=user.auth, expect_errors=True, bulk=True
        )
        assert res.status_code == 400
        assert (
        ...
        res = app.post_json_api(
            public_url, payload, auth=user_two.auth, expect_errors=True, bulk=True
        )
        assert res.status_code == 400
        assert res.json['errors'][0]['source']['pointer'] == '/data/relationships'
        assert (
        ...
        res = app.post_json_api(
            public_url, public_payload, expect_errors=True, bulk=True
        )
        assert res.status_code == 401
        assert 'detail' in res.json['errors'][0]
        ...
        res = app.post_json_api(
            private_url, private_payload, expect_errors=True, bulk=True
        )
        assert res.status_code == 401
        assert 'detail' in res.json['errors'][0]
        ...
        res = app.post_json_api(
            private_url, fake_payload, auth=user_two.auth, expect_errors=True, bulk=True
        )
        assert res.status_code == 403
        assert 'detail' in res.json['errors'][0]
        ...
        res = app.post_json_api(
            private_url, fake_payload, auth=user.auth, expect_errors=True, bulk=True
        )
        assert res.status_code == 400
        assert 'detail' in res.json['errors'][0]
        ...
        res = app.post_json_api(
            fake_url, private_payload, auth=user.auth, expect_errors=True, bulk=True
        )
        assert res.status_code == 404
        assert 'detail' in res.json['errors'][0]

        res = app.post_json_api(
            fake_url, private_payload, auth=user_two.auth, expect_errors=True, bulk=True
        )
        assert res.status_code == 404
        assert 'detail' in res.json['errors'][0]
        ...
        res = app.post_json_api(
            private_url, payload, auth=user.auth, expect_errors=True, bulk=True
        )
        assert res.status_code == 400
        assert res.json['errors'][0]['detail'] == 'This field may not be null.'
    ...
    def test_bulk_creates_private_node_pointer_logged_in_contributor(
        ...
        with assert_latest_log(NodeLog.POINTER_CREATED, private_project):
            res = app.post_json_api(
                private_url, private_payload, auth=user.auth, bulk=True
            )
            assert res.status_code == 201
            res_json = res.json['data']
            embedded = res_json[0]['embeds']['target_node']['data']['id']
    ...
    def test_bulk_creates_node_pointers_contributing_node_to_non_contributing_node(
        ...
        with assert_latest_log(NodeLog.POINTER_CREATED, private_project):
            res = app.post_json_api(
                private_url, user_two_payload, auth=user.auth, bulk=True
            )
            assert res.status_code == 201
            assert res.content_type == 'application/vnd.api+json'
            res_json = res.json['data']
        ...
        with assert_latest_log(NodeLog.POINTER_CREATED, public_project):
            ...
            assert (
                "Target Node '{}' already pointed to by '{}'.".format(
                    public_pointer_project_one._id, public_project._id
                )
...
class TestBulkDeleteNodeLinks:
    ...
    def private_pointer_one(
        self, user, private_project, private_project_pointer_project_one
    ):
    ...
    def private_pointer_two(
        self, user, private_project, private_project_pointer_project_two
    ):
        return private_project.add_pointer(
    ...
    def public_pointer_one(
        self, user, public_project, public_project_pointer_project_one
    ):
    ...
    def public_pointer_two(
        self, user, public_project, public_project_pointer_project_two
    ):
        return public_project.add_pointer(
    ...
    def test_bulk_delete_errors(
        ...
        res = app.delete_json_api(
            public_url,
            {'data': {'id': public_project._id, 'type': 'node_links'}},
            auth=user.auth,
            expect_errors=True,
            bulk=True,
        )
    ...
    def test_return_bulk_deleted_public_node_pointer(
        self, app, user, public_project, public_pointer_one, public_url, public_payload
    ):
        with assert_latest_log(NodeLog.POINTER_REMOVED, public_project):
            res = app.delete_json_api(
                public_url, public_payload, auth=user.auth, bulk=True
    ...
    def test_bulk_delete_link_that_is_not_linked_to_correct_node(
        ...
        res = app.delete_json_api(
            private_url, public_payload, auth=user.auth, expect_errors=True, bulk=True
        )
        assert res.status_code == 400
