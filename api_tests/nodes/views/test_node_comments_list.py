...
class NodeCommentsListMixin(object):
    ...
    def test_return_comments(
        ...
        res = app.get(
            project_private_dict['url'], auth=user_non_contrib, expect_errors=True
        )
        assert res.status_code == 401
        assert (
            res.json['errors'][0]['detail']
...
class TestNodeCommentsListFiles(NodeCommentsListMixin):
    ...
    def project_public_dict(self, user):
        ...
        comment_public = CommentFactory(
            node=project_public, user=user, target=file_public.get_guid(), page='files'
        )
        url_public = '/{}nodes/{}/comments/'.format(API_BASE, project_public._id)
        return {
...
class TestNodeCommentsListWiki(NodeCommentsListMixin):
    ...
    def project_private_dict(self, user):
        project_private = ProjectFactory(is_public=False, creator=user)
        wiki_page_private = WikiFactory(
            node=project_private, user=user, page_name='Charmander'
        )
        comment_private = CommentFactory(
            node=project_private,
            user=user,
            target=Guid.load(wiki_page_private._id),
            page='wiki',
        )
        url_private = '/{}nodes/{}/comments/'.format(API_BASE, project_private._id)
        return {
...
class NodeCommentsCreateMixin(object):
    ...
    def test_node_comments(
        ...
        res = app.post_json_api(
            project_dict['url'], project_dict['payload'], auth=user.auth
        )
        assert res.status_code == 201
        assert (
            res.json['data']['attributes']['content']
            == project_dict['payload']['data']['attributes']['content']
        )

        #   test_private_node_private_comment_level_logged_in_read_contributor_can_comment
        project_dict = project_private_comment_private
        res = app.post_json_api(
            project_dict['url'], project_dict['payload'], auth=user_read_contrib.auth
        )
        assert res.status_code == 201
        assert (
            res.json['data']['attributes']['content']
        ...
        project_dict = project_private_comment_private
        res = app.post_json_api(
            project_dict['url'], project_dict['payload'], expect_errors=True
        )
        assert res.status_code == 401
        assert (
            res.json['errors'][0]['detail']
        ...
        project_dict = project_public_comment_public
        res = app.post_json_api(
            project_dict['url'], project_dict['payload'], auth=user_non_contrib.auth
        )
        assert res.status_code == 201
        assert (
            res.json['data']['attributes']['content']
...
class TestNodeCommentCreate(NodeCommentsCreateMixin):
    ...
    def payload(self):
        def make_payload(target_id):
            return {
                'data': {
                    'type': 'comments',
                    'attributes': {'content': 'This is a comment'},
                    'relationships': {
                        'target': {'data': {'type': 'nodes', 'id': target_id}}
                    },
                }
            }
    ...
    def test_create_comment_errors(
        ...
        project_dict = project_private_comment_private
        res = app.post_json_api(
            project_dict['url'], 'Incorrect data', auth=user.auth, expect_errors=True
        )
        assert res.status_code == 400

        #   test_create_comment_no_relationships
        project_dict = project_private_comment_private
        payload_req = {'data': {'type': 'comments', 'attributes': {'content': '4:44'}}}
        res = app.post_json_api(
            project_dict['url'], payload_req, auth=user.auth, expect_errors=True
        )
        assert res.status_code == 400
        assert (
            res.json['errors'][0]['detail']
        ...
        payload_req = {
            'data': {
                'type': 'comments',
                'attributes': {'content': 'Center for Closed Logic'},
                'relationships': {},
            }
        }
        res = app.post_json_api(
        ...
        payload_req = {
            'data': {
                ...
                'attributes': {'content': 'This is a comment'},
                'relationships': {
                    'data': {'type': 'nodes', 'id': project_dict['project']._id}
                },
            }
        }
        res = app.post_json_api(
        ...
        payload_req = {
            'data': {
                'type': 'comments',
                'attributes': {'content': 'This is a comment'},
                'relationships': {'target': {'data': {'type': 'nodes', 'id': ''}}},
            }
        }
        res = app.post_json_api(
            project_dict['url'], payload_req, auth=user.auth, expect_errors=True
        ...
        res = app.post_json_api(
            project_dict['url'], payload_project, auth=user.auth, expect_errors=True
        )
        assert res.status_code == 400
        assert (
            res.json['errors'][0]['detail']
            == "Invalid comment target '" + str(project_id) + "'."
        )
        ...
        payload_req = {
            'data': {
                ...
                'relationships': {
                    'target': {
                        'data': {'type': 'Invalid', 'id': project_dict['project']._id}
                    }
                },
            }
        }
        ...
        assert (
            res.json['errors'][0]['detail']
            == 'The target resource has a type of "nodes", but you set the json body\'s type field to "Invalid".  You probably need to change the type field to match the target resource\'s type.'
        )
        ...
        payload_req = {
            'data': {
                ...
                'attributes': {'content': 'This is a comment'},
                'relationships': {
                    'target': {'data': {'id': project_dict['project']._id}}
                },
            }
        }
        res = app.post_json_api(
        ...
        payload_req = {
            'data': {
                'type': 'comments',
                'attributes': {'content': ('c' * (osf_settings.COMMENT_MAXLENGTH + 3))},
                'relationships': {
                    'target': {
                        'data': {'type': 'nodes', 'id': project_dict['project']._id}
                    }
        ...
        res = app.post_json_api(
            url_fake, payload_fake, auth=user.auth, expect_errors=True
        )
    ...
    def test_create_comment_with_allowed_tags(
        ...
        payload = {
            'data': {
                'type': 'comments',
                'attributes': {'content': '<em>Logic</em> <strong>Reason</strong>'},
                'relationships': {
                    'target': {
                        'data': {'type': 'nodes', 'id': project_dict['project']._id}
                    }
...
class TestFileCommentCreate(NodeCommentsCreateMixin):
        ...
        def make_payload(target_id):
            return {
                'data': {
                    ...
                    'attributes': {'content': 'This is a comment'},
                    'relationships': {
                        'target': {'data': {'type': 'files', 'id': target_id}}
                    },
                }
            }
    ...
    def test_create_file_comment_errors(
        ...
        assert (
            res.json['errors'][0]['detail']
            == "Invalid comment target '" + str(file._id) + "'."
        )
        ...
        assert (
            res.json['errors'][0]['detail']
            == 'The target resource has a type of "files", but you set the json body\'s type field to "Invalid".  You probably need to change the type field to match the target resource\'s type.'
        )
...
class TestWikiCommentCreate(NodeCommentsCreateMixin):
        ...
        def make_payload(target_id):
            return {
                'data': {
                    ...
                    'attributes': {'content': 'This is a comment'},
                    'relationships': {
                        'target': {'data': {'type': 'wiki', 'id': target_id}}
                    },
                }
            }
    ...
    def test_create_wiki_comment_errors(
        ...
        assert (
            res.json['errors'][0]['detail']
            == "Invalid comment target '" + str(wiki_page._id) + "'."
        )
        ...
        payload_req = {
            'data': {
                ...
                'relationships': {
                    'target': {
                        'data': {'type': 'Invalid', 'id': project_dict['wiki']._id}
                    }
                },
            }
        }
        ...
        assert (
            res.json['errors'][0]['detail']
            == 'The target resource has a type of "wiki", but you set the json body\'s type field to "Invalid".  You probably need to change the type field to match the target resource\'s type.'
        )
...
class TestCommentRepliesCreate(NodeCommentsCreateMixin):
        ...
        def make_payload(comment_id):
            return {
                'data': {
                    ...
                    'attributes': {'content': 'This is a comment'},
                    'relationships': {
                        'target': {'data': {'type': 'comments', 'id': comment_id}}
                    },
                }
            }
    ...
    def project_private_comment_public(self, user, user_read_contrib, payload):
        ...
        comment_reply = CommentFactory(
            node=project_private, target=Guid.load(comment_private._id), user=user
        )
        url_private = '/{}nodes/{}/comments/'.format(API_BASE, project_private._id)
    ...
    def project_public_comment_public(self, user, user_read_contrib, payload):
        ...
        comment_public = CommentFactory(node=project_public, user=user)
        comment_reply = CommentFactory(
            node=project_public, target=Guid.load(comment_public._id), user=user
        )
        url_public = '/{}nodes/{}/comments/'.format(API_BASE, project_public._id)
    ...
    def test_create_comment_reply_invalid_target_id(
        ...
        assert (
            res.json['errors'][0]['detail']
            == "Invalid comment target '" + str(target_comment._id) + "'."
        )
...
class TestCommentFiltering:
    ...
    def test_filtering(
        ...
        assert (
            project._id
            in res.json['data'][0]['relationships']['target']['links']['related'][
                'href'
            ]
        )
        assert (
            project._id
            in res.json['data'][1]['relationships']['target']['links']['related'][
                'href'
            ]
        )
    ...
    def test_filtering_for_comment_replies(self, app, user, project, comment, url_base):
        CommentFactory(node=project, user=user, target=Guid.load(comment._id))
    ...
    def test_filtering_by_target_file(self, app, user, project, url_base):
    ...
    def test_filtering_by_target_wiki(
        self, app, user, project, comment, comment_deleted, url_base
    ):
        wiki_page = WikiFactory(node=project, user=user)
        CommentFactory(
            node=project, user=user, target=Guid.load(wiki_page._id), page='wiki'
        )
        url = url_base + '?filter[target]=' + str(wiki_page._id)
        res = app.get(url, auth=user.auth)
        assert len(res.json['data']) == 1
        assert (
            wiki_page.get_absolute_url()
            == res.json['data'][0]['relationships']['target']['links']['related'][
                'href'
            ]
        )

    def test_filtering_by_page_files(self, app, user, project, url_base):
        test_file = test_utils.create_test_file(project, user)
        CommentFactory(
            node=project, user=user, target=test_file.get_guid(), page='files'
        )
