...
class TestNodeDetail:
    ...
    def test_return_project_details(
        ...
        assert (
            res.json['data']['attributes']['description'] == project_public.description
        )
        ...
        assert res.json['data']['attributes']['current_user_permissions'] == [
            permissions.READ
        ]
        ...
        assert (
            res.json['data']['attributes']['description'] == project_private.description
        )
        ...
        assert (
            res.json['data']['relationships']['region']['data']['id']
            == project_private.osfstorage_region._id
        )
    ...
    def test_node_has(self, app, url_public, project_public):
        ...
        url = res.json['data']['relationships']['contributors']['links']['related'][
            'href'
        ]
        ...
        url = res.json['data']['relationships']['node_links']['links']['related'][
            'href'
        ]
        ...
        url = res.json['data']['relationships']['registrations']['links']['related'][
            'href'
        ]
        ...
        related_url = res.json['data']['relationships']['affiliated_institutions'][
            'links'
        ]['related']['href']
        expected_url = '{}institutions/'.format(url_public)
        assert urlparse(related_url).path == expected_url
        self_url = res.json['data']['relationships']['affiliated_institutions'][
            'links'
        ]['self']['href']
        ...
        res = app.get(
            url_public + '?version={}'.format(subjects_as_relationships_version)
        )
    ...
    def test_node_has_correct_unread_comments_count(
        ...
        CommentFactory(node=project_public, user=contributor, page='node')
        res = app.get('{}?related_counts=True'.format(url_public), auth=user.auth)
        unread = res.json['data']['relationships']['comments']['links']['related'][
            'meta'
        ]['unread']
        unread_comments_node = unread['node']
        assert unread_comments_node == 1

    def test_node_has_correct_wiki_page_count(
        self, user, app, url_private, project_private
    ):
        res = app.get('{}?related_counts=True'.format(url_private), auth=user.auth)
        assert (
            res.json['data']['relationships']['wikis']['links']['related']['meta'][
                'count'
            ]
            == 0
        )
    ...
    def test_cannot_return_registrations_at_node_detail_endpoint(
        ...
        res = app.get(
            '/{}nodes/{}/'.format(API_BASE, registration._id),
            auth=user.auth,
            expect_errors=True,
        )
        assert res.status_code == 404
    ...
    def test_node_list_embed_identifier_link(
        self, app, user, project_public, url_public
    ):
        url = url_public + '?embed=identifiers'
        res = app.get(url)
        assert res.status_code == 200
        link = res.json['data']['relationships']['identifiers']['links']['related'][
            'href'
        ]
        assert '{}identifiers/'.format(url_public) in link

    def test_node_shows_wiki_relationship_based_on_disabled_status_and_version(
        self, app, user, project_public, url_public
    ):
    ...
    def test_shows_access_requests_enabled_field_based_on_version(
        self, app, user, project_public, url_public
    ):
        url = url_public + '?version=latest'
        res = app.get(url, auth=user.auth)
    ...
    def test_node_shows_correct_templated_from_count(
        self, app, user, project_public, url_public
    ):
        url = url_public
        res = app.get(url)
        ...
        ProjectFactory(
            title='template copy', template_node=project_public, creator=user
        )
        project_public.reload()
        res = app.get(url)
        assert res.json['meta']['templated_by_count'] == 1

    def test_node_show_correct_children_count(
        ...
        assert (
            res.json['data']['relationships']['children']['links']['related']['meta'][
                'count'
            ]
            == 1
        )
    ...
    def test_node_shows_related_count_for_linked_by_relationships(
        self, app, user, project_public, url_public, project_private
    ):
        url = url_public + '?related_counts=true'
        res = app.get(url)
        assert (
            'count'
            in res.json['data']['relationships']['linked_by_nodes']['links']['related'][
                'meta'
            ]
        )
        assert (
            'count'
            in res.json['data']['relationships']['linked_by_registrations']['links'][
                'related'
            ]['meta']
        )
        assert (
            res.json['data']['relationships']['linked_by_nodes']['links']['related'][
                'meta'
            ]['count']
            == 0
        )
        assert (
            res.json['data']['relationships']['linked_by_registrations']['links'][
                'related'
            ]['meta']['count']
            == 0
        )
    ...
    def test_node_shows_correct_forks_count_including_private_forks(
        self, app, user, project_private, url_private, user_two
    ):
        project_private.add_contributor(
            user_two, permissions=permissions.ADMIN, auth=Auth(user)
        )
        ...
        assert (
            'count'
            in res.json['data']['relationships']['forks']['links']['related']['meta']
        )
        assert (
            res.json['data']['relationships']['forks']['links']['related']['meta'][
                'count'
            ]
            == 0
        )
    ...
    def test_current_user_permissions(
        self, app, user, url_public, project_public, user_two
    ):
        ...
        assert (
            permissions.READ
            not in res.json['data']['attributes']['current_user_permissions']
        )
        assert (
        ...
        project_public.add_contributor(
            new_user, permissions=permissions.WRITE, auth=Auth(project_public.creator)
        )
        ...
        assert (
            permissions.READ
            in res.json['data']['attributes']['current_user_permissions']
        )
        assert (
            res.json['data']['attributes'][
                'current_user_is_contributor_or_group_member'
...
class NodeCRUDTestCase:
    ...
    def make_node_payload(self):
        def payload(node, attributes, relationships=None):
            ...
            payload_data = {
                'data': {'id': node._id, 'type': 'nodes', 'attributes': attributes,}
            }
...
class TestNodeUpdate(NodeCRUDTestCase):
    def test_node_institution_update(
        ...
        project_private.add_contributor(
            user_two, permissions=permissions.ADMIN, auth=Auth(project_private.creator)
        )
        ...
        payload = make_node_payload(
            project_private, {'public': False}, relationships=affiliated_institutions
        )
        res = app.patch_json_api(
            url_private, payload, auth=user_two.auth, expect_errors=False
        )
        assert res.status_code == 200
    ...
    def test_node_update_invalid_data(self, app, user, url_public):
        res = app.put_json_api(
            url_public, 'Incorrect data', auth=user.auth, expect_errors=True
        )
    ...
    def test_cannot_make_project_public_if_non_admin_contributor(
        ...
        project_private.add_contributor(
            non_admin, permissions=permissions.WRITE, auth=Auth(project_private.creator)
        )
        project_private.save()
        res = app.patch_json(
            url_private,
    ...
    def test_partial_update_errors(
        ...
        res = app.patch_json_api(
            url_public,
            {'data': {'type': 'nodes', 'attributes': {'title': title_new,}}},
            auth=user.auth,
            expect_errors=True,
        )
        assert res.status_code == 400
        ...
        res = app.patch_json_api(
            url_public,
            {'data': {'id': project_public._id, 'attributes': {'title': title_new,}}},
            auth=user.auth,
            expect_errors=True,
        )
        assert res.status_code == 400
        ...
        res = app.patch_json_api(
            url_public,
            {'data': {'id': project_public._id, 'type': 'nodes', 'title': title_new,}},
            auth=user.auth,
            expect_errors=True,
        )
        assert res.status_code == 200
    ...
    def test_multiple_patch_requests_with_same_category_generates_one_log(
        ...
        new_category = 'data'
        payload = make_node_payload(
            project_private, attributes={'category': new_category}
        )
...
class TestNodeDelete(NodeCRUDTestCase):
    ...
    def test_deletes_private_node_logged_in_write_contributor(
        ...
        project_private.add_contributor(user_two, permissions=permissions.WRITE)
        project_private.save()
        res = app.delete(url_private, auth=user_two.auth, expect_errors=True)
        project_private.reload()
        assert res.status_code == 403
...
class TestReturnDeletedNode:
    ...
    def test_return_deleted_node(
        ...
        res = app.delete(
            url_project_private_deleted, auth=user.auth, expect_errors=True
        )
        assert res.status_code == 410
...
class TestNodeTags:
    ...
    def test_contributor_can_add_tag_to_public_project(
        ...
        with assert_latest_log(NodeLog.TAG_ADDED, project_public):
            res = app.patch_json_api(
                url_public, payload_public, auth=user.auth, expect_errors=True
            )
            assert res.status_code == 200
    ...
    def test_add_tag_to_project_errors(
        ...
        res = app.patch_json_api(
            url_public, payload_public, expect_errors=True, auth=None
        )
        assert res.status_code == 401

        #   test_non_authenticated_user_cannot_add_tag_to_private_project
        res = app.patch_json_api(
            url_private, payload_private, expect_errors=True, auth=None
        )
        assert res.status_code == 401

        #   test_non_contributor_cannot_add_tag_to_public_project
        res = app.patch_json_api(
            url_public, payload_public, expect_errors=True, auth=user_non_contrib.auth
        )
        assert res.status_code == 403

        #   test_non_contributor_cannot_add_tag_to_private_project
        res = app.patch_json_api(
            url_private, payload_private, expect_errors=True, auth=user_non_contrib.auth
        )
        assert res.status_code == 403

        #   test_read_only_contributor_cannot_add_tag_to_public_project
        res = app.patch_json_api(
            url_public, payload_public, expect_errors=True, auth=user_read_contrib.auth
        )
        assert res.status_code == 403
    ...
    def test_tags_add_and_remove_properly(
        ...
        with assert_latest_log(NodeLog.TAG_ADDED, project_private):
            ...
            assert len(res.json['data']['attributes']['tags']) == 1
            assert res.json['data']['attributes']['tags'][0] == 'new-tag'
        with assert_latest_log(NodeLog.TAG_REMOVED, project_private), assert_latest_log(
            NodeLog.TAG_ADDED, project_private, 1
        ):
...
class TestNodeLicense:
    ...
    def project_public(self, user, user_admin, node_license, year, copyright_holders):
        ...
        project_public.node_license = NodeLicenseRecordFactory(
            node_license=node_license, year=year, copyright_holders=copyright_holders
        )
        project_public.save()
        return project_public
    ...
    def test_node_has(
        ...
        res = app.get(url_public)
        assert (
            project_public.node_license.year
        ...
        actual_license_url = res.json['data']['relationships']['license']['links'][
            'related'
        ]['href']
        ...
        res = app.get(url_private, auth=user.auth)
        assert (
            project_private.node_license.year
            == res.json['data']['attributes']['node_license']['year']
        )
...
class TestNodeUpdateLicense:
    ...
    def make_payload(self):
        def payload(
            node_id, license_id=None, license_year=None, copyright_holders=None
        ):
            attributes = {}
            ...
            if license_year and copyright_holders:
                ...
                attributes = {'node_license': {'year': license_year}}
            elif copyright_holders:
                attributes = {'node_license': {'copyright_holders': copyright_holders}}
            ...
            return (
                ...
                if license_id
                    ...
                    'data': {'type': 'nodes', 'id': node_id, 'attributes': attributes}
                }
            )
    ...
    def test_admin_update_license_with_invalid_id(
        self, user_admin_contrib, node, make_payload, make_request, url_node
    ):
        data = make_payload(node_id=node._id, license_id='thisisafakelicenseid')

        assert node.node_license is None

        res = make_request(
            url_node, data, auth=user_admin_contrib.auth, expect_errors=True
        )
        assert res.status_code == 404
    ...
    def test_admin_can_update_license(
        ...
        data = make_payload(node_id=node._id, license_id=license_cc0._id)
    ...
    def test_cannot_update(
        ...
        res = make_request(
            url_node, data, auth=user_write_contrib.auth, expect_errors=True
        )
        assert res.status_code == 403
        assert (
            res.json['errors'][0]['detail']
        ...
        res = make_request(
            url_node, data, auth=user_read_contrib.auth, expect_errors=True
        )
        assert res.status_code == 403
        assert (
            res.json['errors'][0]['detail']
        ...
        res = make_request(
            url_node, data, auth=user_non_contrib.auth, expect_errors=True
        )
        assert res.status_code == 403
        assert (
            res.json['errors'][0]['detail']
    ...
    def test_update_node_with_existing_license_year_attribute_only(
        ...
        data = make_payload(node_id=node._id, license_year='2015')
    ...
    def test_update_node_with_existing_license_copyright_holders_attribute_only(
        ...
        data = make_payload(
            node_id=node._id, copyright_holders=['Mr. Monument', 'Princess OSF']
        )
    ...
    def test_update_node_license_without_license_id(
        ...
        data = make_payload(
            node_id=node._id, license_year='2015', copyright_holders=['Ben, Jerry']
        )
    ...
    def test_update_node_license_without_required_copyright_holders_in_payload_(
        ...
        data = make_payload(
            node_id=node._id, license_id=license_no._id, license_year='1994'
        )
