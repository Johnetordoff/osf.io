...
class TestNodeList:
    ...
    def test_node_list_has_root(
        self, app, user, url, public_project, private_project, deleted_project
    ):
        res = app.get(url, auth=user.auth)
        ...
        assert all(
            [each['relationships'].get('root') is not None for each in res.json['data']]
        )
    ...
    def test_node_list_embed_region(self, app, url, public_project):
        ...
        assert res.status_code == 200
        assert (
            res.json['data'][0]['embeds']['region']['data']['id'] == DEFAULT_REGION_ID
        )
    ...
    def test_node_list_region_relationship(self, app, url, public_project):
        ...
        assert res.status_code == 200
        assert (
            res.json['data'][0]['relationships']['region']['data']['id']
            == public_project.osfstorage_region._id
        )
    ...
    def test_current_user_permissions(
        self, app, user, url, public_project, non_contrib
    ):
        ...
        assert (
            permissions.READ
            not in res.json['data'][0]['attributes']['current_user_permissions']
        )
        ...
        public_project.add_contributor(
            new_user, permissions=permissions.WRITE, auth=Auth(public_project.creator)
        )
        res = app.get(url_public, auth=new_user.auth)
        assert res.json['data'][0]['attributes']['current_user_permissions'] == [
            permissions.WRITE,
            permissions.READ,
        ]
        ...
        assert (
            permissions.READ
            in res.json['data'][0]['attributes']['current_user_permissions']
        )
        assert res.json['data'][0]['attributes']['current_user_is_contributor'] is False
    ...
    def test_current_user_permissions_group_member(
        self, app, user, url, public_project
    ):
        # in most recent API version, read isn't implicit for public nodes
        url_public = url + '?version=2.11'
...
class TestNodeFiltering:
    ...
    def test_filtering(
        ...
        filter_url = '{}?filter[id]={}'.format(url, public_project_one._id)
        ...
        filter_url = '{}?filter[title][ne]=Public%20Project%20One'.format(url)
        res = app.get(filter_url, auth=user_one.auth)
        assert res.status_code == 200
        data = res.json['data']
        assert len(data) == 4
        ...
        filter_url = '{}?filter[description][ne]=reason%20is%20shook'.format(url)
        res = app.get(filter_url, auth=user_one.auth)
        assert res.status_code == 200
        data = res.json['data']
        assert len(data) == 5
    ...
    def test_filtering_tags(
        self, app, public_project_one, public_project_two, tag_one, tag_two
    ):
    ...
    def test_filtering_multiple_fields(self, app, user_one):
        ...
        project_public_two = ProjectFactory(
            is_public=True, title='kitten', creator=user_one, description='test'
        )
        ...
        project_public_four = ProjectFactory(
            is_public=True, title='test', creator=user_one, description='test'
        )
    ...
    def test_filtering_tags_exact(
        self, app, user_one, public_project_one, public_project_two
    ):
        ...
        res = app.get(
            '/{}nodes/?filter[tags]=reason'.format(API_BASE), auth=user_one.auth
        )
    ...
    def test_filtering_tags_capitalized_query(self, app, user_one, public_project_one):
        ...
        res = app.get(
            '/{}nodes/?filter[tags]=COVFEFE'.format(API_BASE), auth=user_one.auth
        )
        assert len(res.json.get('data')) == 1
    ...
    def test_filtering_tags_returns_distinct(self, app, user_one, public_project_one):
        ...
        public_project_one.add_tag('caT', Auth(user_one))
        public_project_one.add_tag('CAT', Auth(user_one))
        res = app.get('/{}nodes/?filter[tags]=cat'.format(API_BASE), auth=user_one.auth)
        assert len(res.json.get('data')) == 1

    def test_filtering_contributors(
        self, app, user_one, user_one_private_project, preprint
    ):
        res = app.get(
            '/{}nodes/?filter[contributors]={}'.format(API_BASE, user_one._id),
            auth=user_one.auth,
        )
    ...
    def test_unpublished_preprint_in_preprint_true_filter_results(
        ...
        unpublished = PreprintFactory(
            creator=user_one, project=ProjectFactory(creator=user_one)
        )
    ...
    def test_withdrawn_preprint_in_preprint_true_filter_results(
        ...
        preprint_one = PreprintFactory(
            is_published=False, creator=user_one, project=project_one
        )
...
class TestNodeCreate:
    ...
    def test_create_node_errors(
        self, app, user_one, public_project, private_project, url
    ):

        #   test_node_create_invalid_data
        res = app.post_json_api(
            url, 'Incorrect data', auth=user_one.auth, expect_errors=True
        )
        assert res.status_code == 400
    ...
    def test_creates_public_project_logged_in(
        ...
        res = app.post_json_api(url, public_project, auth=user_one.auth)
        assert res.status_code == 201
        self_link = res.json['data']['links']['self']
        assert (
            res.json['data']['attributes']['title']
            == public_project['data']['attributes']['title']
        )
        assert (
        ...
        assert (
            res.json['data']['attributes']['category']
            == public_project['data']['attributes']['category']
        )
        assert res.json['data']['relationships']['affiliated_institutions']['links'][
            'self'
        ]['href'] == '{}relationships/institutions/'.format(self_link)
        assert res.content_type == 'application/vnd.api+json'
    ...
    def test_creates_private_project_logged_in_contributor(
        ...
        assert (
            res.json['data']['attributes']['title']
            == private_project['data']['attributes']['title']
        )
        assert (
            res.json['data']['attributes']['description']
            == private_project['data']['attributes']['description']
        )
        assert (
            res.json['data']['attributes']['category']
            == private_project['data']['attributes']['category']
        )
        pid = res.json['data']['id']
        project = AbstractNode.load(pid)
    ...
    def test_create_from_template_errors(self, app, user_one, user_two, url):
        ...
        res = app.post_json_api(
            url, templated_project_data, auth=user_one.auth, expect_errors=True
        )
    ...
    def test_create_component_inherit_contributors(
        ...
        component_data = {
            'data': {
                'type': 'nodes',
                'attributes': {'title': title, 'category': category,},
            }
        }
    ...
    def test_create_component_inherit_contributors_with_unregistered_contributor(
        ...
        osf_group.add_unregistered_member(
            fullname='far', email='foo@bar.baz', auth=Auth(user_one)
        )
        osf_group.save()
        ...
        actual_perms = set(
            [
                ...
                for contributor in new_component.contributor_set.all()
            ]
        )
    ...
    def test_create_component_inherit_contributors_with_blacklisted_email(
        ...
        res = app.post_json_api(
            url, component_data, auth=user_one.auth, expect_errors=True
        )
        assert res.status_code == 400
        assert (
            res.json['errors'][0]['detail']
            == 'Unregistered contributor email address domain is blacklisted.'
        )
    ...
    def test_create_project_with_region_relationship(
        ...
        private_project['data']['relationships'] = {
            'region': {'data': {'type': 'region', 'id': region._id}}
        }
        res = app.post_json_api(url, private_project, auth=user_one.auth)
        assert res.status_code == 201
    ...
    def test_create_project_with_no_region_specified(
        self, app, user_one, private_project, url
    ):
        res = app.post_json_api(url, private_project, auth=user_one.auth)
        assert res.status_code == 201
    ...
    def test_create_project_with_bad_region_query_param(
        ...
        res = app.post_json_api(
            url + '?region={}'.format(bad_region_id),
            private_project,
            auth=user_one.auth,
            expect_errors=True,
        )
        assert res.status_code == 400
        assert res.json['errors'][0]['detail'] == 'Region {} is invalid.'.format(
            bad_region_id
        )
    ...
    def test_create_project_errors(
        ...
        assert (
            res.json['errors'][0]['detail']
            == 'This resource has a type of "nodes", but you set the json body\'s type field to "Wrong type.". You probably need to change the type field to match the resource\'s type.'
        )
...
class TestNodeLicenseOnCreate:
    ...
    def make_payload(self):
        def payload(license_id=None, license_year=None, copyright_holders=None):
            attributes = {}
            ...
            if license_year and copyright_holders:
                ...
                attributes = {
                    ...
                    'category': 'project',
                    'tags': ['foo', 'bar'],
                    'node_license': {'year': license_year,},
                }
                ...
                attributes = {
                    ...
                    'category': 'project',
                    'tags': ['foo', 'bar'],
                    'node_license': {'copyright_holders': copyright_holders},
                }
            ...
            return (
                ...
                if license_id
                else {'data': {'type': 'nodes', 'attributes': attributes}}
            )

        return payload

    def test_node_license_on_create(self, app, user, url, node_license, make_payload):
        data = make_payload(
            copyright_holders=['Haagen', 'Dazs'],
            license_year='2200',
            license_id=node_license._id,
        )
        res = app.post_json_api(url, data, auth=user.auth)
        ...
        assert (
            res.json['data']['relationships']['license']['data']['id']
            == node_license._id
        )
    ...
    def test_create_node_license_errors(
        ...
        data = make_payload(license_year='2200', copyright_holders=['Ben', 'Jerry'])
        res = app.post_json_api(url, data, auth=user.auth, expect_errors=True)
        assert res.status_code == 400
        assert (
            res.json['errors'][0]['detail']
            == 'License ID must be provided for a Node License.'
        )

        # test_creating_a_node_license_without_a_copyright_holder
        data = make_payload(license_year='2200', license_id=node_license._id)
        res = app.post_json_api(url, data, auth=user.auth, expect_errors=True)
        assert res.status_code == 400
        assert (
            res.json['errors'][0]['detail']
        ...
        data = make_payload(
            copyright_holders=['Baskin', 'Robbins'], license_id=node_license._id
        )
        res = app.post_json_api(url, data, auth=user.auth, expect_errors=True)
        assert res.status_code == 400
        assert (
            res.json['errors'][0]['detail'] == 'year must be specified for this license'
        )
...
class TestNodeBulkCreate:
    ...
    def test_bulk_create(
        ...
        res = app.post_json_api(
            url, node_create_list, auth=user_one.auth, expect_errors=True, bulk=True
        )
        assert (
        ...
        payload = {
            'data': [
                ...
                {
                    'type': 'Incorrect type.',
                    'attributes': {'category': category, 'title': title},
                },
            ]
        }
        res = app.post_json_api(
        ...
        assert res.json['errors'][0]['detail'] == 'This field is required.'
        assert (
            res.json['errors'][1]['source']['pointer'] == '/data/1/attributes/category'
        )
        ...
        payload = {
            'data': [
                public_project,
                {'type': 'nodes', 'attributes': {'category': category}},
            ]
        }
        res = app.post_json_api(
            url, payload, auth=user_one.auth, expect_errors=True, bulk=True
    ...
    def test_bulk_create_logged_in(
        ...
        assert (
            ...
            == public_project['attributes']['title']
        )
        assert (
            res.json['data'][0]['attributes']['category']
            == public_project['attributes']['category']
        )
        assert (
            res.json['data'][0]['attributes']['description']
            == public_project['attributes']['description']
        )
        assert (
            res.json['data'][1]['attributes']['title']
            == private_project['attributes']['title']
        )
        assert (
            res.json['data'][1]['attributes']['category']
            == public_project['attributes']['category']
        )
        assert (
            res.json['data'][1]['attributes']['description']
            == public_project['attributes']['description']
        )
        assert res.content_type == 'application/vnd.api+json'
...
class TestNodeBulkUpdate:
    ...
    def test_bulk_update_errors(
        ...
        payload = {
            'data': [
                ...
                },
                {
                    'id': public_project_two._id,
                    'type': 'nodes',
                    'attributes': {'title': '', 'category': 'hypothesis'},
                },
            ]
        }
        public_project_one_url = '/{}nodes/{}/'.format(API_BASE, public_project_one._id)
        res = app.put_json_api(
            url, payload, auth=user.auth, expect_errors=True, bulk=True
        )
        assert res.status_code == 400
            ...
            'data': [
                {
                    'id': '12345',
                    'type': 'nodes',
                    'attributes': {'title': new_title, 'category': new_category},
                },
        ...
        res = app.put_json_api(
            url, private_payload, auth=non_contrib.auth, expect_errors=True, bulk=True
        )
        assert res.status_code == 403
        assert (
            res.json['errors'][0]['detail']
        ...
        res = app.put_json_api(
            url, empty_payload, auth=user.auth, expect_errors=True, bulk=True
        )
        assert res.status_code == 400
        assert len(res.json['errors']) == 2
        errors = res.json['errors']
        ...
        res = app.put_json_api(
            url, node_update_list, auth=user.auth, expect_errors=True, bulk=True
        )
        assert (
            res.json['errors'][0]['detail'] == 'Bulk operation limit is 100, got 101.'
        )
        ...
        res = app.put_json_api(
            url,
            {'data': [public_payload['data'][1], new_payload]},
            auth=user.auth,
            expect_errors=True,
            bulk=True,
        )
    ...
    def test_bulk_update_private_projects_logged_in_read_only_contrib(
        ...
        res = app.put_json_api(
            url, private_payload, auth=read_contrib.auth, expect_errors=True, bulk=True
        )
        assert res.status_code == 403
        assert (
            res.json['errors'][0]['detail']
    ...
    def test_bulk_update_with_tags(self, app, user, public_project_one, url):
        ...
        res = app.put_json_api(
            url, new_payload, auth=user.auth, expect_errors=True, bulk=True
        )
        assert res.status_code == 200
...
class TestNodeBulkPartialUpdate:
    ...
    @pytest.fixture()
    def public_payload(self, public_project_one, public_project_two, new_title):
        return {
            'data': [
                {
                    'id': public_project_one._id,
    ...
    @pytest.fixture()
    def private_payload(self, private_project_one, private_project_two, new_title):
        return {
            'data': [
                {
                    'id': private_project_one._id,
    ...
    def test_bulk_partial_update_errors(
        ...
        payload = {
            'data': [
                {'id': '12345', 'type': 'nodes', 'attributes': {'title': new_title}},
                public_payload['data'][0],
            ]
        }
        ...
        res = app.patch_json_api(
            url,
            {'data': [{'type': 'nodes', 'attributes': {'title': new_title}}]},
            auth=user.auth,
            expect_errors=True,
            bulk=True,
        )
    ...
    def test_bulk_partial_update_public_projects_logged_in(
        ...
        res = app.patch_json_api(
            url, public_payload, auth=user.auth, expect_errors=True, bulk=True
        )
        assert res.status_code == 200
        assert {public_project_one._id, public_project_two._id} == {
            res.json['data'][0]['id'],
...
class TestNodeBulkUpdateSkipUneditable:
    ...
    def test_bulk_update_skips(
        ...
        res = app.put_json_api(
            nodes_url, public_payload, auth=user_one.auth, expect_errors=True, bulk=True
        )
        assert res.status_code == 403
        ...
        res = app.patch_json_api(
            url, public_payload, auth=user_one.auth, expect_errors=True, bulk=True
        )
        assert res.status_code == 403
        user_one_public_project_one.reload()
        user_one_public_project_two.reload()
...
class TestNodeBulkDelete:
    ...
    def public_project_one(self, user_one):
        return ProjectFactory(
            title='Project One', is_public=True, creator=user_one, category='project'
        )
    ...
    def public_payload(self, public_project_one, public_project_two):
        return {
            'data': [
                ...
                {'id': public_project_two._id, 'type': 'nodes'},
            ]
        }
    ...
    def private_payload(self, user_one_private_project):
        return {'data': [{'id': user_one_private_project._id, 'type': 'nodes'}]}
    ...
    def test_bulk_delete_errors(
        ...
        res = app.delete_json_api(
            res_url, public_payload, auth=user_one.auth, expect_errors=True, bulk=True
        )
        assert res.status_code == 409
        assert (
            res.json['errors'][0]['detail']
        ...
        res = app.delete_json_api(
            res_url, auth=user_one.auth, expect_errors=True, bulk=True
        )
        assert res.status_code == 400
        assert (
            res.json['errors'][0]['detail']
        ...
        res = app.delete_json_api(
            url, auth=user_one.auth, expect_errors=True, bulk=True
        )
        assert res.status_code == 400

        #   test_bulk_delete_no_type
        payload = {
            'data': [{'id': public_project_one._id}, {'id': public_project_two._id}]
        }
        res = app.delete_json_api(
            url, payload, auth=user_one.auth, expect_errors=True, bulk=True
        )
        ...
        payload = {'data': [{'type': 'nodes'}, {'id': 'nodes'}]}
        res = app.delete_json_api(
            url, payload, auth=user_one.auth, expect_errors=True, bulk=True
        )
        assert res.status_code == 400
        ...
        res = app.delete_json_api(
            url,
            {'data': {'id': public_project_one._id, 'type': 'nodes'}},
            auth=user_one.auth,
            expect_errors=True,
            bulk=True,
        )
        ...
        res = app.delete_json_api(
            url,
            {'data': [{'type': 'Wrong type', 'id': public_project_one._id}]},
            auth=user_one.auth,
            expect_errors=True,
            bulk=True,
        )
        ...
        res = app.delete_json_api(
            url, new_payload, auth=user_one.auth, expect_errors=True, bulk=True
        )
        assert res.status_code == 400
        assert (
            res.json['errors'][0]['detail'] == 'Bulk operation limit is 100, got 101.'
    ...
    def test_bulk_delete_with_query_params(
        self, app, user_one, url, type_query_param, public_query_params
    ):
    ...
    def test_bulk_delete_private_projects_logged_in_contributor(
        ...
        res = app.delete_json_api(
            url, private_payload, auth=user_one.auth, expect_errors=True, bulk=True
        )
        assert res.status_code == 204

        res = app.get(
            user_one_private_project_url, auth=user_one.auth, expect_errors=True
        )
        assert res.status_code == 410
    ...
    def test_bulk_delete_private_projects_logged_in_non_contributor(
        ...
        res = app.delete_json_api(
            url, private_payload, auth=user_two.auth, expect_errors=True, bulk=True
        )
        assert res.status_code == 403
        assert (
            res.json['errors'][0]['detail']
    ...
    def test_bulk_delete_invalid_payload_one_not_found(
        ...
        new_payload = {
            'data': [public_payload['data'][0], {'id': '12345', 'type': 'nodes'}]
        }
        res = app.delete_json_api(
            url, new_payload, auth=user_one.auth, expect_errors=True, bulk=True
        )
    ...
    def test_bulk_delete_project_with_linked_node(
        self, app, user_one, public_project_parent, public_component, url
    ):
...
class TestNodeBulkDeleteSkipUneditable:
    ...
    def payload(
        ...
        return {
            'data': [
                {'id': public_project_one._id, 'type': 'nodes',},
                {'id': public_project_two._id, 'type': 'nodes',},
                {'id': public_project_three._id, 'type': 'nodes',},
                {'id': public_project_four._id, 'type': 'nodes',},
            ]
        }
    ...
    def test_skip_uneditable_bulk_delete(
        self, app, user_one, public_project_three, public_project_four, payload, url
    ):
...
class TestNodeListPagination:
    ...
    def test_embed_page_size_not_affected(self, app, users, projects, url):
        ...
        assert (
            res.json['data'][0]['embeds']['contributors']['links']['meta']['per_page']
            == 10
        )
