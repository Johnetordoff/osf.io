...
class TestNodeChildrenList:
    ...
    def test_return_public_node_children_list(
        self, app, public_component, public_project_url
    ):
    ...
    def test_node_children_list_does_not_include_deleted(
        self, app, user, public_project, public_component, component, public_project_url
    ):
    ...
    def test_node_children_related_counts_duplicate_query_results(
        self, app, user, public_project, private_project, public_project_url
    ):
        user_2 = AuthUserFactory()

        # Adding a child component
        child = NodeFactory(
            parent=public_project, creator=user, is_public=True, category='software'
        )
        child.add_contributor(user_2, permissions.WRITE, save=True)
        # Adding a grandchild
        NodeFactory(parent=child, creator=user, is_public=True)
        # Adding a node link
        public_project.add_pointer(private_project, auth=Auth(public_project.creator))
    ...
    def test_node_children_related_counts(self, app, user, public_project):
        ...
        child = NodeFactory(
            parent=parent, creator=user_2, is_public=False, category='software'
        )
        NodeFactory(parent=child, creator=user_2, is_public=False)

        # child has one component. `user` can view due to implict admin perms
        component_url = '/{}nodes/{}/children/'.format(
            API_BASE, child._id, auth=user.auth
        )
    ...
    def test_child_counts_permissions(self, app, user, public_project):
        ...
        url = '/{}nodes/{}/?related_counts=children'.format(
            API_BASE, public_project._id
        )
    ...
    def test_private_node_children_with_view_only_link(
        self, user, app, private_project, component, view_only_link, private_project_url
    ):

        # get node related_counts with vol before vol is attached to components
        node_url = '/{}nodes/{}/?related_counts=children&view_only={}'.format(
            API_BASE, private_project._id, view_only_link.key
        )
        ...
        view_only_link_url = '{}?view_only={}'.format(
            private_project_url, view_only_link.key
        )
...
class TestNodeChildCreate:
    ...
    def test_creates_child(self, app, user, project, child, url):
        ...
        project.add_contributor(
            read_contrib, permissions=permissions.READ, auth=Auth(user), save=True
        )
        ...
        res = app.post_json_api(url, child, auth=group_mem.auth, expect_errors=True)
        assert res.status_code == 403
    ...
    def test_creates_child_logged_in_write_contributor(
        ...
        project.add_contributor(
            write_contrib, permissions=permissions.WRITE, auth=Auth(user), save=True
        )
        ...
        assert (
            ...
            == child['data']['attributes']['category']
        )
        ...
        assert (
            AbstractNode.load(child_id).logs.latest().action == NodeLog.PROJECT_CREATED
        )
...
class TestNodeChildrenBulkCreate:
    ...
    def test_bulk_creates_children_auth_errors(
        ...
        res = app.post_json_api(
            url, {'data': [child_one, child_two]}, expect_errors=True, bulk=True
        )
        assert res.status_code == 401
    ...
    def test_bulk_creates_children_logged_in_owner(
        ...
        res = app.post_json_api(
            url, {'data': [child_one, child_two]}, auth=user.auth, bulk=True
        )
        assert res.status_code == 201
        assert (
            res.json['data'][0]['attributes']['title']
            == child_one['attributes']['title']
        )
        assert (
        ...
        assert (
            ...
            == child_one['attributes']['category']
        )
        assert (
            res.json['data'][1]['attributes']['title']
            == child_two['attributes']['title']
        )
        assert (
    ...
    def test_bulk_creates_children_child_logged_in_write_contributor(
        ...
        res = app.post_json_api(
            url, {'data': [child_one, child_two]}, auth=write_contrib.auth, bulk=True
        )
        assert res.status_code == 201
        assert (
            res.json['data'][0]['attributes']['title']
    ...
    def test_bulk_creates_children_payload_errors(
        ...
        res = app.post_json_api(
            url, child, auth=user.auth, expect_errors=True, bulk=True
        )
        assert res.status_code == 400
        assert res.json['errors'][0]['detail'] == 'This field may not be null.'
        ...
        assert (
            res.json['errors'][2]['source']['pointer'] == '/data/1/attributes/category'
        )
