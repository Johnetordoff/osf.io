...
class TestNodeForksList:
    ...
    def test_authenticated_contributor_can_access_private_node_forks_list(
        ...
        auth = Auth(user)
        expected_logs = list(
            private_project.get_logs_queryset(auth).values_list('action', flat=True)
        )
...
class TestNodeForkCreate:
    ...
    def test_create_fork_from_public_project_with_new_title(
        ...
        res = app.post_json_api(
            public_project_url, fork_data_with_title, auth=user.auth
        )
    ...
    def test_create_fork_from_private_project_with_new_title(
        ...
        res = app.post_json_api(
            private_project_url, fork_data_with_title, auth=user.auth
        )
        assert res.status_code == 201
    ...
    def test_can_fork_public_node_logged_in(
        ...
        assert (
            res.json['data']['attributes']['title'] == 'Fork of ' + public_project.title
        )
        assert public_project.logs.latest().date and fork.last_logged

    def test_cannot_fork_errors(
        self, app, fork_data, public_project_url, private_project_url
    ):
        ...
        non_contrib = AuthUserFactory()
        res = app.post_json_api(
            private_project_url, fork_data, auth=non_contrib.auth, expect_errors=True
        )
        assert res.status_code == 403
        assert (
            res.json['errors'][0]['detail']
    ...
    def test_fork_private_components_no_access(
        self, app, user_two, public_project, fork_data, public_project_url
    ):
        user_three = AuthUserFactory()
        url = public_project_url + '?embed=children'
        NodeFactory(parent=public_project, creator=user_two, is_public=False)
    ...
    def test_fork_components_you_can_access(
        ...
        assert (
            res.json['data']['embeds']['children']['data'][0]['attributes']['title']
            == new_component.title
        )

    def test_fork_private_node_links(
        self, app, user, user_two, private_project, fork_data, private_project_url
    ):
    ...
    def test_can_fork_registration(self, app, user, private_project, fork_data):
        ...
        assert (
            res.json['data']['attributes']['title'] == 'Fork of ' + registration.title
        )
    ...
    def test_send_email_failed(
        ...
        with mock.patch.object(NodeForksSerializer, 'save', side_effect=Exception()):
            with mock.patch.object(
                mails, 'send_mail', return_value=None
            ) as mock_send_mail:
