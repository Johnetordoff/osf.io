...
class TestNodeLinkedByNodesList:
    ...
    def test_linked_by_nodes_lists_nodes(
        self, app, url_public, user, project_public, project_private
    ):
    ...
    def test_linked_by_nodes_doesnt_list_registrations(
        self, app, url_public, user, project_public, project_private
    ):
        project_private.add_pointer(project_public, auth=Auth(user), save=True)
        project_public.reload()
    ...
    def test_linked_by_nodes_respect_permissions(
        self, app, url_public, user, project_public, project_private
    ):
        project_private.add_pointer(project_public, auth=Auth(user), save=True)
        project_public.reload()
    ...
    def test_linked_by_nodes_doesnt_show_deleted(
        self, app, url_public, user, project_public, project_private
    ):
        project_private.add_pointer(project_public, auth=Auth(user), save=True)
...
class TestNodeLinkedByRegistrationsList:
    ...
    @pytest.fixture()
    def url_public(self, project_public):
        return '/{}nodes/{}/linked_by_registrations/'.format(
            API_BASE, project_public._id
        )

    def test_linked_by_registrations_links_registrations(
        self, app, url_public, user, project_public, project_private
    ):
        res = app.get(url_public, auth=user.auth)
        assert len(res.json['data']) == 0
    ...
    def test_linked_by_registrations_doesnt_list_nodes(
        self, app, url_public, user, project_public, project_private
    ):
        project_private.add_pointer(project_public, auth=Auth(user), save=True)
        project_public.reload()
    ...
    def test_linked_by_registrations_respect_permissions(
        self, app, url_public, user, project_public, project_private
    ):
        project_private.add_pointer(project_public, auth=Auth(user), save=True)
        registration = RegistrationFactory(project=project_private, creator=user)
    ...
    def test_linked_by_registrations_doesnt_show_retracted(
        self, app, url_public, user, project_public, project_private
    ):
        project_private.add_pointer(project_public, auth=Auth(user), save=True)
        registration = RegistrationFactory(project=project_private, creator=user)
