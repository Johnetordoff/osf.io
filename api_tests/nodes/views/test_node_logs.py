...
class TestNodeLogList:
    ...
    def pointer_embargo(self, user):
        return RegistrationFactory(
            creator=user, embargo=EmbargoFactory(user=user), is_public=False
        )
    ...
    def test_project_creation(
        self, app, user, public_project, private_project, public_url, private_url
    ):
        ...
        assert (
            public_project.logs.first().action
        ...
        assert (
            res.json['data'][API_FIRST]['attributes']['action']
            == public_project.logs.first().action
        )
        ...
        assert (
            res.json['data'][API_FIRST]['attributes']['action']
            == private_project.logs.first().action
        )
    ...
    def test_project_add_remove_contributor(
        self, app, user, contrib, user_auth, public_project, public_url
    ):
    ...
    def test_pointers(
        self, app, user, user_auth, contrib, public_project, pointer, public_url
    ):
        ...
        assert (
            res.json['data'][API_LATEST]['attributes']['params']['pointer']['id']
            == pointer._id
        )
    ...
    def test_registration_pointers(
        ...
        assert res.json['data'][API_LATEST]['attributes']['action'] == 'pointer_created'
        assert (
            res.json['data'][API_LATEST]['relationships']['linked_registration'][
                'data'
            ]['id']
            == pointer_registration._id
        )
