...
class DraftRegistrationTestCase:
    ...
    @pytest.fixture()
    def project_public(
        self, user, user_write_contrib, user_read_contrib, group, group_mem
    ):
        project_public = ProjectFactory(is_public=True, creator=user)
        project_public.add_contributor(
            user_write_contrib, permissions=permissions.WRITE
        )
...
class TestDraftRegistrationList(DraftRegistrationTestCase):
    ...
    def schema(self):
        return RegistrationSchema.objects.get(
            name='Open-Ended Registration', schema_version=OPEN_ENDED_SCHEMA_VERSION
        )
    ...
    def test_admin_can_view_draft_list(
        ...
        assert (
            schema._id
            in data[0]['relationships']['registration_schema']['links']['related'][
                'href'
            ]
        )
    ...
    def test_draft_with_registered_node_does_not_show_up_in_draft_list(
        ...
        reg = RegistrationFactory(
            project=project_public, draft_registration=draft_registration
        )
...
class TestDraftRegistrationCreate(DraftRegistrationTestCase):
        ...
        return {
            'data': {
                ...
                'relationships': {
                    ...
                    'provider': {
                        'data': {'type': 'registration-providers', 'id': provider._id,}
                    },
                },
            }
        }
    ...
    def test_type_is_draft_registrations(
        self, app, user, metaschema_open_ended, url_draft_registrations
    ):
        draft_data = {
            'data': {
                'type': 'nodes',
        ...
        res = app.post_json_api(
            url_draft_registrations, draft_data, auth=user.auth, expect_errors=True
        )
    ...
    def test_registration_supplement_errors(
        ...
        draft_data = {
            'data': {
                ...
                'relationships': {
                    'registration_schema': {
                        'data': {'type': 'registration_schema', 'id': 'Invalid schema'}
                    },
                    'provider': {
                        'data': {'type': 'registration-providers', 'id': provider._id,}
                    },
        ...
        draft_data = {
            'data': {
                ...
                'relationships': {
                    'registration_schema': {
                        'data': {'type': 'registration_schema', 'id': schema._id}
                    },
                    'provider': {
                        'data': {'type': 'registration-providers', 'id': provider._id,}
                    },
    ...
    def test_cannot_create_draft_errors(self, app, user, project_public, payload):
        ...
        res = app.post_json_api(
            url_project, payload, auth=user.auth, expect_errors=True
        )
        ...
        payload = {
            'data': {
                ...
                'relationships': {
                    ...
                    'registration_schema': {
                        'data': {'type': 'registration_schema', 'id': prereg_schema._id}
                    },
                    'provider': {
                        'data': {'type': 'registration-providers', 'id': provider._id,}
                    },
    ...
    def test_registration_supplement_must_be_supplied(
        ...
        draft_data = {'data': {'type': 'draft_registrations', 'attributes': {}}}
        res = app.post_json_api(
            url_draft_registrations, draft_data, auth=user.auth, expect_errors=True
        )
    ...
    def test_cannot_supply_both_registration_metadata_and_registration_responses(
        ...
        assert res.status_code == 400
        assert (
            'Please use `registration_responses` as `registration_metadata` will be deprecated in the future.'
            in errors['detail']
        )
    ...
    def test_supply_registration_responses_on_creation(
        ...
        payload['data']['relationships']['registration_schema']['data'][
            'id'
        ] = schema._id
        payload['data']['attributes']['registration_responses'] = {
            'looked': 'Yes',
    ...
    def test_registration_responses_question_values_must_not_be_dictionaries(
        ...
        payload['data']['attributes']['registration_responses'] = {}
        payload['data']['attributes']['registration_responses']['datacompletion'] = {
            'value': 'No, data collection has not begun'
        }
    ...
    def test_registration_responses_multiple_choice_question_value_must_match_value_in_schema(
        ...
        payload['data']['attributes']['registration_responses'][
            'datacompletion'
        ] = 'Nope, data collection has not begun'
