...
class TestNodeRelationshipInstitutions:
    ...
    def test_get_public_node(self, app, node, node_institutions_url):
        ...
        res = app.get(node_institutions_url)
    ...
    def test_remove_institutions_with_affiliated_user(
        ...
        res = app.put_json_api(node_institutions_url, {'data': []}, auth=user.auth)
    ...
    def test_using_post_making_no_changes_returns_204(
        self, app, user, institution_one, node, node_institutions_url, create_payload
    ):
        node.affiliated_institutions.add(institution_one)
        node.save()
        assert institution_one in node.affiliated_institutions.all()
        ...
        res = app.post_json_api(
            node_institutions_url, create_payload(institution_one._id), auth=user.auth
        )
    ...
    def test_add_through_patch_one_inst_while_removing_other(
        ...
        res = app.patch_json_api(
            node_institutions_url, create_payload(institution_two._id), auth=user.auth
        )
    ...
    def test_delete_user_is_admin(
        self, app, user, institution_one, node, make_resource_url, create_payload
    ):
        node.affiliated_institutions.add(institution_one)
        node.save()
    ...
    def test_admin_can_add_affiliated_institution(
        ...
        payload = {'data': [{'type': 'institutions', 'id': institution_one._id}]}
    ...
    def test_admin_can_remove_read_write_contributor_affiliated_institution(
        ...
        node.save()
        payload = {
            'data': [{'type': 'institutions', 'id': read_contrib_institution._id}]
        }
        res = app.delete_json_api(node_institutions_url, payload, auth=user.auth)
        node.reload()
        assert res.status_code == 204
    ...
    def test_read_write_contributor_can_add_affiliated_institution(
        ...
        payload = {
            'data': [{'type': 'institutions', 'id': write_contrib_institution._id}]
        }
    ...
    def test_read_write_contributor_can_remove_affiliated_institution(
        ...
        res = app.delete_json_api(
            node_institutions_url, payload, auth=write_contrib.auth
        )
        node.reload()
        assert res.status_code == 204
    ...
    def test_contribs_cannot_perform_action(
        ...
        payload = {'data': [{'type': 'institutions', 'id': institution_one._id}]}
        res = app.delete_json_api(
            node_institutions_url, payload, auth=write_contrib.auth, expect_errors=True
        )
        node.reload()
        assert res.status_code == 403
        assert institution_one in node.affiliated_institutions.all()
        ...
        payload = {'data': [{'type': 'institutions', 'id': institution_one._id}]}
        res = app.delete_json_api(
            node_institutions_url, payload, auth=read_contrib.auth, expect_errors=True
        )
        node.reload()
        assert res.status_code == 403
        assert institution_one in node.affiliated_institutions.all()
