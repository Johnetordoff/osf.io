...
class TestContributorDetail:
    ...
    def make_resource_url(self):
        def make_resource_url(resource_id, user_id):
            return '/{}nodes/{}/contributors/{}/'.format(API_BASE, resource_id, user_id)
    ...
    def test_get_contributor_detail_valid_response(
        ...
        assert res.json['data']['id'] == '{}-{}'.format(project_public._id, user._id)
    ...
    def test_get_contributor_detail_errors(
        ...
        res = app.get(
            url_private_base.format(non_contrib._id), auth=user.auth, expect_errors=True
        )
        assert res.status_code == 404

        #   test_get_private_node_invalid_user_detail_contributor_auth
        res = app.get(
            url_private_base.format('invalid'), auth=user.auth, expect_errors=True
        )
        assert res.status_code == 404

    def test_unregistered_contributor_detail_show_up_as_name_associated_with_project(
        self, app, user, project_public, project_private, make_resource_url
    ):
    ...
    def test_detail_includes_index(
        self, app, user, project_public, make_resource_url, url_public
    ):
        ...
        project_public.add_contributor(other_contributor, auth=Auth(user), save=True)

        other_contributor_detail = make_resource_url(
            project_public._id, other_contributor._id
        )
...
class TestNodeContributorOrdering:
    ...
    def url_creator(self, user, project):
        return '/{}nodes/{}/contributors/{}/'.format(API_BASE, project._id, user._id)

    @pytest.fixture()
    def urls_contrib(self, contribs, project):
        return [
            '/{}nodes/{}/contributors/{}/'.format(API_BASE, project._id, contrib._id)
            for contrib in contribs
        ]
...
class TestNodeContributorUpdate:
    ...
    def url_contrib(self, project, contrib):
        return '/{}nodes/{}/contributors/{}/'.format(API_BASE, project._id, contrib._id)

    def test_change_contrib_errors(self, app, user, contrib, project, url_contrib):
    ...
    def test_change_contributor_non_admin_osf_group_member_auth(
        ...
        res = app.put_json_api(
            url_contrib, data, auth=group_mem.auth, expect_errors=True
        )
        assert res.status_code == 403
    ...
    def test_change_contributor_permission_and_bibliographic(
        ...
        with assert_latest_log(
            NodeLog.PERMISSIONS_UPDATED, project, 1
        ), assert_latest_log(NodeLog.MADE_CONTRIBUTOR_INVISIBLE, project):
            contrib_id = '{}-{}'.format(project._id, contrib._id)
            data = {
                'data': {
                    'id': contrib_id,
...
class TestNodeContributorDelete:
    ...
    def test_remove_contributor_osf_group_member_read(
        ...
        res = app.delete(
            url_user_write_contrib, auth=group_mem.auth, expect_errors=True
        )
        assert res.status_code == 403
    ...
    def test_remove_contributor_non_admin_is_forbidden(
        self, app, user_write_contrib, user_non_contrib, project, url_user_non_contrib
    ):
        project.add_contributor(
    ...
    def test_remove_self_non_admin(
        self, app, user_non_contrib, project, url_user_non_contrib
    ):
        with assert_latest_log(NodeLog.CONTRIB_REMOVED, project):
            project.add_contributor(
                user_non_contrib, permissions=permissions.WRITE, visible=True, save=True
