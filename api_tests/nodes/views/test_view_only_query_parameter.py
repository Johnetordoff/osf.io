...
class TestNodeDetailViewOnlyLinks:
    def test_private_node(
        ...
        res_linked = app.get(
            private_node_one_url, {'view_only': private_node_one_private_link.key}
        )
        ...
        res = app.get(
            private_node_one_url,
            {'view_only': private_node_one_private_link.key, 'embed': 'contributors',},
        )
        assert res.status_code == 200
        ...
        res = app.get(
            public_node_one_url,
            {'view_only': public_node_one_anonymous_link.key, 'embed': 'contributors',},
        )
        assert res.status_code == 200
        embeds = res.json['data'].get('embeds', None)
        assert embeds is None or 'contributors' not in embeds
        ...
        res = app.get(
            public_node_one_url,
            {'view_only': public_node_one_private_link.key, 'embed': 'contributors',},
        )
        assert res.status_code == 200
        contributors = res.json['data']['embeds']['contributors']['data']
        for contributor in contributors:
        ...
        res = app.get(public_node_one_url, {'embed': 'contributors',})
        assert res.status_code == 200
        contributors = res.json['data']['embeds']['contributors']['data']
        for contributor in contributors:
            assert contributor['id'].split('-')[1] in valid_contributors
        ...
        payload = {
            'data': {
                'attributes': {'title': 'Cannot touch this'},
        ...
        res = app.get(
            private_node_one_url, {'view_only': private_node_one_anonymous_link.key,}
        )
        assert res.status_code == 200
            ...
            if isinstance(value, list):
                for relationship in value:
                    ...
                    if links.get('related', False):
                        assert (
                            private_node_one_private_link.key
                            in links['related']['href']
                        )
                    if links.get('self', False):
                        assert (
                            private_node_one_private_link.key in links['self']['href']
                        )
            else:
...
class TestNodeListViewOnlyLinks:
    def test_node_list_view_only_links(
        ...
        res = app.get(base_url, {'view_only': private_node_one_private_link.key,})
        assert res.status_code == 200
        nodes = res.json['data']
        node_ids = []
        for node in nodes:
