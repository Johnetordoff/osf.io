...
class TestNodeContributorsAndGroupMembers:
    def test_list_and_filter_contributors_and_group_members(
        ...
        url = '/{}nodes/{}/contributors_and_group_members/'.format(
        ...
        url = '/{}nodes/{}/contributors_and_group_members/?filter[given_name]={}'.format(
            API_BASE, project._id, group_manager.given_name
        )
        res = app.get(url, auth=admin_contributor.auth)
        assert res.status_code == 200
        assert res.content_type == 'application/vnd.api+json'
        ...
        url = '/{}nodes/{}/contributors_and_group_members/?filter[given_name]=NOT_EVEN_A_NAME'.format(
            API_BASE, project._id
        )
        res = app.get(url, auth=admin_contributor.auth)
        assert res.status_code == 200
