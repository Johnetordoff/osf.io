...
class TestNodeRelationshipNodeLinks:
    ...
    def test_get_relationship_linked_nodes(
        ...
        assert (
            node_linking_private.linked_nodes_related_url in res.json['links']['html']
        )
    ...
    def test_post_contributing_node(
        self, app, user, node_contrib, node_private, make_payload, url_private
    ):
        res = app.post_json_api(
            url_private, make_payload([node_contrib._id]), auth=user.auth
        )
    ...
    def test_post_public_node(
        self, app, user, node_private, node_public, make_payload, url_private
    ):
        res = app.post_json_api(
            url_private, make_payload([node_public._id]), auth=user.auth
        )
    ...
    def test_post_node_already_linked(
        self, app, user, node_private, make_payload, url_private
    ):
        res = app.post_json_api(
            url_private, make_payload([node_private._id]), auth=user.auth
        )

        assert res.status_code == 204

    def test_put_contributing_node(
        self, app, user, node_private, node_contrib, make_payload, url_private
    ):
    ...
    def test_put_private_node(
        self, app, user, node_private, node_other, make_payload, url_private
    ):
        res = app.put_json_api(
            url_private,
            make_payload([node_other._id]),
    ...
    def test_delete_with_put_empty_array(
        self, app, user, node_admin, node_linking_private, make_payload, url_private
    ):
        node_linking_private.add_pointer(node_admin, auth=Auth(user))
        empty_payload = make_payload()
        empty_payload['data'].pop()
        res = app.put_json_api(url_private, empty_payload, auth=user.auth)
        assert res.status_code == 200
    ...
    def test_delete_not_present(
        self, app, user, node_other, node_linking_private, make_payload, url_private
    ):
        number_of_links = node_linking_private.linked_nodes.count()
        res = app.delete_json_api(
            url_private, make_payload([node_other._id]), auth=user.auth
        )
        assert res.status_code == 204
    ...
    def test_delete_invalid_payload(self, app, user, node_linking_private, url_private):
        number_of_links = node_linking_private.linked_nodes.count()
        # No id in datum
        payload = {'data': [{'type': 'linked_nodes'}]}
        res = app.delete_json_api(
            url_private, payload, auth=user.auth, expect_errors=True
        )
    ...
    def test_node_errors(
        ...
        res = app.post_json_api(
            url_private, make_payload(['aquarela']), auth=user.auth, expect_errors=True
        )
        ...
        res = app.post_json_api(
            url_private,
            {'data': [{'type': 'not_linked_nodes', 'id': node_contrib._id}]},
            auth=user.auth,
            expect_errors=True,
        )
        ...
        res = app.post_json_api(
            url_private,
            {'data': [{'type': 'nodes', 'id': node_contrib._id}]},
            auth=user.auth,
            expect_errors=True,
        )
        ...
        res = app.post_json_api(
            '{}?version=2.13'.format(url_private),
            {'data': [{'type': 'linked_nodes', 'id': node_contrib._id}]},
            auth=user.auth,
            expect_errors=True,
        )
        ...
        res = app.post_json_api(
            url_public, make_payload([node_public._id]), expect_errors=True
        )
        ...
        res = app.post_json_api(
            url_private, make_payload([node_other._id]), expect_errors=True
        )
        ...
        res = app.post_json_api(
            url,
            {'data': [{'type': 'linked_nodes', 'id': node_parent_child.child._id}]},
            auth=user.auth,
            expect_errors=True,
        )
        ...
        url = '/{}nodes/{}/relationships/linked_nodes/'.format(API_BASE, node_self._id)
        res = app.post_json_api(
            url_private,
        ...
        res = app.post_json_api(
            url,
            {'data': [{'type': 'linked_nodes', 'id': node_parent_child.parent._id}]},
            auth=user.auth,
            expect_errors=True,
        )
...
class TestNodeLinkedNodes:
    ...
    def test_linked_nodes_returns_everything_2_13(
        ...
        node_types = [linked_node['type'] for linked_node in res.json['data']]
    ...
    def test_linked_nodes_doesnt_return_deleted_nodes(
        self, app, user, node_one, node_two, node_public, node_ids, url_linked_nodes
    ):
    ...
    def test_attempt_to_return_linked_nodes_logged_out(self, app, url_linked_nodes):
        res = app.get(url_linked_nodes, auth=None, expect_errors=True)
