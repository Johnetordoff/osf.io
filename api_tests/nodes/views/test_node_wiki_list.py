...
def create_wiki_payload(name):
    return {'data': {'type': 'wikis', 'attributes': {'name': name}}}
...
class TestNodeWikiList:
    ...
    def test_return_wikis(
        ...
        res = app.get(
            private_registration_url, auth=non_contrib.auth, expect_errors=True
        )
        assert res.status_code == 403
        assert (
            res.json['errors'][0]['detail']
        ...
        wiki_ids = [wiki['id'] for wiki in res.json['data']]
        assert (
            WikiPage.objects.get_for_node(private_registration, 'home')._id in wiki_ids
        )
    ...
    def test_relationship_links(
        ...
        assert (
            ...
            in res.json['data'][0]['relationships']['node']['links']['related']['href']
        )
        assert (
            expected_comments_relationship_url
            in res.json['data'][0]['relationships']['comments']['links']['related'][
                'href'
            ]
        )
...
class TestNodeWikiCreate(WikiCRUDTestCase):
    ...
    def url_registration_public(self, wiki_registration_public):
        return '/{}registrations/{}/wikis/'.format(
            API_BASE, wiki_registration_public.node._id
        )

    @pytest.fixture
    def url_registration_private(self, wiki_registration_private):
        return '/{}registrations/{}/wikis/'.format(
            API_BASE, wiki_registration_private.node._id
        )

    def test_create_public_wiki_page(
        self, app, user_write_contributor, url_node_public
    ):
        page_name = fake.word()
        res = app.post_json_api(
            url_node_public,
            create_wiki_payload(page_name),
            auth=user_write_contributor.auth,
        )
        assert res.status_code == 201
        assert res.json['data']['attributes']['name'] == page_name

    def test_create_public_wiki_page_with_content(
        self, app, user_write_contributor, url_node_public, project_public
    ):
        page_name = (
            'using random variables in tests can sometimes expose Testmon problems!'
        )
        payload = create_wiki_payload(page_name)
        payload['data']['attributes']['content'] = 'my first wiki page'
        res = app.post_json_api(
            url_node_public, payload, auth=user_write_contributor.auth
        )
        assert res.status_code == 201
        assert res.json['data']['attributes']['name'] == page_name
    ...
    def test_create_public_wiki_page_with_empty_content(
        self, app, user_write_contributor, url_node_public, project_public
    ):
        page_name = fake.word()
        payload = create_wiki_payload(page_name)
    ...
    def test_do_not_create_public_wiki_page(
        ...
        assert (
            res.json['errors'][0]['detail']
            == "A wiki page with the name 'home' already exists."
        )
        ...
        assert res.json['errors'][0][
            'detail'
        ] == "A wiki page with the name '{}' already exists.".format(
        ...
        res = app.post_json_api(
            url_node_public, create_wiki_payload(fake.word()), expect_errors=True
        )
        assert res.status_code == 401

    def test_create_private_wiki_page(
        self, app, user_write_contributor, url_node_private
    ):
        page_name = fake.word()
        res = app.post_json_api(
            url_node_private,
    ...
    def test_do_not_create_private_wiki_page(
        ...
        res = app.post_json_api(
            url_node_private, create_wiki_payload(fake.word()), expect_errors=True
        )
        assert res.status_code == 401

    def test_do_not_create_registration_wiki_page(
        self, app, user_creator, url_registration_public, url_registration_private
    ):
    ...
    def test_do_not_create_wiki_page_if_disabled(
        self, app, user_creator, project_public, url_node_public, wiki_public
    ):
