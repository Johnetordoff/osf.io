...
class TestGuid:
    ...
    @pytest.mark.parametrize(
        'Factory', [UserFactory, NodeFactory, RegistrationFactory,]
    )
...
@pytest.mark.django_db
class TestReferent:
    @pytest.mark.parametrize('Factory', [UserFactory, NodeFactory])
    ...
    def test_querying_with_multiple_guids(self, Factory):
        ...
        try:
            ...
            pytest.fail(
                'Multiple objects returned for {} with multiple guids. {}'.format(
                    Factory._meta.model, ex
                )
            )
...
class TestResolveGuid(OsfTestCase):
    ...
    def test_resolve_guid(self):
        res_guid = self.app.get(
            self.node.web_url_for('node_setting', _guid=True),
            auth=self.node.creator.auth,
        )
        res_full = self.app.get(
            self.node.web_url_for('node_setting'), auth=self.node.creator.auth
        )
    ...
    def test_resolve_guid_download_file(self):
        ...
        assert (
            '{}/v1/resources/{}/providers/{}{}?action=download&direct&version=1'.format(
        ...
        res = self.app.get(
            '/{}/download'.format(pp.primary_file.get_guid(create=True)._id)
        )
        assert res.status_code == 302
        assert (
            '{}/v1/resources/{}/providers/{}{}?action=download&direct&version=1'.format(
        ...
        assert res.status_code == 302
        assert (
            '{}/v1/resources/{}/providers/{}{}?action=download&direct&version=2'.format(
                WATERBUTLER_URL, pp._id, pp.primary_file.provider, pp.primary_file.path
            )
            in res.location
        )
        ...
        assert res.status_code == 302
        assert (
            '{}/v1/resources/{}/providers/{}{}?version=1&action=download&direct'.format(
                WATERBUTLER_URL, pp._id, pp.primary_file.provider, pp.primary_file.path
            )
            in res.location
        )
        ...
        res = self.app.get(
            unpub_pp.url + 'download/?version=1', auth=unpub_pp.creator.auth
        )
        assert res.status_code == 302
        assert (
            '{}/v1/resources/{}/providers/{}{}?version=1&action=download&direct'.format(
                WATERBUTLER_URL,
                unpub_pp._id,
                unpub_pp.primary_file.provider,
                unpub_pp.primary_file.path,
            )
            in res.location
        )
    ...
    @mock.patch(
        ...
        {'preprints': {'server': 'http://localhost:4200', 'path': '/preprints/'},},
    )
    ...
    def test_resolve_guid_download_file_from_emberapp_preprints_unpublished(self):
        ...
        provider = PreprintProviderFactory(
            _id='sockarxiv', name='Sockarxiv', reviews_workflow='pre-moderation'
        )

        # branded domains
        branded_provider = PreprintProviderFactory(
            _id='spot', name='Spotarxiv', reviews_workflow='pre-moderation'
        )
        ...
        pp = PreprintFactory(
            finish=True, provider=provider, is_published=False, creator=submitter
        )
        pp.run_submit(submitter)
        pp_branded = PreprintFactory(
            finish=True,
            provider=branded_provider,
            is_published=False,
    ...
    def test_resolve_guid_download_file_export(self):
        ...
        assert (
            '{}/v1/resources/{}/providers/{}{}%3Fformat%3Dasdf%26action%3Ddownload%26direct%26version%3D1'.format(
                quote(WATERBUTLER_URL),
                pp._id,
                pp.primary_file.provider,
                pp.primary_file.path,
            )
            in res.location
        )
        ...
        res = self.app.get(
            '/{}/download?format=asdf'.format(pp.primary_file.get_guid(create=True)._id)
        )
        assert res.status_code == 302
        assert '{}/export?format=asdf&url='.format(MFR_SERVER_URL) in res.location
        ...
        res = self.app.get(
            '/{}/download/?format=asdf'.format(
                pp.primary_file.get_guid(create=True)._id
            )
        )
        assert res.status_code == 302
        assert '{}/export?format=asdf&url='.format(MFR_SERVER_URL) in res.location
        ...
        assert '{}/export?format=asdf&url='.format(MFR_SERVER_URL) in res.location
        assert (
            '{}/v1/resources/{}/providers/{}{}%3F'.format(
                quote(WATERBUTLER_URL),
                pp._id,
                pp.primary_file.provider,
                pp.primary_file.path,
        ...
        unpub_pp = PreprintFactory(project=self.node, is_published=False)
        res = self.app.get(
            unpub_pp.url + 'download?format=asdf', auth=unpub_pp.creator.auth
        )
        assert res.status_code == 302
        assert res.status_code == 302
        assert '{}/export?format=asdf&url='.format(MFR_SERVER_URL) in res.location
    ...
    def test_resolve_guid_download_file_export_same_format_optimization(self):
        ...
        assert (
            '{}/v1/resources/{}/providers/{}{}?format=pdf&action=download&direct&version=1'.format(
                WATERBUTLER_URL, pp._id, pp.primary_file.provider, pp.primary_file.path
            )
            in res.location
        )
    ...
    def test_resolve_guid_download_errors(self):
        ...
        res = self.app.get(
            pp.url + 'download', auth=non_contrib.auth, expect_errors=True
        )
