...
from osf_tests.test_node import (
...
class TestDraftRegistrations:
    ...
    def test_factory(self):
        ...
        data = {'some': 'data'}
        draft = factories.DraftRegistrationFactory(
            registration_schema=schema, registration_metadata=data
        )
    ...
    def test_update_metadata_updates_registration_responses(self, project):
        ...
        draft = factories.DraftRegistrationFactory(
            registration_schema=schema, branched_from=project
        )
        new_metadata = {
    ...
    def test_draft_registrations_active(self):
        ...
        draft = factories.DraftRegistrationFactory(
            branched_from=project, user=project.creator
        )

        draft2 = factories.DraftRegistrationFactory(
            branched_from=project, user=project.creator
        )
        draft2.registered_node = deleted_registration
        draft2.save()

        finished_draft = factories.DraftRegistrationFactory(
            branched_from=project, user=project.creator
        )
    ...
    def test_update_metadata_interleaves_comments_by_created_timestamp(self, project):
        ...
        for time in times:
            comments.append({'created': time.isoformat(), 'value': 'Foo'})
        orig_data = {
            'foo': {'value': 'bar', 'comments': [comments[i] for i in range(0, 6, 2)]}
        }
        draft.update_metadata(orig_data)
        draft.save()
        assert draft.registration_metadata['foo']['comments'] == [
            comments[i] for i in range(0, 6, 2)
        ]

        new_data = {
            'foo': {'value': 'bar', 'comments': [comments[i] for i in range(1, 6, 2)]}
        }
    ...
    def test_draft_registration_url(self):
        ...
        assert draft.url == settings.DOMAIN + 'project/{}/drafts/{}'.format(
    ...
    def test_create_from_node_existing(self, user):
        ...
        draft = DraftRegistration.create_from_node(
            node=node, user=user, schema=factories.get_default_metaschema(),
        )
    ...
    def test_create_from_node_draft_node(self, user):
        draft = DraftRegistration.create_from_node(
            user=user, schema=factories.get_default_metaschema(),
        )
    ...
    def test_can_view_property(self, user):
        ...
        draft = DraftRegistration.create_from_node(
            user=user, node=project, schema=factories.get_default_metaschema()
        )
...
class TestSetDraftRegistrationEditableFields(TestNodeEditableFieldsMixin):
    ...
    def resource(self, project):
        return factories.DraftRegistrationFactory(
            branched_from=project, title='That Was Then', description='A description'
        )
...
class TestDraftRegistrationContributorMethods:
    ...
    def test_is_contributor(self, draft_registration):
        ...
        DraftRegistrationContributor.objects.create(
            user=contrib, draft_registration=draft_registration
        )
    ...
    def test_visible_initiator(self, project, user):
        ...
        draft_reg = factories.DraftRegistrationFactory(
            branched_from=project, initiator=user
        )
    ...
    def test_non_visible_initiator(self, project, user):
        ...
        project.add_contributor(
            contributor=invisible_user, permissions=ADMIN, visible=False
        )
        ...
        draft_reg = factories.DraftRegistrationFactory(
            branched_from=project, initiator=invisible_user
        )
        invisible_draft_reg_contributor = draft_reg.contributor_set.get(
            user=invisible_user
        )
    ...
    def test_visible_contributor_ids(self, draft_registration, user):
        ...
        DraftRegistrationContributor.objects.create(
            user=visible_contrib, draft_registration=draft_registration, visible=True
        )
        DraftRegistrationContributor.objects.create(
            user=invisible_contrib, draft_registration=draft_registration, visible=False
        )
    ...
    def test_set_visible_false(self, draft_registration, auth):
        contrib = factories.UserFactory()
        DraftRegistrationContributor.objects.create(
            user=contrib, draft_registration=draft_registration, visible=True
        )
        draft_registration.set_visible(contrib, visible=False, auth=auth)
        draft_registration.save()
        assert (
            DraftRegistrationContributor.objects.filter(
                user=contrib, draft_registration=draft_registration, visible=False
            ).exists()
            is True
        )
    ...
    def test_set_visible_is_noop_if_visibility_is_unchanged(
        self, draft_registration, auth
    ):
        visible, invisible = factories.UserFactory(), factories.UserFactory()
        DraftRegistrationContributor.objects.create(
            user=visible, draft_registration=draft_registration, visible=True
        )
        DraftRegistrationContributor.objects.create(
            user=invisible, draft_registration=draft_registration, visible=False
        )
    ...
    def test_remove_contributors(self, draft_registration, auth):
        ...
        draft_registration.remove_contributors(
            auth=auth, contributors=[user1, user2], save=True
        )
        draft_registration.reload()
    ...
    def test_replace_contributor(self, draft_registration):
        ...
        draft_registration.add_contributor(
            contrib, auth=Auth(draft_registration.creator)
        )
        draft_registration.save()
        ...
        assert draft_registration._id not in contrib.unclaimed_records.keys()
    ...
    def test_update_contributor(self, draft_registration, auth):
        ...
        draft_registration.update_contributor(new_contrib, READ, False, auth=auth)
    ...
    def test_update_contributor_non_admin_raises_error(self, draft_registration, auth):
        ...
        draft_registration.add_contributor(non_admin, permissions=WRITE, auth=auth)
        with pytest.raises(PermissionsError):
            draft_registration.update_contributor(
                non_admin, None, False, auth=Auth(non_admin)
            )

    def test_update_contributor_only_admin_raises_error(self, draft_registration, auth):
        with pytest.raises(DraftRegistrationStateError):
            draft_registration.update_contributor(auth.user, WRITE, True, auth=auth)

    def test_update_contributor_non_contrib_raises_error(
        self, draft_registration, auth
    ):
        non_contrib = factories.AuthUserFactory()
        with pytest.raises(ValueError):
            draft_registration.update_contributor(non_contrib, ADMIN, True, auth=auth)
...
class TestDraftRegistrationSubjects(TestNodeSubjects):
    ...
    def read_contrib(self, project):
        ...
        project.add_contributor(
            read_contrib, auth=Auth(project.creator), permissions=READ
        )
        project.save()
        return read_contrib
