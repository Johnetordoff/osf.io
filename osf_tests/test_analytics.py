...
class TestAnalytics(OsfTestCase):
    def test_get_total_activity_count(self):
        ...
        assert_equal(
            analytics.get_total_activity_count(user._id), user.get_activity_points()
        )

        analytics.increment_user_activity_counters(
            user._id, 'project_created', date.isoformat()
        )
...
def page_counter(project, file_node):
    ...
    page_counter, created = PageCounter.objects.get_or_create(
        _id=page_counter_id,
        resource=resource,
        file=file_node,
        version=None,
        action='download',
        date={u'2018/02/04': {u'total': 41, u'unique': 33}},
    )
    return page_counter
...
def page_counter2(project, file_node2):
    ...
    page_counter, created = PageCounter.objects.get_or_create(
        ...
        version=None,
        action='download',
        date={u'2018/02/04': {u'total': 4, u'unique': 26}},
    )
    return page_counter
...
def page_counter_for_individual_version(project, file_node3):
    ...
    page_counter, created = PageCounter.objects.get_or_create(
        ...
        version=0,
        action='download',
        date={u'2018/02/04': {u'total': 1, u'unique': 1}},
    )
    return page_counter
...
class TestPageCounter:
    ...
    def test_download_update_counter(self, mock_session, project, file_node):
        ...
        PageCounter.update_counter(
            resource, file_node, version=None, action='download', node_info={}
        )

        page_counter = PageCounter.objects.get(
            resource=resource, file=file_node, version=None, action='download'
        )
    ...
    @mock.patch('osf.models.analytics.session')
    def test_download_update_counter_contributor(
        self, mock_session, user, project, file_node
    ):
    ...
    def test_get_all_downloads_on_date_exclude_versions(
        self, page_counter, page_counter2, page_counter_for_individual_version
    ):
        """
