...
class TestRegistrationSchema:
    ...
    def test_get_latest_version(self, schema_name):
        assert (
            RegistrationSchema.objects.get_latest_version(
                name=schema_name
            ).schema_version
            == 3
        )
...
class TestRegistrationSchemaValidation:
    ...
    def registered_report_schema(self):
        schema = RegistrationSchema.objects.get(
            name='Registered Report Protocol Preregistration', schema_version=2
        )
        return schema
    ...
    def test_successful_validation(
        self, prereg_schema, prereg_test_data, osf_standard_schema, osf_standard_data
    ):
        validated = prereg_schema.validate_registration_responses(prereg_test_data)
        assert validated is True

        validated = osf_standard_schema.validate_registration_responses(
            osf_standard_data
        )
        assert validated is True
    ...
    def test_bad_validation(self, osf_standard_schema):
        ...
        bad_multiple_choice = {'datacompletion': 'Of course!'}
        ...
        assert (
            excinfo.value.message
            == 'For your registration, your response to '
    ...
    def test_uploader_validation(self, prereg_schema, prereg_test_data):
        ...
        assert (
            excinfo.value.message
            == "For your registration, your response to the 'q13.uploader' field is invalid. {} is not of type 'array'"
        )
        ...
        assert (
            excinfo.value.message
            == "For your registration, your response to the 'q13.uploader' field is invalid. 'hello' is not of type 'object'"
        )
        ...
        assert (
            excinfo.value.message
            == "For your registration, your response to the 'q13.uploader' field is invalid. Additional properties are not allowed ('bad_key' was unexpected)"
        )
        ...
        assert excinfo.value.message.startswith(
            "For your registration, your response to the 'q13.uploader' field is invalid."
        )
    ...
    def test_validate_required_fields(
        self, registered_report_schema, prereg_schema, prereg_test_data
    ):
        # Passing in required_fields is True enforces that required fields are present
        with pytest.raises(ValidationValueError) as excinfo:
            registered_report_schema.validate_registration_responses(
        ...
        with pytest.raises(ValidationValueError) as excinfo:
            prereg_schema.validate_registration_responses(
                prereg_test_data, required_fields=True
            )
        assert (
            excinfo.value.message
            == "For your registration, your response to the 'Existing Data' field is invalid, your response must be one of the provided options."
        )
