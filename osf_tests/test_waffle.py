...
from osf_tests.factories import AuthUserFactory, FlagFactory, ProjectFactory
...
class TestWaffleCookies(OsfTestCase):
    ...
    @mock.patch(
        'website.project.views.node._view_project',
    ...
    def test_waffle_leaves_cookie(self):
        ...
        resp = self.app.get(
            node.web_url_for('view_project'), auth=user.auth, auto_follow=True
        )
        ...
        assert waffle_cookie == cookie_str.format(
            'True'
        ) or waffle_cookie == cookie_str.format('False')
