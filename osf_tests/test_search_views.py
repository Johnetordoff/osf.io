...
class TestSearchViews(OsfTestCase):
    ...
    def test_search_views(self):
        ...
        assert_equal(
            res.json['results'][0]['social']['github'],
            'http://github.com/{}'.format(user_one.given_name),
        )
        assert_equal(
            res.json['results'][0]['social']['twitter'],
            'http://twitter.com/{}'.format(user_one.given_name),
        )
        assert_equal(
            res.json['results'][0]['social']['ssrn'],
            'http://papers.ssrn.com/sol3/cf_dev/AbsByAuth.cfm?per_id={}'.format(
                user_one.given_name
            ),
        )
        ...
        assert_equal(
            res.json['results'][0]['social']['profileWebsites'][0],
        ...
        assert_equal(
            res.json['results'][0]['social']['orcid'],
        ...
        assert_equal(
            res.json['results'][0]['social']['linkedIn'],
            'https://www.linkedin.com/{}'.format(user_two.given_name),
        )
        assert_equal(
            res.json['results'][0]['social']['scholar'],
            'http://scholar.google.com/citations?user={}'.format(user_two.given_name),
        )
...
class TestODMTitleSearch(OsfTestCase):
    ...
    def setUp(self):
        ...
        self.public_project = factories.ProjectFactory(
            creator=self.user_two, is_public=True, title='baz'
        )
