...
from osf.management.commands.check_crossref_dois import (
    check_crossref_dois,
    report_stuck_dois,
)
...
class TestCheckCrossrefDOIs:
    ...
    def crossref_response(self):
        with open(
            os.path.join(HERE, 'fixtures/crossref_works_response.json'), 'rb'
        ) as fp:
    ...
    def test_check_crossref_dois(self, crossref_response, stuck_preprint, preprint):
        doi = settings.DOI_FORMAT.format(
            prefix=stuck_preprint.provider.doi_prefix,
