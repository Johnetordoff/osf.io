...
def remove_notification_subscriptions_from_registrations(dry_run=True):
    ...
    notifications_to_delete = NotificationSubscription.objects.filter(
        node__type='osf.registration'
    )
    registrations_affected = Registration.objects.filter(
        id__in=notifications_to_delete.values_list('node_id', flat=True)
    )
    logger.info(
        '{} NotificationSubscriptions will be deleted.'.format(
            notifications_to_delete.count()
        )
    )
    logger.info(
