...
class TestFixRegistrationUnclaimedRecords:
    ...
    @pytest.fixture()
    def registration(
        self, project, contributor_unregistered, contributor_unregistered_no_email
    ):
    ...
    def test_migrate_bad_data(
        ...
        assert contributor_unregistered_no_email.unclaimed_records.get(
            registration._id, None
        )
        ...
        contributor_unregistered.refresh_from_db()
        contributor_unregistered_no_email.refresh_from_db()
        assert not contributor_unregistered.unclaimed_records.get(
            registration._id, False
        )
        ...
        record_two = contributor_unregistered_no_email.unclaimed_records.get(
            registration._id
        )
        assert record_two
