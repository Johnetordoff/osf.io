...
from scripts.analytics.migrate_analytics import (
    generate_events_between_events,
    fill_in_event_gaps,
)
...
class TestMigrateAnalytics(OsfTestCase):
    def setUp(self):
        ...
        self.keen_event = {
            ...
            'nodes': {'deleted': 0, 'total': 1, 'connected': 1, 'disconnected': 0},
        }
        ...
        self.keen_event_2 = {
            ...
            'nodes': {'deleted': 0, 'total': 5, 'connected': 4, 'disconnected': 1},
        }
        ...
        self.keen_event_3 = {
            ...
            'nodes': {'deleted': 0, 'total': 8, 'connected': 6, 'disconnected': 2},
        }

    def test_generate_events_between_events(self):
        generated_events = generate_events_between_events(
            [self.day_one, self.day_two], self.keen_event
        )
        ...
        expected_totals = [
            self.keen_event['nodes']['total'] for i in range(len(generated_events))
        ]
        assert_equals(returned_totals, expected_totals)

    def test_fill_in_event_gaps(self):
        filled_in_events = fill_in_event_gaps(
            'test', [self.keen_event, self.keen_event_2, self.keen_event_3]
        )
