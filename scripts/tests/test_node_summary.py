...
class TestNodeCount(OsfTestCase):
    def setUp(self):
        ...
        self.withdrawn_registration = WithdrawnRegistrationFactory(
            registration=self.reg_to_be_withdrawn, user=self.reg_to_be_withdrawn.creator
        )
    ...
    def test_counts(self):
        ...
        assert_equal(
            nodes['private'], 3
        )  # 1 Project, 1 Component, 1 spam node (spam is always private)
        ...
        assert_equal(
            registered_nodes['public'], 4
        )  # 3 Registrations, 1 Withdrawn registration
        ...
        assert_equal(
            registered_nodes['public_daily'], 4
        )  # 3 Registrations, 1 Withdrawn registration
        ...
        assert_equal(
            registered_projects['total'], 4
        )  # Not including a Registration Component
        ...
        assert_equal(
            registered_nodes['public_daily'], 0
        )  # 3 Registrations, 1 Withdrawn registration
