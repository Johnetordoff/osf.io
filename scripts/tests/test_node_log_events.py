...
class TestNodeLogAnalytics(OsfTestCase):
    def setUp(self):
        ...
        self.node_log_node_created = NodeLogFactory(
            action='node_created', user=self.user_one
        )
        self.node_log_file_added = NodeLogFactory(
            action='file_added', user=self.user_one
        )

        # Two node logs for user two
        self.node_log_wiki_updated = NodeLogFactory(
            action='wiki_updated', user=self.user_two
        )
    ...
    def test_results_structure(self):
        expected = [
            {
                'keen': {
                    'timestamp': self.node_log_node_created.date.replace(
                        tzinfo=pytz.UTC
                    ).isoformat()
                },
                'date': self.node_log_node_created.date.replace(
                    tzinfo=pytz.UTC
                ).isoformat(),
                'action': 'node_created',
            ...
            {
                'keen': {
                    'timestamp': self.node_log_file_added.date.replace(
                        tzinfo=pytz.UTC
                    ).isoformat()
                },
                'date': self.node_log_file_added.date.replace(
                    tzinfo=pytz.UTC
                ).isoformat(),
                'action': 'file_added',
                'user_id': self.user_one._id,
            ...
            {
                'keen': {
                    'timestamp': self.node_log_wiki_updated.date.replace(
                        tzinfo=pytz.UTC
                    ).isoformat()
                },
                'date': self.node_log_wiki_updated.date.replace(
                    tzinfo=pytz.UTC
                ).isoformat(),
                'action': 'wiki_updated',
            ...
            {
                'keen': {
                    'timestamp': self.node_log_project_created.date.replace(
                        tzinfo=pytz.UTC
                    ).isoformat()
                },
                'date': self.node_log_project_created.date.replace(
                    tzinfo=pytz.UTC
                ).isoformat(),
