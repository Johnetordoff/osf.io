...
from osf_tests.factories import AuthUserFactory, DraftRegistrationFactory
...
from osf.models.contributor import (
    get_contributor_permission,
...
class TestFixDraftNodePermissions:
    ...
    def bugged_reg(self, user, write_contrib):
        ...
        draft_registration.branched_from.add_contributor(
            write_contrib, permissions='write', visible=True
        )
        draft_registration.contributor_set.all().delete()
        draft_registration.created = datetime.datetime(
            2017, 2, 4, 0, 0
        )  # A date before the migration
    ...
    def test_fix_draft_node_permissions(
        self, app, bugged_reg, user, write_contrib, url
    ):
