...
def generate_preprint_csv(preprint_ids):
    search = PreprintDownload.search().aggs.metric(
        'times',
        {
            'date_histogram': {
                'field': 'timestamp',
    ...
    writer = csv.DictWriter(
        ...
        restval=0,
        fieldnames=[
            bucket['key_as_string'] for bucket in data.aggregations.times.buckets
        ],
    )
    writer.writeheader()
            ...
            writer.writerow(
                {
                    ...
                    for bucket in data.aggregations.times.buckets
                }
            )
        else:
...
def main():
    ...
    cli.add_argument(
        '--guids', nargs='*', type=str, default=preprint_guids_to_search,
    )
    args = cli.parse_args()
