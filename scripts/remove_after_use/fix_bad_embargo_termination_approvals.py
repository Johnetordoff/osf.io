...
def main(dry=True):
    ...
    logging.info(
        '{} EmbargoTerminationApprovals that are going to be marked as approved'.format(
            bad_state_embargo_termination_approvals.count()
        )
    )
    logging.info(
        'Affected Registrations: {}'.format(
            (
                list(
                    bad_state_embargo_termination_approvals.values_list(
                        'embargoed_registration__guids___id', flat=True
                    ).distinct()
                )
            )
        )
