...
def main():
    ...
    merged_users = OSFUser.objects.filter(
        merged_by__isnull=False, preprints__isnull=False
    )
    # Use merged_list to feed back into verify_merge after script has run.
    merged_list = [
        {'user_id': user._id, 'preprints': [prep._id for prep in user.preprints.all()]}
    ...
    for user in merged_users:
        ...
        logging.info(
            "Merging user {}'s preprints.. into merged_by {}".format(
                user._id, merged_by._id
            )
        )
        for preprint in user.preprints.all():
            ...
            print '   Perms: {}, {}'.format(
                user_contrib.permission,
                merged_by_contrib.permission if merged_by_contrib else None,
            )
            print '     Bib: {}, {}'.format(
                user_contrib.visible,
                merged_by_contrib.visible if merged_by_contrib else None,
            )
            print ' Creator: {}, {}'.format(
                preprint.creator == user, preprint.creator == merged_by
            )
...
def verify_merge(merged_list):
    ...
    for user_dict in merged_list:
        ...
        for preprint_id in user_dict['preprints']:
            ...
            print '    User: {}, Merged by: {}'.format(user._id, merged_by._id)
            print '   Perms: {}, {}'.format(
                user_contrib.permission if user_contrib else None,
                merged_by_contrib.permission if merged_by_contrib else None,
            )
            print '     Bib: {}, {}'.format(
                user_contrib.visible if user_contrib else None,
                merged_by_contrib.visible if merged_by_contrib else None,
            )
            print ' Creator: {}, {}'.format(
                preprint.creator == user, preprint.creator == merged_by
