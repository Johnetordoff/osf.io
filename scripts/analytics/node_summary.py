...
class NodeSummary(SummaryAnalytics):
    ...
    def get_events(self, date):
        ...
        registration_qs = Registration.objects.filter(
            is_deleted=False, created__lte=query_datetime
        )
        ...
        totals = {
            ...
            'nodes': {
                ...
                'total_daily_excluding_spam': node_qs.filter(daily_query)
                .filter(exclude_spam)
                .count(),
            ...
            'registered_projects': {
                ...
                'embargoed_v2': registration_qs.filter(private_query & embargo_v2_query)
                .get_roots()
                .count(),
                ...
                'public_daily': registration_qs.filter(public_query & daily_query)
                .get_roots()
                .count(),
                'embargoed_daily': registration_qs.filter(private_query & daily_query)
                .get_roots()
                .count(),
                'embargoed_v2_daily': registration_qs.filter(
                    private_query & daily_query & embargo_v2_query
                ...
                .get_roots()
                .count(),
                'withdrawn_daily': registration_qs.filter(retracted_query & daily_query)
                .get_roots()
                .count(),
            },
        }
