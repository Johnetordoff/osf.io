...
def parse_args():
    ...
    parser.add_argument(
        '-t', '--transfer', dest='transfer_collection', action='store_true'
    )
...
def validate_args(args):
    ...
    if args.dry:
        logger.info(
            'Running analytics on DRY RUN mode! No data will actually be sent to Keen.'
        )

    potential_operations = [
        args.smooth_events,
        args.transfer_collection,
        args.old_analytics,
    ]
    if len([arg for arg in potential_operations if arg]) > 1:
        raise ValueError(
            'You may only choose one analytic type to run: transfer, smooth, or import old analytics.'
        )
    ...
    if args.transfer_collection and not (
        args.source_collection and args.destination_collection
    ):
        raise ValueError(
            'To transfer between keen collections, enter both a source and a destination collection.'
        )

    if any([args.start_date, args.end_date]) and not all(
        [args.start_date, args.end_date]
    ):
        raise ValueError(
            'You must provide both a start and an end date if you provide either.'
        )

    if args.remove_event and not args.source_collection:
        raise ValueError(
            'You must provide both a source collection to remove an event from.'
        )
...
def fill_in_event_gaps(collection_name, events):
    ...
    given_days = [
        parse(event['keen']['timestamp']).date()
        for event in events
        if not event.get('generated')
    ]
    given_days.sort()
    ...
    if given_days:
        if collection_name == 'addon_snapshot':
            ...
            for provider in all_providers:
                for date_pair in date_chunks:
                    if (
                        date_pair[1] - date_pair[0] > datetime.timedelta(1)
                        and date_pair[0] != date_pair[1]
                    ):
                        first_event = [
                            event
                            for event in events
                            if date_from_event_ts(event) == date_pair[0]
                            and event['provider']['name'] == provider
                            and not event.get('generated')
                        ]
                        if first_event:
                            events_to_add += generate_events_between_events(
                                date_pair, first_event[0]
                            )
        elif collection_name == 'institution_summary':
            all_instutitions = list(
                set([event['institution']['name'] for event in events])
            )
                ...
                for date_pair in date_chunks:
                    if (
                        ...
                        first_event = [
                            ...
                            for event in events
                            if date_from_event_ts(event) == date_pair[0]
                            and event['institution']['name'] == institution
                            and not event.get('generated')
                        ]
                        if first_event:
                            events_to_add += generate_events_between_events(
        ...
        logger.info(
            'Generated {} events to add to the {} collection.'.format(
                len(events_to_add), collection_name
            )
        )
    else:
...
def extract_events_from_keen(client, event_collection, start_date=None, end_date=None):
    ...
    if start_date and end_date:
        logger.info(
            'Gathering events from the {} collection between {} and {}'.format(
                event_collection, start_date, end_date
            )
        )
        timeframe = {'start': start_date.isoformat(), 'end': end_date.isoformat()}
    else:
        logger.info(
            'Gathering events from the {} collection using timeframe {}'.format(
                event_collection, VERY_LONG_TIMEFRAME
            )
        )
...
def make_sure_keen_schemas_match(
    source_collection, destination_collection, keen_client
):
...
def transfer_events_to_another_collection(
    client, source_collection, destination_collection, dry, reverse=False
):
    ...
    :return: None
    """
    schemas_match = make_sure_keen_schemas_match(
        source_collection, destination_collection, client
    )
    if not schemas_match:
        raise ValueError(
            'The two provided schemas in keen do not match, you will need to do a bit more work.'
        )
    ...
    if reverse:
        ...
        logger.info(
            'Transferred {} events from the {} collection to the {} collection'.format(
                len(events_from_source), source_collection, destination_collection
            )
        )
...
def add_events_to_keen(client, collection, events, dry):
    logger.info(
        'Adding {} events to the {} collection...'.format(len(events), collection)
    )
    if not dry:
        client.add_events({collection: events})


def smooth_events_in_keen(
    client, source_collection, start_date, end_date, dry, reverse
):
    base_events = extract_events_from_keen(
        client, source_collection, start_date, end_date
    )
...
def remove_events_from_keen(client, source_collection, events, dry):
    for event in events:
        filters = [
            {
                ...
                'operator': 'eq',
                'property_value': event['keen']['timestamp'],
            }
        ]
        ...
        if filtered_event:
            ...
            filtered_event['keen']['timestamp'] = filtered_event['keen']['timestamp'][
                :10
            ]  # ends of timestamps differ
            ...
            if event != filtered_event:
                ...
                logger.info(
                    'About to delete a generated event from the {} collection from the date {}'.format(
...
def import_old_events_from_spreadsheet():
    ...
    dictReader = csv.DictReader(
        open(spreadsheet_path, 'rb'), fieldnames=col_names, delimiter=','
    )
    ...
    for event in events[
        3:
    ]:  # The first few rows have blank and/or bad data because they're extra headers
        node_event = {}
        user_event = {}
        addon_event = {}
    ...
    logger.info(
        'Gathered {} old user events, {} old node events and {} old dropbox addon events for keen'.format(
            len(user_events), len(node_events), len(addon_events)
        )
    )

    return {
        'user_summary': user_events,
        'node_summary': node_events,
        'addon_snapshot': addon_events,
    }
...
def format_event(event, analytics_type):
    user_event_template = {'status': {}, 'keen': {}}

    node_event_template = {'projects': {}, 'registered_projects': {}, 'keen': {}}

    addon_event_template = {'keen': {}, 'users': {}}
    ...
    if analytics_type == 'user':
        ...
        if event['total_users'] and event['active']:
            template_to_use['status']['unconfirmed'] = comma_int(
                event['total_users']
            ) - comma_int(event['active'])
        if event['profile_edited']:
            template_to_use['status']['profile_edited'] = comma_int(
        ...
        if event['registrations.total']:
            template_to_use['registered_projects']['total'] = comma_int(
                event['registrations.total']
            )
        if event['projects.total'] and event['projects.public']:
            template_to_use['projects']['private'] = (
                template_to_use['projects']['total']
                - template_to_use['projects']['public']
            )
    ...
    template_to_use['keen']['timestamp'] = (
        parse(event['timestamp']).replace(hour=12, tzinfo=pytz.UTC).isoformat()
    )
    template_to_use['imported'] = True

    formatted_event = {key: value for key, value in template_to_use.items() if value}
    if (
        len(formatted_event.items()) > 2
    ):  # if there's more than just the auto-added timestamp for keen
        return template_to_use


def remove_event_from_keen(client, source_collection, event_id):
    filters = [
        {'property_name': 'keen.id', 'operator': 'eq', 'property_value': event_id}
    ]
    client.delete_events(source_collection, filters=filters)
...
def main():
    ...
    if args.smooth_events:
        smooth_events_in_keen(
            client,
            args.source_collection,
            parse(args.start_date),
            parse(args.end_date),
            dry,
            reverse,
        )
        ...
        transfer_events_to_another_collection(
            client, args.source_collection, args.destination_collection, dry, reverse
        )
