...
class Sitemap(object):
    def __init__(self):
        ...
        if not settings.SITEMAP_TO_S3:
            ...
            assert (
                settings.SITEMAP_AWS_BUCKET
            ), 'SITEMAP_AWS_BUCKET must be set for sitemap files to be sent to S3'
            assert (
                settings.AWS_ACCESS_KEY_ID
            ), 'AWS_ACCESS_KEY_ID must be set for sitemap files to be sent to S3'
            assert (
                settings.AWS_SECRET_ACCESS_KEY
            ), 'AWS_SECRET_ACCESS_KEY must be set for sitemap files to be sent to S3'
    ...
    def write_doc(self):
        ...
        print(
            'Writing and gzipping `{}`: url_count = {}'.format(
                file_path, str(self.url_count)
            )
        )
    ...
    def ship_to_s3(self, name, path):
        data = open(path, 'rb')
        try:
            self.s3.Bucket(settings.SITEMAP_AWS_BUCKET).put_object(
                Key='sitemaps/{}'.format(name), Body=data
            )
        except Exception as e:
            logger.info('Error sending data to s3 via boto3')
            logger.exception(e)
            sentry.log_message(
                'ERROR: Sitemaps could not be uploaded to s3, see `generate_sitemap` logs'
            )
        data.close()
    ...
    def write_sitemap_index(self):
        ...
        for f in range(self.sitemap_count):
            ...
            loc_text = self.doc.createTextNode(
                urljoin(settings.DOMAIN, 'sitemaps/sitemap_{}.xml'.format(str(f)))
            )
            loc.appendChild(loc_text)

            datemod = doc.createElement('lastmod')
            sitemap.appendChild(datemod)
            datemod_text = self.doc.createTextNode(
    ...
    def log_errors(self, obj, obj_id, error):
        ...
        if self.errors == 1000:
            sentry.log_message(
                'ERROR: generate_sitemap stopped execution after reaching 1000 errors. See logs for details.'
            )
    ...
    def generate(self):
        ...
        objs = (
            ...
            .exclude(date_confirmed__isnull=True)
        ...
        objs = (
            AbstractNode.objects.filter(
                is_public=True, is_deleted=False, retraction_id__isnull=True
            )
        ...
        for obj in objs:
            try:
                ...
                config['loc'] = urljoin(
                    settings.DOMAIN, '/{}/'.format(obj['guids___id'])
                )
        ...
        objs = Preprint.objects.can_view().select_related(
            'node', 'provider', 'primary_file'
        )
            ...
            try:
                ...
                try:
                    ...
                    file_config['loc'] = urljoin(
                        ...
                        os.path.join(obj._id, 'download', '?format=pdf'),
                    )
        ...
        if (
            self.sitemap_count > settings.SITEMAP_INDEX_MAX * 0.90
        ):  # 10% of urls remaining
            sentry.log_message('WARNING: Max sitemaps nearly reached.')
        print(
            'Total url_count = {}'.format(
                (self.sitemap_count - 1) * settings.SITEMAP_URL_MAX + self.url_count
            )
        )
        print('Total sitemap_count = {}'.format(str(self.sitemap_count)))
        if self.errors:
            sentry.log_message(
                'WARNING: Generate sitemap encountered errors. See logs for details.'
            )
