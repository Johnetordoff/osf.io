...
def main():
    ...
    with transaction.atomic():
        ...
        users = OSFUser.objects.filter(
            ...
            date_registered__range=[start_time, end_time],
        )
        ...
        for user in users:
            if settings.MAILCHIMP_GENERAL_LIST not in user.mailchimp_mailing_lists:
                if not dry:
                    ...
                    logger.info(
                        'User {} has been subscribed to OSF general mailing list'.format(
                            user._id
                        )
                    )
                count += 1
        ...
        logger.info(
            '{} users have been subscribed to OSF general mailing list'.format(count)
        )
