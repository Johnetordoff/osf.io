...
def main(dry_run=True):
    old_sessions = Session.objects.filter(
        modified__lt=timezone.now() - datetime.timedelta(days=SESSION_AGE_THRESHOLD)
    )

    if dry_run:
        logger.warn('Dry run mode, will delete sessions and then abort the transaction')
    logger.info(
        'Preparing to delete Session objects older than {} days'.format(
            SESSION_AGE_THRESHOLD
        )
    )
    ...
    with transaction.atomic():
        ...
        logger.info(
            'Deleting {} Session objects took {} seconds'.format(
                sessions_deleted, end - start
            )
        )
