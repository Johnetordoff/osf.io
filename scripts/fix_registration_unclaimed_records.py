...
def main():
    ...
    with transaction.atomic():
        ...
        logger.info(
            'Found {} registrations with unregistered contributors'.format(qs.count())
        )
        for registration in qs:
            registration_id = registration._id
            logger.info(
                'Adding unclaimed_records for unregistered contributors on {}'.format(
                    registration_id
                )
            )
            ...
            for contributor in registration.contributors.filter(is_registered=False):
                ...
                if not record:
                    ...
                    logger.info(
                        'No record for node {} for user {}, inferring from other data'.format(
                            registered_from_id, contrib_id
                        )
                    )
                ...
                logger.info(
                    'Writing new unclaimed_record entry for user {} for registration {}.'.format(
                        contrib_id, registration_id
                    )
                )
