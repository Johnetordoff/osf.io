...
def add_http_scheme_to_gitlab_host(apps, schema_editor):
    ...
    for gitlab_account in gitlab_accounts:
        if not gitlab_account.oauth_secret.startswith('https://'):
            gitlab_account.oauth_secret = 'https://{}'.format(
                gitlab_account.oauth_secret
            )
            gitlab_account.save()
...
def revert_add_http_scheme_to_gitlab_host(apps, schema_editor):
    logger.warning(
        "No reverse migration for 0003_add_schemes_to_schemeless_hosts.py because we can't identify"
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(
            add_http_scheme_to_gitlab_host, revert_add_http_scheme_to_gitlab_host
        )
    ]
