...
class HookTestCase(StorageTestCase):
    def send_hook(
        self, view_name, view_kwargs, payload, target, method='get', **kwargs
    ):
...
class TestGetMetadataHook(HookTestCase):
    ...
    def test_file_metdata(self):
        ...
        record.save()
        res = self.send_hook(
            'osfstorage_get_metadata', {'fid': record.parent._id}, {}, self.node
        )
    ...
    def test_preprint_primary_file_metadata(self):
        ...
        record.save()
        res = self.send_hook(
            'osfstorage_get_metadata', {'fid': record.parent._id}, {}, preprint
        )
        assert_true(isinstance(res.json, dict))
        assert_equal(res.json, record.parent.serialize(True))
...
class TestUploadFileHook(HookTestCase):
    ...
    def test_upload_create(self):
        ...
        res = self.send_upload_hook(
            self.node_settings.get_root(), self.project, self.make_payload(name=name)
        )
    ...
    def test_upload_update(self):
        ...
        with AssertDeltas(delta):
            res = self.send_upload_hook(
                self.node_settings.get_root(), self.project, self.make_payload()
            )
            self.record.reload()
    ...
    def test_upload_duplicate(self):
        ...
        with AssertDeltas(Delta(lambda: self.record.versions.count())):
            res = self.send_upload_hook(
                self.node_settings.get_root(), payload=self.make_payload()
            )
            self.record.reload()
        assert_equal(res.status_code, 200)
        assert_equal(res.json['status'], 'success')
    ...
    def test_upload_create_child(self):
        ...
        assert_equals(
            record.versions.first().get_basefilenode_version(record).version_name, name
        )
    ...
    def test_upload_fail_to_create_version_due_to_checkout(self):
        ...
        file.save()
        res = self.send_upload_hook(
            root, payload=self.make_payload(name=name), expect_errors=True
        )
    ...
    def test_upload_weird_name(self):
        ...
        parent = self.node_settings.get_root().append_folder('cheesey')
        res = self.send_upload_hook(
            parent, payload=self.make_payload(name=name), expect_errors=True
        )
    ...
    def test_archive(self):
        ...
        parent = self.node_settings.get_root().append_folder('cheesey')
        res = self.send_upload_hook(
            parent, payload=self.make_payload(name=name, hashes={'sha256': 'foo'})
        )
        ...
        res = self.send_hook(
            ...
            target=self.project,
            payload={
                'metadata': {'vault': 'Vault 101', 'archive': '101 tluaV',},
        ...
        res = self.send_upload_hook(
            ...
            payload=self.make_payload(
                name=name,
                hashes={'sha256': 'foo'},
                metadata={'name': 'lakdjf', 'provider': 'testing',},
            ),
        )
...
class TestUploadFileHookPreprint(TestUploadFileHook):
    ...
    def test_upload_create(self):
        ...
        res = self.send_upload_hook(
            self.preprint.root_folder, self.preprint, self.make_payload(name=name)
        )
    ...
    def test_upload_update(self):
        ...
        with AssertDeltas(delta):
            res = self.send_upload_hook(
                self.preprint.root_folder, self.preprint, self.make_payload()
            )
            self.record.reload()
        assert_equal(res.status_code, 200)
        assert_equal(res.json['status'], 'success')
        ...
        assert_in(version, self.record.versions.all())
        assert_equal(
            self.record.versions.first()
            .get_basefilenode_version(self.record)
            .version_name,
            self.name,
        )
    ...
    def test_upload_fail_to_create_version_due_to_checkout(self):
        ...
        file.save()
        res = self.send_upload_hook(
            root, self.preprint, self.make_payload(name=name), expect_errors=True
        )
    ...
    def test_upload_weird_name(self):
        ...
        parent = self.preprint.root_folder.append_folder('cheesey')
        res = self.send_upload_hook(
            parent, self.preprint, self.make_payload(name=name), expect_errors=True
        )
    ...
    def test_upload_no_data(self):
        res = self.send_upload_hook(
            self.preprint.root_folder, self.preprint, expect_errors=True
        )
...
class TestUpdateMetadataHook(HookTestCase):
    ...
    def test_archived(self):
        ...
        assert_equal(
            self.version.metadata['archive'], 'Some really long glacier object id here'
        )
...
class TestGetRevisions(StorageTestCase):
    def setUp(self):
        ...
        self.record = recursively_create_file(self.node_settings, self.path)
        attach_versions(
            self.record, [factories.FileVersionFactory() for __ in range(15)]
        )
        self.record.save()
...
class TestCreateFolder(HookTestCase):
    ...
    def create_folder(self, name, parent=None, target=None, **kwargs):
        ...
        return self.send_hook(
            ...
            payload={'name': name, 'user': self.user._id, 'kind': 'folder'},
            target=self.project,
            method='post_json',
            **kwargs
        )
    ...
    def test_create_with_parent(self):
        ...
        resp = self.create_folder(
            'name', parent=OsfStorageFileNode.load(resp.json['data']['id'])
        )
        ...
        assert_equal(
            self.root_node.children.all()[0].children.all()[0].serialize(),
            resp.json['data'],
        )
...
class TestMoveHook(HookTestCase):
    ...
    def test_can_rename_file(self):
        ...
        assert_equal(
            file.versions.first().get_basefilenode_version(file).version_name, new_name
        )

    def test_can_move_file_out_of_quickfiles_node(self):
        quickfiles_node = QuickFilesNode.objects.get_for_user(self.user)
        quickfiles_file = create_test_file(
            quickfiles_node, self.user, filename='slippery.mp3'
        )
    ...
    def test_can_rename_file_in_quickfiles_node(self):
        quickfiles_node = QuickFilesNode.objects.get_for_user(self.user)
        quickfiles_file = create_test_file(
            quickfiles_node, self.user, filename='road_dogg.mp3'
        )
        quickfiles_folder = OsfStorageFolder.objects.get_root(target=quickfiles_node)
        dest_folder = OsfStorageFolder.objects.get_root(target=self.project)
        new_name = 'JesseJames.mp3'
...
class TestFileTags(StorageTestCase):
    ...
    def test_tag_the_same_tag(self):
        ...
        res = self.app.post_json(
            url, {'tag': 'Run_the_Jewels'}, auth=self.user.auth, expect_errors=True
        )
        assert_equal(res.status_code, 400)
    ...
    def test_remove_nonexistent_tag(self):
        ...
        res = self.app.delete_json(
            url, {'tag': 'Chance'}, auth=self.user.auth, expect_errors=True
        )
        assert_equal(res.status_code, 400)
        assert_equal(res.json['status'], 'failure')
    ...
    def test_file_add_tag_fail_doesnt_create_log(self, mock_log):
        ...
        url = api_url_for('osfstorage_add_tag', guid=self.node._id, fid=file._id)
        res = self.app.post_json(
            url, {'tag': 'The Life of Pablo'}, auth=self.user.auth, expect_errors=True
        )
    ...
    def test_file_remove_tag_creates_log(self):
        ...
        res = self.app.delete_json(
            url,
            {'tag': 'You that when you cause all this conversation'},
            auth=self.user.auth,
        )
    ...
    def test_file_remove_tag_fail_doesnt_create_log(self, mock_log):
        ...
        url = api_url_for('osfstorage_remove_tag', guid=self.node._id, fid=file._id)
        res = self.app.delete_json(
            url, {'tag': 'wonder'}, auth=self.user.auth, expect_errors=True
        )
...
class TestFileViews(StorageTestCase):
    def test_file_views(self):
        ...
        url = self.node.web_url_for(
            'addon_view_or_download_file', path=file._id, provider=file.provider
        )
