...
from website.project.decorators import must_not_be_registration, must_have_permission
...
def osfstorage_get_revisions(file_node, payload, target, **kwargs):
    ...
    qs = (
        FileVersion.includable_objects.filter(basefilenode__id=file_node.id)
        .include('creator__guids')
        .order_by('-created')
    )
    ...
    return {
        'revisions': [
            utils.serialize_revision(
                target, file_node, version, index=version_count - idx - 1, anon=is_anon
            )
...
def osfstorage_create_child(file_node, payload, **kwargs):
    ...
    if getattr(file_node.target, 'is_quickfiles', False) and is_folder:
        raise HTTPError(
            http_status.HTTP_400_BAD_REQUEST,
            data={'message_long': 'You may not create a folder for QuickFiles'},
        )
    ...
    if file_node.checkout and file_node.checkout._id != user._id:
        raise HTTPError(
            http_status.HTTP_403_FORBIDDEN,
            data={'message_long': 'File cannot be updated due to checkout status.'},
        )
...
def osfstorage_download(file_node, payload, **kwargs):
    ...
    return {
        'data': {'name': name, 'path': version.location_hash,},
        'settings': {
