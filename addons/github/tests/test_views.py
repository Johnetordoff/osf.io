...
class TestGitHubAuthViews(
    GitHubAddonTestCase, OAuthAddonAuthViewsTestCaseMixin, OsfTestCase
):
    @mock.patch(
...
class TestGitHubConfigViews(
    GitHubAddonTestCase, OAuthAddonConfigViewsTestCaseMixin, OsfTestCase
):
    folder = None
    ...
    def test_set_config(self, mock_repo, mock_add_hook):
        ...
        res = self.app.post_json(
            url,
            {'github_user': 'octocat', 'github_repo': 'repo_name',},
            auth=self.user.auth,
        )
...
class TestGithubViews(OsfTestCase):
    ...
    def test_permissions_no_auth(self):
        ...
        assert_false(
            check_permissions(
                self.node_settings, non_authenticated_auth, connection, branch
            )
        )
    ...
    def test_permissions(self, mock_has_auth):
        ...
        with mock.patch(
            ...
            assert_false(
                check_permissions(
                    self.node_settings, self.consolidated_auth, connection, 'master'
                )
            )
...
class TestGithubSettings(OsfTestCase):
    ...
    def test_link_repo(self, mock_repo, mock_add_hook):
        ...
        self.app.post_json(
            url,
            {'github_user': 'queen', 'github_repo': 'night at the opera',},
            auth=self.auth,
        ).maybe_follow()
    ...
    def test_link_repo_no_change(self, mock_repo, mock_add_hook):
        ...
        self.app.post_json(
            url,
            {'github_user': 'Queen', 'github_repo': 'Sheer-Heart-Attack',},
            auth=self.auth,
        ).maybe_follow()
