...
s3_set_config = generic_views.set_config(
    SHORT_NAME, FULL_NAME, S3Serializer, _set_folder
)
...
def s3_add_user_account(auth, **kwargs):
    ...
    try:
        ...
        account = ExternalAccount.objects.get(
            provider=SHORT_NAME, provider_id=user_info.id
        )
...
def create_bucket(auth, node_addon, **kwargs):
    ...
    try:
        ...
        return (
            {'message': e.message, 'title': 'Problem connecting to S3',},
            http_status.HTTP_400_BAD_REQUEST,
        )
        ...
        return (
            {'message': e.message, 'title': 'Error connecting to S3',},
            http_status.HTTP_400_BAD_REQUEST,
        )
