...
class TestAuthViews(
    FigshareAddonTestCase, OAuthAddonAuthViewsTestCaseMixin, OsfTestCase
):
    pass


class TestConfigViews(
    FigshareAddonTestCase, OAuthAddonConfigViewsTestCaseMixin, OsfTestCase
):
    ...
    def test_folder_list(self, mock_about, mock_folders):
        mock_folders.return_value = [
            {'path': 'fileset', 'name': 'Memes', 'id': '009001'}
        ]
    ...
    def test_set_config(self, mock_about):
        ...
        assert_equal(self.project.logs.latest().params['folder'], self.folder['path'])
