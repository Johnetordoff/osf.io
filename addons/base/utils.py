...
def get_addons_by_config_type(config_type, user):
    addons = [
        addon for addon in settings.ADDONS_AVAILABLE if config_type in addon.configs
    ]
    return [
...
def format_last_known_metadata(auth, node, file, error_type):
    ...
    if error_type != 'FILE_SUSPENDED' and (
        ...
        or (auth.private_key and auth.private_key in node.private_link_keys_active)
    ):
        ...
        parts = [
            ...
            """and found at path {} """.format(markupsafe.escape(path))
            if last_seen and path
            else '',
            """last found at path {} """.format(markupsafe.escape(path))
            if not last_seen and path
            else '',
            """with a file size of {} bytes""".format(size)
            if size and (last_seen or path)
            else '',
            """last seen with a file size of {} bytes""".format(size)
