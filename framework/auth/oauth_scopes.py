...
class ComposedScopes(object):
    ...
    USERS_READ = (
        ...
        CoreScopes.SUBSCRIPTIONS_READ,
        CoreScopes.ALERTS_READ,
        CoreScopes.USER_SETTINGS_READ,
    )
    ...
    NODE_METADATA_READ = (
        CoreScopes.NODE_BASE_READ,
        CoreScopes.NODE_CHILDREN_READ,
        CoreScopes.NODE_LINKS_READ,
        CoreScopes.NODE_CITATIONS_READ,
        CoreScopes.NODE_COMMENTS_READ,
        CoreScopes.NODE_LOG_READ,
        CoreScopes.NODE_FORKS_READ,
        CoreScopes.WIKI_BASE_READ,
        CoreScopes.LICENSE_READ,
        CoreScopes.IDENTIFIERS_READ,
        CoreScopes.NODE_PREPRINTS_READ,
        CoreScopes.PREPRINT_REQUESTS_READ,
    )
    NODE_METADATA_WRITE = NODE_METADATA_READ + (
        CoreScopes.NODE_BASE_WRITE,
        CoreScopes.NODE_CHILDREN_WRITE,
        CoreScopes.NODE_LINKS_WRITE,
        CoreScopes.IDENTIFIERS_WRITE,
        CoreScopes.NODE_CITATIONS_WRITE,
        CoreScopes.NODE_COMMENTS_WRITE,
        CoreScopes.NODE_FORKS_WRITE,
        CoreScopes.NODE_PREPRINTS_WRITE,
        CoreScopes.PREPRINT_REQUESTS_WRITE,
        CoreScopes.WIKI_BASE_WRITE,
    )
    ...
    PREPRINT_METADATA_READ = (
        CoreScopes.PREPRINTS_READ,
        CoreScopes.PREPRINT_CITATIONS_READ,
        CoreScopes.IDENTIFIERS_READ,
        CoreScopes.METRICS_BASIC,
    )
    PREPRINT_METADATA_WRITE = PREPRINT_METADATA_READ + (
        CoreScopes.PREPRINTS_WRITE,
        CoreScopes.PREPRINT_CITATIONS_WRITE,
        CoreScopes.METRICS_RESTRICTED,
    )

    # Organizer Collections collection
    # Using Organizer Collections and the node links they collect. Reads Node Metadata.
    ORGANIZER_READ = (
        CoreScopes.ORGANIZER_COLLECTIONS_BASE_READ,
        CoreScopes.COLLECTED_META_READ,
    ) + NODE_METADATA_READ
    ORGANIZER_WRITE = ORGANIZER_READ + (
        CoreScopes.ORGANIZER_COLLECTIONS_BASE_WRITE,
        CoreScopes.NODE_LINKS_WRITE,
        CoreScopes.COLLECTED_META_WRITE,
    )
    ...
    PREPRINT_DATA_WRITE = PREPRINT_DATA_READ + (CoreScopes.PREPRINT_FILE_WRITE,)

    # Privileges relating to who can access a node (via contributors or registrations)
    NODE_ACCESS_READ = (
        CoreScopes.NODE_CONTRIBUTORS_READ,
        CoreScopes.NODE_REGISTRATIONS_READ,
        CoreScopes.NODE_VIEW_ONLY_LINKS_READ,
        CoreScopes.REGISTRATION_VIEW_ONLY_LINKS_READ,
        CoreScopes.NODE_REQUESTS_READ,
        CoreScopes.NODE_SETTINGS_READ,
        CoreScopes.NODE_OSF_GROUPS_READ,
    )
    ...
    NODE_ALL_WRITE = (
        NODE_ALL_READ + NODE_METADATA_WRITE + NODE_DATA_WRITE + NODE_ACCESS_WRITE
    )

    # Combine preprint permissions
    PREPRINT_ALL_READ = (
        PREPRINT_METADATA_READ + PREPRINT_ACCESS_READ + PREPRINT_DATA_READ
    )
    PREPRINT_ALL_WRITE = (
        PREPRINT_ALL_READ
        + PREPRINT_METADATA_WRITE
        + PREPRINT_ACCESS_WRITE
        + PREPRINT_DATA_WRITE
    )
    ...
    REVIEWS_WRITE = (
        CoreScopes.ACTIONS_WRITE,
        CoreScopes.MODERATORS_WRITE,
        CoreScopes.PROVIDERS_WRITE,
    )
    ...
    ADMIN_LEVEL = (
        ...
        + APPLICATIONS_WRITE
        + TOKENS_WRITE
        + COMMENT_REPORTS_WRITE
        + USERS_CREATE
        + REVIEWS_WRITE
        + (
            CoreScopes.USER_EMAIL_READ,
            CoreScopes.USER_ADDON_READ,
            CoreScopes.NODE_ADDON_READ,
            CoreScopes.NODE_ADDON_WRITE,
            CoreScopes.WAFFLE_READ,
        )
    )
...
public_scopes = {
    'osf.full_read': scope(
        parts_=frozenset(ComposedScopes.FULL_READ),
        description='View all information associated with this account, including for '
        'private projects.',
        is_public=True,
    ),
    'osf.full_write': scope(
        parts_=frozenset(ComposedScopes.FULL_WRITE),
        description='View and edit all information associated with this account, including for '
        'private projects.',
        is_public=True,
    ),
    'osf.users.profile_read': scope(
        parts_=frozenset(ComposedScopes.USERS_READ),
        description='Read your profile data.',
        is_public=True,
    ),
    'osf.users.email_read': scope(
        parts_=frozenset(ComposedScopes.USER_EMAIL_READ),
...
if settings.DEV_MODE:
    public_scopes.update(
        {
            'osf.users.profile_write': scope(
                parts_=frozenset(ComposedScopes.USERS_WRITE),
                description='Read and edit your profile data.',
                is_public=True,
            ),
            'osf.nodes.metadata_read': scope(
                parts_=frozenset(ComposedScopes.NODE_METADATA_READ),
            ...
            'osf.nodes.access_write': scope(
                ...
                'registrations.',
                is_public=True,
            ),  # TODO: Language: Does registrations endpoint allow creation of registrations? Is that planned?
            'osf.nodes.full_read': scope(
                parts_=frozenset(ComposedScopes.NODE_ALL_READ),
            ...
            'osf.nodes.full_write': scope(
                parts_=frozenset(ComposedScopes.NODE_ALL_WRITE),
            ...
            'osf.users.create': scope(
                parts_=frozenset(ComposedScopes.USERS_CREATE),
            ...
            'osf.admin': scope(
                parts_=frozenset(ComposedScopes.ADMIN_LEVEL),
