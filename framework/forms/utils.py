...
def process_data(data, func):
    if isinstance(data, dict):
        return {key: process_data(value, func) for key, value in data.items()}
    elif isinstance(data, list):
        return [process_data(item, func) for item in data]
    return func(data)


def process_payload(data):
    return process_data(
        data, lambda value: quote(value.encode('utf-8') if value else '', safe=' ')
    )
