...
def format_user(user):
    ...
    person.attrs['identifiers'] = [
        GraphNode('agentidentifier', agent=person, uri='mailto:{}'.format(uri))
        for uri in user.emails.values_list('address', flat=True)
    ]
    ...
    if user.is_registered:
        person.attrs['identifiers'].append(
            GraphNode('agentidentifier', agent=person, uri=user.profile_image_url())
        )
    ...
    person.attrs['related_agents'] = [
        ...
        for institution in user.affiliated_institutions.all()
    ]
...
def format_subject(subject, context=None):
    ...
    context[subject.id] = GraphNode(
        'subject', name=subject.text, is_deleted=False, uri=subject.absolute_api_v2_url,
    )
