...
def webpack_asset(path, asset_paths=asset_paths, debug=settings.DEBUG_MODE):
    ...
    if not asset_paths:
        logger.warn(
            'webpack-assets.json has not yet been generated. Falling back to non-cache-busted assets'
        )
        return path
    if not debug:
...
def resolve_addon_path(addon_config, file_name):
    ...
    source_path = os.path.join(
        settings.ADDON_PATH, addon_config.short_name, 'static', file_name,
    )
    if os.path.exists(source_path):
        return os.path.join(
            '/', 'static', 'public', 'js', addon_config.short_name, file_name,
        )
    return None
