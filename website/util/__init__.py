...
# Move to api utils?
def api_url_for(
    view_name, _absolute=False, _xml=False, _internal=False, *args, **kwargs
):
    ...
    if _absolute:
        ...
        domain = (
            website_settings.INTERNAL_DOMAIN if _internal else website_settings.DOMAIN
        )
...
def api_v2_url(
    ...
    **kwargs
    ...
    params = (
        params or {}
    )  # Optional params dict for special-character param names, eg filter[fullname]
...
# Move to api utils?
def web_url_for(
    view_name, _absolute=False, _internal=False, _guid=False, *args, **kwargs
):
