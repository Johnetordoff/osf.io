...
def set_license(node, license_detail, auth, node_type='node'):
    ...
    if node_type == 'preprint':
        if not node.has_permission(auth.user, permissions.WRITE):
            raise framework_exceptions.PermissionsError(
                "You need admin or write permissions to change a {}'s license".format(
                    node_type
                )
            )
    else:
    ...
    try:
        ...
        raise osf_exceptions.NodeStateError(
            'Trying to update a {} with an invalid license'.format(node_type)
        )

    if node_type == 'preprint':
        if node.provider.licenses_acceptable.exists() and not node.provider.licenses_acceptable.filter(
            id=node_license.id
        ):
            raise framework_exceptions.PermissionsError(
    ...
    for required_property in node_license.properties:
        if not license_detail.get(required_property):
            raise ValidationError(
                '{} must be specified for this license'.format(required_property)
            )
