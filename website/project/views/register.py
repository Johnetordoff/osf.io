...
def node_register_page(auth, node, **kwargs):
    ...
    if node.is_registration:
        ...
        return redirect(
            node.web_url_for('node_registrations', view='draft', _guid=True)
        )
...
def node_registration_retraction_post(auth, node, **kwargs):
    ...
    try:
        ...
        raise HTTPError(
            http_status.HTTP_403_FORBIDDEN, data=dict(message_long=str(err))
        )
...
def node_register_template_page(auth, node, metaschema_id, **kwargs):
    ...
    if node.is_registration and bool(node.registered_schema):
        try:
            ...
            meta_schema = (
                RegistrationSchema.objects.filter(name=_id_to_name(metaschema_id))
        ...
        return redirect(
            node.web_url_for(
                'node_registrations', view=kwargs.get('template'), _guid=True
            )
        )
...
def project_before_register(auth, node, **kwargs):
    ...
    addon_set = [
        ...
        for n in itertools.chain(
            [node], node.get_descendants_recursive(primary_only=True)
        )
    ]
    for addon in itertools.chain(*addon_set):
        ...
        if name in settings.ADDONS_ARCHIVABLE:
            messages[settings.ADDONS_ARCHIVABLE[name]]['addons'].add(
    ...
    return {'prompts': prompts, 'errors': error_messages}
...
def registration_callbacks(node, payload, *args, **kwargs):
    ...
    if errors:
        node.archive_job.update_target(
            src_provider, ARCHIVER_FAILURE, errors=errors,
        )
    else:
