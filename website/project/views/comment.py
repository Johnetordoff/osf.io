...
def update_file_guid_referent(self, target, event_type, payload, user=None):
    ...
    source_node, destination_node = (
        Node.load(source['node']['_id']),
        Node.load(destination['node']['_id']),
    )
    ...
    file_guids = BaseFileNode.resolve_class(
        source['provider'], BaseFileNode.ANY
    ).get_file_guids(
    ...
    for guid in file_guids:
        ...
        if (
            source_node != destination_node
            and Comment.objects.filter(root_target___id=guid).count() != 0
        ):
            update_comment_node(guid, source_node, destination_node)

        if (
            source['provider'] != destination['provider']
            or source['provider'] != 'osfstorage'
        ):
...
def create_new_file(obj, source, destination, destination_node):
    ...
    if not source['path'].endswith('/'):
        ...
        new_file = BaseFileNode.resolve_class(
            destination['provider'], BaseFileNode.FILE
        ).get_or_create(destination_node, destination['path'])
        if destination['provider'] != 'osfstorage':
            new_file.update(revision=None, data=data)
    else:
        new_file = find_and_create_file_from_metadata(
            destination.get('children', []), source, destination, destination_node, obj
        )
        if not new_file:
            ...
            new_file = BaseFileNode.resolve_class(
                destination['provider'], BaseFileNode.FILE
            ).get_or_create(destination_node, new_path)
...
def find_and_create_file_from_metadata(
    children, source, destination, destination_node, obj
):
    ...
    for item in children:
        ...
        if item['kind'] == 'folder':
            return find_and_create_file_from_metadata(
                item.get('children', []), source, destination, destination_node, obj
            )
        elif (
            item['kind'] == 'file'
            and item['materialized'].replace(
                destination['materialized'], source['materialized']
            )
            == obj.referent.materialized_path
        ):
            data = dict(item)
            new_file = BaseFileNode.resolve_class(
                destination['provider'], BaseFileNode.FILE
            ).get_or_create(destination_node, item['path'])
            if destination['provider'] != 'osfstorage':
                new_file.update(revision=None, data=data)
            return new_file
...
def update_comment_node(root_target_id, source_node, destination_node):
    Comment.objects.filter(root_target___id=root_target_id).update(
...
def send_comment_added_notification(comment, auth, new_mentions=None):
    ...
    sent_subscribers = notify(
        event='comments', user=auth.user, node=node, timestamp=time_now, **context
    )
...
def send_mention_added_notification(comment, new_mentions, auth):
    ...
    context = dict(
        ...
        page_type='file'
        if comment.page == Comment.FILES
        else node.project_or_component,
...
def _update_comments_timestamp(auth, node, page=Comment.OVERVIEW, root_id=None):
    if node.is_contributor_or_group_member(auth.user):
        enqueue_postcommit_task(
            ban_url, (node,), {}, celery=False, once_per_request=True
        )
