...
def set_status_message(user):
    if user and not user.accepted_terms_of_service:
        status.push_status_message(
            message=language.TERMS_OF_SERVICE.format(
...
def get_globals():
    ...
    location = (
        geolite2.reader().get(request.remote_addr) if request.remote_addr else None
    )
    if request.host_url != settings.DOMAIN:
        try:
            inst_id = Institution.objects.get(
                domains__icontains=request.host, is_deleted=False
            )._id
    ...
    return {
        ...
        'sign_up_url': util.web_url_for(
            'auth_register', _absolute=True, next=request_login_url
        ),
        'reauth_url': util.web_url_for(
            'auth_logout', redirect_url=request.url, reauth=True
        ),
        ...
        'institutional_landing_flag': flag_is_active(
            request, features.INSTITUTIONAL_LANDING_FLAG
        ),
...
def is_private_link_anonymous_view():
    ...
    try:
        return (
            PrivateLink.objects.filter(key=request.args.get('view_only'))
            .values_list('anonymous', flat=True)
            .get()
        )
        ...
        if hasattr(request, 'waffles'):
            for k in request.waffles:
                ...
                resp.headers.add(
                    'Set-Cookie',
                    dump_cookie(
                        name.encode(), str(active), max_age=age, expires='True'
                    ),
                )
        return resp
...
def favicon():
    return send_from_directory(
        settings.STATIC_FOLDER, 'favicon.ico', mimetype='image/vnd.microsoft.icon'
    )
...
def robots():
    ...
    if os.path.exists(os.path.join(settings.STATIC_FOLDER, 'robots.local.txt')):
        robots_file = 'robots.local.txt'
    else:
        robots_file = 'robots.txt'
    return send_from_directory(settings.STATIC_FOLDER, robots_file, mimetype='html')
...
def sitemap_file(path):
    ...
    return send_from_directory(
        settings.STATIC_FOLDER + '/sitemaps/', path, mimetype=mime
    )
...
def ember_app(path=None):
    ...
    if not os.path.abspath(os.path.join(ember_app_folder, fp)).startswith(
        ember_app_folder
    ):
...
def make_url_map(app):
    ...
    process_rules(
        ...
        [
            ...
            Rule('/reproducibility/', 'get', website_views.reproducibility, notemplate),
            ...
            Rule(
                ['/getting-started/', '/getting-started/email/', '/howosfworks/'],
                'get',
    ...
    process_rules(
        ...
        [
            Rule(
                '/confirmed_emails/',
                'put',
                auth_views.unconfirmed_email_add,
                json_renderer,
            ),
            Rule(
                '/confirmed_emails/',
                'delete',
                auth_views.unconfirmed_email_remove,
                json_renderer,
            ),
        ],
        prefix='/api/v1',
    ...
    process_rules(
        ...
        [
            ...
            Rule(
                ...
                'get',
                website_views.forgot_password_form,
                json_renderer,
            ),
        ],
        prefix='/api/v1',
    ...
    process_rules(
        ...
        [
            ...
            Rule(
                ...
                OsfWebRenderer(
                    'public/resetpassword.mako', render_mako_string, trust=False
                ),
            ),
            # reset password post
            Rule(
            ...
            Rule(
                ...
                OsfWebRenderer(
                    'external_login_email.mako', render_mako_string, trust=False
                ),
            ),
            ...
            Rule(['/login/', '/account/'], 'get', auth_views.auth_login, notemplate),
    ...
    process_rules(
        ...
        [
            ...
            Rule(
                '/profile/<uid>/',
                'get',
                profile_views.profile_view_id_json,
                json_renderer,
            ),
            # Used by profile.html
            Rule(
                '/user/<uid>/<pid>/claim/email/',
                'post',
                project_views.contributor.claim_user_post,
                json_renderer,
            ),
            Rule(
                '/profile/export/', 'post', profile_views.request_export, json_renderer,
            ),
            Rule(
                '/profile/region/',
                'put',
            ...
            Rule(
                '/settings/names/', 'get', profile_views.serialize_names, json_renderer
            ),
            Rule(
                '/settings/names/',
                'put',
                profile_views.unserialize_names,
                json_renderer,
            ),
            Rule(
                '/settings/names/impute/',
                'get',
                profile_views.impute_names,
                json_renderer,
            ),
            Rule(
            ...
            Rule(
                ['/settings/jobs/', '/settings/jobs/<uid>/',],
                'get',
    ...
    process_rules(
        ...
        [
            ...
            Rule(
                '/api/v1/user/search/',
                'get',
                search_views.search_contributor,
                json_renderer,
            ),
            Rule(
                '/api/v1/search/node/',
    ...
    process_rules(
        ...
        [
            ...
            Rule(
                '/search/projects/',
                'get',
                search_views.search_projects_by_title,
                json_renderer,
            ),
            Rule(
                '/share/search/',
                'get',
                website_views.legacy_share_v1_search,
                json_renderer,
            ),
        ],
        prefix='/api/v1',
    ...
    process_rules(
        ...
        [
            Rule(
                ...
                'get',
                institution_views.view_institution,
                OsfWebRenderer('institution.mako', trust=False),
            )
        ],
    )
    ...
    process_rules(
        ...
        [
            ...
            Rule(
                '/project/new/<pid>/beforeTemplate/',
                'get',
                project_views.node.project_before_template,
                json_renderer,
            ),
            Rule(
                [
    ...
    process_rules(
        ...
        [
            Rule(
                '/email/meeting/', 'post', conference_views.meeting_hook, json_renderer,
            ),
            Rule(
                '/mailchimp/hooks/',
                'get',
                profile_views.mailchimp_get_endpoint,
                json_renderer,
            ),
            Rule(
                '/mailchimp/hooks/',
                'post',
                profile_views.sync_data_from_mailchimp,
                json_renderer,
            ),
            ...
            Rule(
                '/project/new/',
                'post',
                project_views.node.project_new_post,
                json_renderer,
            ),
            Rule(
                [
            ...
            Rule(
                ['/pointer/',], 'post', project_views.node.add_pointer, json_renderer,
            ),
            Rule(
                ['/project/<pid>/pointer/', '/project/<pid>/node/<nid>pointer/',],
                'delete',
            ...
            Rule(
                ...
                'get',
                project_views.contributor.get_contributors_from_parent,
                json_renderer,
            ),
            ...
            # Reorder components
            Rule(
                '/project/<pid>/reorder_components/',
                'post',
                project_views.node.project_reorder_components,
                json_renderer,
            ),
            # Edit node
            Rule(
            ...
            Rule(
                ...
                'post',
                project_views.tag.project_add_tag,
                json_renderer,
            ),
            Rule(
                [
                ...
                'delete',
                project_views.tag.project_remove_tag,
                json_renderer,
            ),
            ...
            Rule(
                ['/project/<pid>/files/grid/', '/project/<pid>/node/<nid>/files/grid/'],
                'get',
            ...
            Rule(
                ['/project/<pid>/tree/', '/project/<pid>/node/<nid>/tree/'],
                'get',
    ...
    if settings.DEV_MODE:
        ...
        def ember_cli_live_reload():
            req = requests.get(
                '{}/ember-cli-live-reload.js'.format(settings.LIVE_RELOAD_DOMAIN),
                stream=True,
            )
            return Response(
