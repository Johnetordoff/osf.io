...
from website.notifications.utils import (
    ...
    subscribe_user_to_global_notifications,
)
...
def subscribe_contributor(node, contributor, auth=None, *args, **kwargs):
    try:
        ...
        logger.warn(
            'Skipping subscription of user {} to node {}'.format(contributor, node._id)
        )
        logger.warn('Reason: {}'.format(str(err)))
...
def subscribe_confirmed_user(user):
    try:
        ...
        logger.warn(
            'Skipping subscription of user {} to global subscriptions'.format(user)
        )
        logger.warn('Reason: {}'.format(str(err)))
