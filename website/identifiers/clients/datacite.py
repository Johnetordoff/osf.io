...
class DataCiteClient(AbstractIdentifierClient):
    ...
    def build_metadata(self, node):
        ...
        data = {
            ...
            'titles': [{'title': node.title}],
            'publisher': 'Open Science Framework',
            'publicationYear': str(datetime.datetime.now().year),
            'resourceType': {'resourceType': 'Project', 'resourceTypeGeneral': 'Text'},
        }

        if node.description:
            data['descriptions'] = [
                {'descriptionType': 'Abstract', 'description': node.description}
            ]

        if node.node_license:
            data['rightsList'] = [
                {'rights': node.node_license.name, 'rightsURI': node.node_license.url}
            ]
    ...
    def create_identifier(self, node, category):
        if category == 'doi':
            ...
            doi = re.match(r'OK \((?P<doi>[a-zA-Z0-9 .\/]{0,})\)', resp).groupdict()[
                'doi'
            ]
            ...
            raise NotImplementedError(
                'Creating an identifier with category {} is not supported'.format(
                    category
                )
            )
