...
class CrossRefClient(AbstractIdentifierClient):
    ...
    def build_doi(self, preprint):
        ...
        prefix = (
            preprint.provider.doi_prefix
            or PreprintProvider.objects.get(_id='osf').doi_prefix
        )
        return settings.DOI_FORMAT.format(prefix=prefix, guid=preprint._id)

    def build_metadata(
        self, preprint, status='public', include_relation=True, **kwargs
    ):
        ...
        element = lxml.builder.ElementMaker(
            nsmap={None: CROSSREF_NAMESPACE, 'xsi': XSI},
        )
        ...
        for preprint in preprints:
            body.append(
                self.build_posted_content(preprint, element, status, include_relation)
            )

        root = element.doi_batch(head, body, version=CROSSREF_SCHEMA_VERSION)
    ...
    def build_posted_content(self, preprint, element, status, include_relation):
        ...
        posted_content = element.posted_content(
            element.group_title(preprint.provider.name), type='preprint'
        )
        if status == 'public':
            posted_content.append(
                element.contributors(
                    *self._crossref_format_contributors(element, preprint)
                )
            )

        title = (
            element.title(remove_control_characters(preprint.title))
            if status == 'public'
            else element.title('')
        )
        ...
        posted_content.append(
            element.posted_date(
                *self._crossref_format_date(element, preprint.date_published)
            )
        )
        ...
        if status == 'public':
            ...
            if preprint.description:
                posted_content.append(
                    element.abstract(
                        element.p(remove_control_characters(preprint.description)),
                        xmlns=JATS_NAMESPACE,
                    )
                )
            ...
            if preprint.license and preprint.license.node_license.url:
                ...
                posted_content.append(element.program(xmlns=CROSSREF_ACCESS_INDICATORS))
        ...
        doi_data = [element.doi(doi), element.resource(settings.DOMAIN + preprint._id)]
    ...
    def _crossref_format_contributors(self, element, preprint):
        ...
        for index, contributor in enumerate(preprint.visible_contributors):
            ...
            if name_parts.get('suffix'):
                person.append(
                    element.suffix(remove_control_characters(name_parts['suffix']))
                )
            if contributor.external_identity.get('ORCID'):
                orcid = list(contributor.external_identity['ORCID'].keys())[0]
                verified = (
                    list(contributor.external_identity['ORCID'].values())[0]
                    == 'VERIFIED'
                )
                if orcid and verified:
                    person.append(
                        element.ORCID(
                            'https://orcid.org/{}'.format(orcid), authenticated='true'
                        )
                    )
    ...
    def get_status(self, preprint):
        return (
            'public'
            if preprint.verified_publishable and not preprint.is_retracted
            else 'unavailable'
        )
...
class ECSArXivCrossRefClient(CrossRefClient):
    def get_credentials(self):
        return (
            settings.ECSARXIV_CROSSREF_USERNAME,
            settings.ECSARXIV_CROSSREF_PASSWORD,
        )
