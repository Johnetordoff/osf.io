...
@requires_search
def update_preprint(
    preprint, index=None, bulk=False, async_update=True, saved_fields=None
):
    kwargs = {'index': index, 'bulk': bulk}
    if async_update:
        ...
        if settings.USE_CELERY:
            enqueue_task(
                search_engine.update_preprint_async.s(preprint_id=preprint_id, **kwargs)
            )
        else:
...
@requires_search
def update_group(
    group, index=None, bulk=False, async_update=True, saved_fields=None, deleted_id=None
):
    ...
    if async_update:
        ...
        if settings.USE_CELERY:
            enqueue_task(
                search_engine.update_group_async.s(group_id=group._id, **kwargs)
            )
        else:
...
def update_collected_metadata(cgm_id, collection_id=None, index=None, op='update'):
    ...
    if settings.USE_CELERY:
        enqueue_task(
            search_engine.update_cgm_async.s(
                cgm_id, collection_id=collection_id, op=op, index=index
            )
        )
    else:
        search_engine.update_cgm_async(
            cgm_id, collection_id=collection_id, op=op, index=index
        )
...
def search_contributor(query, page=0, size=10, exclude=None, current_user=None):
    ...
    result = search_engine.search_contributor(
        query=query, page=page, size=size, exclude=exclude, current_user=current_user
    )
    return result
