...
preprints_dir = os.path.abspath(
    os.path.join(os.getcwd(), EXTERNAL_EMBER_APPS['preprints']['path'])
)
ember_osf_web_dir = os.path.abspath(
    os.path.join(os.getcwd(), EXTERNAL_EMBER_APPS['ember_osf_web']['path'])
)
        ...
        summary.update(
            {
                ...
                'is_contributor_or_group_member': node.is_contributor_or_group_member(
                    user
                ),
                ...
                if len(node.description) <= 150
                else node.description[0:150] + '...',
            }
        )
    else:
...
def find_bookmark_collection(user):
    ...
    return Collection.objects.get(
        creator=user, deleted__isnull=True, is_bookmark_collection=True
    )
...
def validate_page_num(page, pages):
    if page < 0 or (pages and page >= pages):
        raise HTTPError(
            ...
            data=dict(message_long='Invalid value for "page".'),
        )
...
def _build_guid_url(base, suffix=None):
    url = '/'.join([each.strip('/') for each in [base, suffix] if each])
...
def resolve_guid(guid, suffix=None):
    ...
    try:
        ...
        if (
            e.message == 'osfstorageguidfile'
        ):  # Used when an old detached OsfStorageGuidFile object is accessed
    ...
    if guid_object:
        ...
        if suffix and suffix.rstrip('/').lower() == 'download':
            ...
            if file_referent:
                if (
                    isinstance(file_referent.target, Preprint)
                    and not file_referent.target.is_published
                ):
                    ...
                    if auth.user is None or not (
                        auth.user.has_perm(
                            'view_submissions', file_referent.target.provider
                        )
        ...
        if isinstance(referent, Preprint):
            if referent.provider.domain_redirect_enabled:
                ...
                return redirect(
                    referent.absolute_url, http_status.HTTP_301_MOVED_PERMANENTLY
                )

            if PROXY_EMBER_APPS:
                resp = requests.get(
                    EXTERNAL_EMBER_APPS['preprints']['server'],
                    stream=True,
                    timeout=EXTERNAL_EMBER_SERVER_TIMEOUT,
                )
                return Response(
                    stream_with_context(resp.iter_content()), resp.status_code
                )
        ...
        if (
            isinstance(referent, BaseFileNode)
            and referent.is_file
            and (getattr(referent.target, 'is_quickfiles', False))
        ):
        ...
        if isinstance(referent, Registration) and (
            not suffix
            or suffix.rstrip('/').lower() in ('comments', 'links', 'components')
        ):
...
def legacy_share_v1_search(**kwargs):
    return HTTPError(
        http_status.HTTP_400_BAD_REQUEST,
        data=dict(
            message_long='Please use v2 of the SHARE search API available at {}api/v2/share/search/creativeworks/_search.'.format(
                settings.SHARE_URL
            )
        ),
    )
...
def get_storage_region_list(user, node=False):
    ...
    available_regions.insert(
        0, available_regions.pop(available_regions.index(default_region))
    )  # default should be at top of list for UI.
