...
def should_update_preprint_identifiers(preprint, old_subjects, saved_fields):
    ...
    return (
        ...
        preprint
        and preprint.date_published
        and not (saved_fields and 'preprint_doi_created' in saved_fields)
        and
...
def update_or_enqueue_on_preprint_updated(
    preprint_id,
    update_share=True,
    share_type=None,
    old_subjects=None,
    ...
    try:
        ...
        if resp.status_code >= 500:
            ...
            raise self.retry(
                ...
                * min(
                    60 + settings.CELERY_RETRY_BACKOFF_BASE ** self.request.retries,
                    60 * 10,
                ),
            )
        else:
...
def format_preprint(preprint, share_type, old_subjects=None):
    ...
    if preprint.get_identifier('doi'):
        to_visit.append(
            GraphNode(
                'workidentifier',
                creative_work=preprint_graph,
                uri='https://doi.org/{}'.format(preprint.get_identifier('doi').value),
            )
        )
    ...
    if preprint.article_doi:
        ...
        to_visit.append(
            GraphNode('workrelation', subject=preprint_graph, related=related_work)
        )
        to_visit.append(
            GraphNode(
                'workidentifier',
    ...
    preprint_graph.attrs['tags'] = [
        GraphNode(
            'throughtags', creative_work=preprint_graph, tag=GraphNode('tag', name=tag)
        )
