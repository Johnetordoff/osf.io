def serialize_group(osf_group):
    ...
    return {
        ...
        'managers': [
            serialize_member(manager, osf_group) for manager in osf_group.managers
        ],
        'members': [
            serialize_member(member, osf_group) for member in osf_group.members_only
        ],
        'nodes': [
            serialize_node_for_groups(node, osf_group) for node in osf_group.nodes
        ],
    }
...
def serialize_member(member, osf_group):
    ...
    return {
        ...
        else member.unclaimed_records[osf_group._id].get('name'),
        'id': member._id,
    }
