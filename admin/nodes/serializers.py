...
def serialize_node(node):
    ...
    return {
        ...
        'contributors': [
            serialize_simple_user_and_node_permissions(node, user)
        ...
        'registrations': [
            serialize_node(registration) for registration in node.registrations.all()
        ],
        'registered_from': node.registered_from._id if node.registered_from else None,
        'osf_groups': [
            serialize_groups_for_node(node, group) for group in list(node.osf_groups)
        ],
    }
...
def serialize_simple_user_and_node_permissions(node, user):
    ...
    return {
        'id': user._id,
        'name': user.fullname,
        'permission': Contributor.objects.get(
